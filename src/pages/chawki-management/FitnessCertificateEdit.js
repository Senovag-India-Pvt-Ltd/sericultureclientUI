...
const fitnessCertificateEditStyles = `
  .sh-page-header {
    padding: 12px 20px;
...
function FitnessCertificateEdit() {
  ...
  // Dropdown lists
  const [diseaseStatusList, setDiseaseStatusList] = useState([]);
  const [raceList, setRaceList] = useState([]);
  const [grainageList, setGrainageList] = useState([]);

  useEffect(() => {
    api
      .get(baseURLMasterData + `diseaseStatus/get-all`)
      .then((res) => setDiseaseStatusList(res.data.content.diseaseStatus || []))
      .catch(() => setDiseaseStatusList([]));
  }, []);
  ...
  const loadFitnessList = (farmerId) => {
    setListLoading(true);
    api
      .get(`${baseURL}cropInspection/getFitnessCertificatePath/${farmerId}`)
      .then((response) => {
  ...
  const handleEditClick = (row) => {
    ...
    setEditData({
      ...
      expectedCocoon: row.expectedCocoon ?? "",
  ...
  const saveEdit = () => {
    ...
    api
      .post(baseURL + `cropInspection/editFitnessCertificateDetails`, {
        ...
        expectedCocoon: editData.expectedCocoon,
  ...
  return (
    <Layout title={t("Fitness Certificate Edit")}>
      <style>{fitnessCertificateEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2" className="sh-page-title">{t("Fitness Certificate Edit")}</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
      ...
      <Block className="mt-n2">
        ...
        {farmerDetails && (
          <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", marginBottom: "20px", overflow: "hidden" }}>
            <Card.Header style={{ background: "linear-gradient(135deg, #1e67a8 0%, #0d4f8a 100%)", padding: "14px 24px" }}>
              <span style={{ color: "white", fontWeight: 700, fontSize: "1rem" }}>📋 {t("Fitness Certificate List")}</span>
            </Card.Header>
            <Card.Body style={{ padding: 0 }}>
              {listLoading ? (
                ...
                <div className="d-flex flex-column justify-content-center align-items-center" style={{ height: "120px", color: "#aaa" }}>
                  ...
                  <p className="mt-2 mb-0">{t("No fitness certificates found.")}</p>
                </div>
              ) : (
                  ...
                  <table className="table mb-0" style={{ fontSize: "0.875rem" }}>
                    <thead>
                      <tr style={{ background: "#f0f6ff" }}>
                        ...
                        <th style={{ padding: "12px 16px", color: "#1e67a8", fontWeight: 700 }}>{t("Spun To")}</th>
                        <th style={{ padding: "12px 16px", color: "#1e67a8", fontWeight: 700 }}>{t("No. of Chandies")}</th>
                        <th style={{ padding: "12px 16px", color: "#1e67a8", fontWeight: 700 }}>{t("Expected Cocoon")}</th>
                        <th style={{ padding: "12px 16px", color: "#1e67a8", fontWeight: 700, textAlign: "center" }}>{t("Action")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {fitnessList.map((row, index) => (
                        <tr
                        ...
                        >
                          ...
                          <td style={{ padding: "11px 16px" }}>{row.spunToDate ? row.spunToDate.substring(0, 10) : "—"}</td>
        ...
        {editData && (
          ...
          >
            <Card.Header style={{ background: "linear-gradient(135deg, #1e67a8 0%, #0d4f8a 100%)", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ color: "white", fontWeight: 700, fontSize: "1rem" }}>✏️ {t("Edit Fitness Certificate")}</span>
              <button
                type="button"
            ...
            <Card.Body style={{ padding: "24px" }}>
              <Row className="g-4">
                ...
                <Col lg={4} md={6}>
                  <Form.Group>
                    <Form.Label style={labelStyle}>{t("No. of Chandies")}</Form.Label>
                    <Form.Control name="noOfChandies" value={editData.noOfChandies} onChange={handleEditInput} type="number" placeholder={t("Enter No. of Chandies")} style={inputStyle} />
                  </Form.Group>
                </Col>

                <Col lg={4} md={6}>
                  <Form.Group>
                    <Form.Label style={labelStyle}>{t("Expected Cocoon")}</Form.Label>
                    <Form.Control name="expectedCocoon" value={editData.expectedCocoon} onChange={handleEditInput} type="number" placeholder={t("Enter Expected Cocoon")} style={inputStyle} />
                  </Form.Group>
                </Col>
                ...
                <Col lg={4} md={6}>
                  <Form.Group>
                    <Form.Label style={labelStyle}>{t("Disease Status")}</Form.Label>
                    <Form.Select name="diseaseStatusId" value={editData.diseaseStatusId} onChange={handleEditInput} style={{ ...inputStyle, padding: "9px 12px" }}>
                      <option value="">{t("Select Disease Status")}</option>
                      {diseaseStatusList.map((d) => (
                ...
                <Col lg={12}>
                  <Form.Group>
                    <Form.Label style={labelStyle}>{t("Lot Test Details")}</Form.Label>
                    <Form.Control as="textarea" rows={2} name="lotTestDetails" value={editData.lotTestDetails} onChange={handleEditInput} placeholder={t("Enter Lot Test Details")} style={{ ...inputStyle, resize: "none" }} />
                  </Form.Group>
                </Col>
              </Row>
            </Card.Body>
...
export default FitnessCertificateEdit;

 ...
 useEffect(() => {
  ...
  const postData = (event) => {
          ...
          if (form.checkValidity() === false) {
            ...
            // event.stopPropagation();
            api
              .post(baseURLTargetSetting + `schemeAchievement/add-Mulberry`, data)
              .then((response) => {
                if (response.data.content.error) {
                  saveError(response.data.content.error_description);
                } else {
  ...
  return (
  <Layout title={t("Mulberry Wise Achievement")}>
    <style>{mulberryAchievementStyles}</style>
    <Block.Head>
      <div className="sh-page-header">
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2" className="sh-page-title">{t("Mulberry Wise Achievement")}</Block.Title>
          </Block.HeadContent>
          {/* Removed View Target Button */}
        </Block.HeadBetween>
      </div>
    </Block.Head>
                ...
                <Block>
                  <Card className="sh-section-card">
                    <Card.Header className="sh-section-header">
                      ...
                      <span>{t("Mulberry Wise Achievement")}</span>
                    </Card.Header>
...
const mulberryAchievementStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default MulberryAchievement;

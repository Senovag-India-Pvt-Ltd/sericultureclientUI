...
function DistrictWiseMontlyMulberry() {
  ...
  const getList = () => {
    ...
    api
      .get(
        baseURLTargetSetting + `mulberryTargets/list-Mulberry-join`,
        _params
      )
      .then((response) => {
      ...
      api
        .post(
          baseURLTargetSetting +
            `mulberryTargets/editMulberryTargets`,
          editData  
        )
        .then((response) => {
          if (response.data.content.error) {
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURLTargetSetting + `mulberryTargets/delete-mulberry/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getFinancialList();
            Swal.fire(
            ...
            <Form noValidate validated={validated} onSubmit={postData}>
              <Row className="g-3 ">
                <Block>
                  <Card>
                    <Card.Header>
                      District Wise Montly Mulberry Page
                    </Card.Header>
                    <Card.Body>

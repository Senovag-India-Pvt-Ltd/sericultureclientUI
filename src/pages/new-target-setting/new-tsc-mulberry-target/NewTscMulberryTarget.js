...
useEffect(() => {
 ...
 useEffect(() => {
  ...
  const getChawkiList = (districtId, talukId) => {
    api
  ...
  useEffect(() => {
    if (data.districtId && data.talukId) {
      getChawkiList(data.districtId, data.talukId);
    }
  ...
  const getList = () => {
    setLoading(true);
    api
      .get(baseURLTargetSetting + `mulberryTargets/list-tsc-join`, _params)
      .then((response) => {
  ...
  const postEditData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(
          baseURLTargetSetting + `mulberryTargets/editTscMulberryTargets`,
          editData
        )
        .then((response) => {
  ...
  return (
    <Layout title={t("TSC Wise Area Under Mulberry Monthly Target")}>
      <style>{newTscMulberryTargetStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("TSC Wise Area Under Mulberry Monthly Target")}</Block.Title>
            </Block.HeadContent>
                ...
                <Block>
                  <Card className="sh-section-card">
                    <Card.Header className="sh-section-header">
                      ...
                      <span>{t("TSC Wise Area Under Mulberry Monthly Target")}</span>
                    </Card.Header>
                        ...
                        <thead>
                          <tr>
                            <th style={styles.ctstyle}>
                              ...
                              {formatQty(viewTotalTargetsDataNonNarega[0]?.mulberryValue) ?? "N/A"}
                            </th>
                          </tr>
                        </thead>
                      </table> */}
                        ...
                        <thead>
                          <tr>
                            <th style={styles.ctstyle}>
                              ...
                              {formatQty(viewTotalTargetsDataNarega[0]?.tscValue) ?? t("N/A")}
                            </th>
                          </tr>
                        </thead>
                      </table>
                        ...
                        <thead>
                          <tr>
                            <th style={styles.ctstyle}>
                              ...
                              {formatQty(viewTotalTargetsDataNonNarega[0]?.tscValue) ?? t("N/A")}
                            </th>
                          </tr>
                        </thead>
                      </table>
  ...
  <h5 className="mb-0 fw-bold text-start">{t("Please enter the below field Physical in Hectares")}</h5>
      ...
      <Modal show={showModal3} onHide={handleCloseModal3} size="xl" className="sh-modal">
        <Modal.Header closeButton className="sh-modal-header">
          <Modal.Title>
            <Icon name="edit" />
            <span>{t("TSC Wise Monthly Mulberry")}</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {/* <Form action="#"> */}
...
const newTscMulberryTargetStyles = `
  .sh-page-header {
    padding: 20px 24px;

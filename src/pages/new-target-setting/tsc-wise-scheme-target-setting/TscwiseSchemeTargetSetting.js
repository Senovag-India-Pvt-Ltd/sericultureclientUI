 ...
 const schemeTargets = ["PHYSICAL TARGET","FINANCIAL TARGET"];
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(baseURLTargetSetting + `schemeTargets/saveTscSchemeTargets`, 
  ...
  const postEditData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(
          baseURLTargetSetting + `schemeTargets/editTscSchemeTargets`,
          editData
        )
        .then((response) => {
          if (response.data.content.error) {
  ...
  const totalTarget = (event) => {
    const { financialYearMasterId,scSchemeDetailsId,scSubSchemeDetailsId,scComponentId,scCategoryId,scHeadAccountId,districtId} = data;
    ...
    api
      .post(
        baseURLTargetSetting + `schemeTargets/getOnlyYearlyTargetDetailsForSchemeAndTSC`,
        {},
        {
          params: {
            financialYearMasterId,
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURLTargetSetting + `schemeTargets/delete-tsc/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(

...
function FinancialTargetSettingView() {
  const styles = {
    ctstyle: {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `tsFinancialTargetSetting/get-join/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="View Financial Target Setting">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">View Financial Target Setting</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
      ...
      <Block className="mt-n4">
        <Card>
          <Card.Header>Financial Target Setting View Details</Card.Header>
          <Card.Body>
            {loading ? (
...
export default FinancialTargetSettingView;

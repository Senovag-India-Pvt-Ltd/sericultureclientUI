...
function ReleaseBudgetDistrictList() {
  ...
  const getList = () => {
    setLoading(true);
     ...
     api
      .post(baseURLTargetSetting + `tsReleaseBudgetDistrict/get-details`, data)
      .then((response) => {
  ...
  const deleteConfirm = (id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `tsReleaseBudgetDistrict/delete/${id}`)
          .then((response) => {
            getList();
            Swal.fire(
              "Deleted",
      ...
      cell: (row) => (
        ...
        <div className="text-start w-100">
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.tsReleaseBudgetDistrictId)}
          >
            View
          </Button>
          <Button
            ...
            size="sm"
            className="ms-2"
            onClick={() => handleEdit(row.tsReleaseBudgetDistrictId)}
          >
            Edit
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.tsReleaseBudgetDistrictId)}
            className="ms-2"
          >
            Delete
          </Button>
  ...
  return (
    <Layout title="Released Budget District List">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Released Budget District List</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
...
export default ReleaseBudgetDistrictList;

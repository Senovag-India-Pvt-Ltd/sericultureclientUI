...
function PhysicalTargetSettingsTscEdit() {
  ...
  const postData = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      for (const month in months) {
        ...
        monthlyList.push({
          tsPhysicalInstitutionMonthlyId: result.tsPhysicalInstitutionMonthlyId,
      ...
      try {
        const response = await api.post(
          baseURLTargetSetting + `tsPhysicalInstitution/edit-primary-monthly`,
          {
            editTsPhysicalInstitutionRequest: data,
            editTsPhysicalInstitutionMonthlyRequest: monthlyList,
          }
        );
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURLTargetSetting + `tsPhysicalInstitution/get/${id}`)
      .then((response) => {
    ...
    const response = api
      .get(
        baseURLTargetSetting +
          `tsPhysicalInstitutionMonthly/getByTsPhysicalInstitutionId/${id}`
      )
      .then((response) => {
        ...
        if (response.data.content.error) {
          ...
          const lists = response.data.content.tsPhysicalInstitutionMonthly;
          setReceivedTscDistData(
            response.data.content.tsPhysicalInstitutionMonthly
          );
          setTscMontlyId(
            response.data.content.tsPhysicalInstitutionMonthly
              .tsPhysicalInstitutionMonthlyId
          );

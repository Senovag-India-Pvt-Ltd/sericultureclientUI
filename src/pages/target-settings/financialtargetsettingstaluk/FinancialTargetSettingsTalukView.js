...
function FinancialTargetSettingsTalukView() {
  const styles = {
    ctstyle: {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURLTargetSetting + `tsFinancialTaluk/get-join/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="Taluk Financial Target Settings View">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              Taluk Financial Target Settings View
            </Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
...
export default FinancialTargetSettingsTalukView;

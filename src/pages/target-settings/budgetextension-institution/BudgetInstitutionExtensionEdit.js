...
if (type.budgetType === "release") {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      if (type.budgetType === "allocate") {
      api
        .post(baseURLTargetSetting + `tsBudgetInstitutionExt/edit`, data)
        .then((response) => {
      ...
      if (type.budgetType === "release") {
        api
          .post(baseURLTargetSetting + `tsReleaseBudgetInstitutionExt/edit`, data)
          .then((response) => {
            if (response.data.content.error) {
              updateError(response.data.content.error_description);
            } else {
 ...
 useEffect(() => {
  ...
  return (
    <Layout title="Edit Institution Budget mapping scheme and programs">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              Edit Institution Budget mapping scheme and programs
            </Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
...
export default BudgetInstitutionExtensionEdit;

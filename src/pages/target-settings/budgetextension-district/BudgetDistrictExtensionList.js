...
function BudgetDistrictExtensionList() {
  ...
  const getList = () => {
    ...
    if (type.budgetType === "allocate") {
      api
        .post(baseURLTargetSetting + `tsBudgetDistrictExt/get-details`, data)
        .then((response) => {
          if (response.data.content.error) {
            saveError(response.data.content.error_description);
            setShow(false);
          } else {
            setListData(response.data.content.tsBudgetDistrictExt);
            setShow(true);
            // saveSuccess();
            // clear();
          }
      ...
      cell: (row) => (
        ...
        <div className="text-start w-100">
          ...
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.tsBudgetDistrictExtId)}
            className="ms-2"
          >
            Delete
          </Button>
...
export default BudgetDistrictExtensionList;

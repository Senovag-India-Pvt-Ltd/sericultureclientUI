...
function PhysicalTargetSettingsTalukList() {
  ...
  const getList = () => {
    ...
    const response = api
      .get(baseURL + `tsPhysicalTargetSettingsTaluk/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.tsPhysicalTargetSettingsTaluk);
  ...
  const handleView = (id) => {
    navigate(`/seriui/physicaltargetsettings-taluk-view/${id}`);
  };

  const handleEdit = (id) => {
    navigate(`/seriui/physicaltargetsettings-taluk-edit/${id}`);
  };
      ...
      cell: (row) => (
        ...
        <div className="text-start w-100">
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.tsPhysicalTargetSettingsTalukId)}
          >
            View
          </Button>
          <Button
            ...
            size="sm"
            className="ms-2"
            onClick={() => handleEdit(row.tsPhysicalTargetSettingsTalukId)}
          >
            Edit
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.tsPhysicalTargetSettingsTalukId)}
            className="ms-2"
          >
            Delete
          </Button>

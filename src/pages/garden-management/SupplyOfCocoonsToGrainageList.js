...
function SupplyOfCocoonsToGrainageList(){
 ...
 const getList = () => {
   ...
   const response = api
     .get(baseURL2 + `supply-cocoons/get-info`)
     .then((response) => {
 ...
 const handleView = (_id) => {
   navigate(`/seriui/supply-of-cocoons-to-grainage-view/${_id}`);
 };

 const handleEdit = (_id) => {
   navigate(`/seriui/supply-of-cocoons-to-grainage-edit/${_id}`);
 ...
 const deleteConfirm = (_id) => {
   Swal.fire({
     ...
     if (result.value) {
       console.log("hello");
       const response = api
         .delete(baseURL2 + `supply-cocoons/delete-info/${_id}`)
         .then((response) => {
           // deleteConfirm(_id);
           getList();
           Swal.fire(
...
return(
 <Layout title={t("Supply of Cocoons to Grainage List")}>
      <style>{supplyOfCocoonsToGrainageListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">
                {t("Supply of Cocoons to Grainage List")}
              </Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
...
const supplyOfCocoonsToGrainageListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default SupplyOfCocoonsToGrainageList;
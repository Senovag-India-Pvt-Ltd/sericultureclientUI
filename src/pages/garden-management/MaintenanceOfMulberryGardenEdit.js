...
function MaintenanceOfMulberryGardenEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(baseURL2 + `Mulberry-garden/update-info`, {
          ...data,
  ...
  return (
    <Layout title={t("Edit Maintenance of Mulberry Garden")}>
      <style>{editMaintenanceMulberryGardenStyles}</style>
      <Block.Head>
        ...
        <Form noValidate validated={validated} onSubmit={postData}>
          {/* <Row className="g-3 "> */}
            ...
            <Card.Header className="sh-section-header">
                <Icon name="map-pin" />
                <span>{t("Edit Maintenance Of Mulberry Garden")}</span>
              </Card.Header>
              <Card.Body>
                {loading ? (
...
const editMaintenanceMulberryGardenStyles = `
  .sh-page-header {
    padding: 20px 24px;

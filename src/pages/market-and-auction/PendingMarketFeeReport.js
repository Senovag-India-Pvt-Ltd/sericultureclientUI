...
function PendingMarketFeeReport() {
  ...
  const handleDownloadExcel = () => {
    ...
    api
      .post(baseURLMarket + "auction/report/downloadPendingMarketFeeReportExcel", payload, {
        responseType: "blob",
      })
      .then((response) => {
        ...
        link.href = url;
        link.setAttribute("download", "PendingMarketFeeReport.xlsx");
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const res = await api.post(
        baseURLMarket + "auction/report/downloadPendingMarketFeeReportPdf",
        payload,
        { responseType: "blob" }
      );
  ...
  return (
    <Layout title={t("Pending Market Fee Report")}>
      <style>{pendingMarketFeeReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Pending Market Fee Report")}</Block.Title>
            </Block.HeadContent>
          </Block.HeadBetween>
        </div>
      </Block.Head>
              ...
              {lotList.length > 0 && (
                <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    disabled={loadPdf}
                    style={{ background: "#dc2626", border: "none", borderRadius: "8px", padding: "8px 16px", fontWeight: 700, fontSize: "12px", color: "#fff", cursor: loadPdf ? "not-allowed" : "pointer", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "6px", opacity: loadPdf ? 0.7 : 1 }}
                  >
                    {loadPdf ? <><span className="spinner-border spinner-border-sm" style={{ width: 12, height: 12 }} /> {t("Generating...")}</> : <>⬇ {t("PDF")}</>}
                  </button>
                </div>
              )}
            </div>
...
const pendingMarketFeeReportStyles = `
  .sh-page-header {
    padding: 20px 24px;

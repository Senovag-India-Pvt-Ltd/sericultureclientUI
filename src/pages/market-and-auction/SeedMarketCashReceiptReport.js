...
function SeedMarketCashReceiptReport() {
  ...
  return (
    <Layout title={t("Cash Receipt Report")}>
      <style>{seedMarketCashReceiptReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Cash Receipt Report")}</Block.Title>
            </Block.HeadContent>
          </Block.HeadBetween>
        </div>
      </Block.Head>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card className="mt-1" style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 24px rgba(30,103,168,0.10)" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #1e67a8 0%, #2d9cdb 100%)", padding: "18px 28px", display: "flex", alignItems: "center", gap: "12px", borderRadius: "14px 14px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 700, fontSize: "17px", lineHeight: 1.2 }}>Cash Receipt Report</div>
              <div style={{ color: "rgba(255,255,255,0.8)", fontSize: "12px", marginTop: "2px" }}>Generate cash receipt by auction date and bidding lot</div>
            </div>
            ...
            <Form onSubmit={handleGenerate} noValidate>
              ...
              <div className="d-flex gap-3">
                <button type="submit" disabled={isGenerating} style={{ background: isGenerating ? "#c8d6e5" : "linear-gradient(135deg,#1e67a8,#2d9cdb)", border: "none", borderRadius: "10px", padding: "12px 32px", fontWeight: 700, fontSize: "14px", color: "#fff", cursor: isGenerating ? "not-allowed" : "pointer", boxShadow: isGenerating ? "none" : "0 4px 14px rgba(30,103,168,0.35)", display: "flex", alignItems: "center", gap: "8px" }}>
                  {isGenerating ? <><span className="spinner-border spinner-border-sm" />Generating…</> : <>📄 Generate Cash Receipt Report</>}
                </button>
              </div>
            </Form>
          </Card.Body>

...
function SeedMarketPermitReport() {
  ...
  const externalOptions = externalListData?.map((list) => ({
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!data.externalUnitRegistrationId) {
      Swal.fire({
        icon: "warning", title: t("Required Field", { ns: "reports" }),
        html: `<div style="padding:8px 2px 12px"><div style="background:linear-gradient(135deg,#fffbeb,#fef9ec);border:1.5px solid #fcd34d;border-radius:14px;padding:16px 20px;display:flex;align-items:flex-start;gap:13px;text-align:left"><div style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#f59e0b,#fbbf24);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">⚠️</div><div><p style="color:#92400e;font-size:14px;font-weight:700;margin:0 0 5px">${t("External User Required", { ns: "reports" })}</p><p style="color:#78350f;font-size:13px;margin:0;line-height:1.65">${t("Please select an External User before generating the permit report.", { ns: "reports" })}</p></div></div></div>`,
        confirmButtonText: t("Got it", { ns: "reports" }), confirmButtonColor: "#d97706", background: "#fff",
    ...
    try {
      const response = await api.post(
        ...
        { marketId: data.marketId, externalUnitRegistrationId: data.externalUnitRegistrationId, fromDate: formatDate(data.fromDate), toDate: formatDate(data.toDate) },
        { responseType: "blob" }
      );
  ...
  const reactSelectStyles = {
    control: (b) => ({ ...b, borderRadius: "8px", border: "1.5px solid #d0d9e8", background: "#f8fafd", fontSize: "14px", minHeight: "40px" }),
    menu: (b) => ({ ...b, borderRadius: "10px", boxShadow: "0 8px 24px rgba(0,0,0,0.12)" }),
    option: (b, s) => ({ ...b, fontSize: "13px", background: s.isSelected ? "#1e67a8" : s.isFocused ? "#ebf4ff" : "#fff", color: s.isSelected ? "#fff" : "#333" }),
  };

  return (
    <Layout title={t("Permit Report", { ns: "reports" })}>
      <style>{seedMarketPermitReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Permit Report", { ns: "reports" })}</Block.Title>
            </Block.HeadContent>
          </Block.HeadBetween>
        </div>
      </Block.Head>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card className="mt-1" style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 24px rgba(30,103,168,0.10)" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #1e67a8 0%, #2d9cdb 100%)", padding: "18px 28px", display: "flex", alignItems: "center", gap: "12px", borderRadius: "14px 14px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 700, fontSize: "17px", lineHeight: 1.2 }}>{t("Permit Report", { ns: "reports" })}</div>
              <div style={{ color: "rgba(255,255,255,0.8)", fontSize: "12px", marginTop: "2px" }}>{t("Generate permit report by external user and date range", { ns: "reports" })}</div>
            </div>
            ...
            <Form onSubmit={handleGenerate} noValidate>
              <Row className="mb-3">
                <Col md={6} style={fieldGroupStyle}>
                  <label style={labelStyle}>{t("External User", { ns: "reports" })} <span style={{ color: "#e53e3e" }}>*</span></label>
                  <ReactSelect
                    options={externalOptions}
                    placeholder={`— ${t("Select External User", { ns: "reports" })} —`}
                    isSearchable
                    menuPlacement="auto"
                    styles={reactSelectStyles}
                    value={externalOptions?.find((o) => o.value === data.externalUnitRegistrationId) || null}
              ...
              <Row className="mb-4">
                <Col md={4} style={fieldGroupStyle}>
                  <label style={labelStyle}>{t("From Date", { ns: "reports" })} <span style={{ color: "#e53e3e" }}>*</span></label>
                ...
                <Col md={4} style={fieldGroupStyle}>
                  <label style={labelStyle}>{t("To Date", { ns: "reports" })} <span style={{ color: "#e53e3e" }}>*</span></label>
              ...
              <div className="d-flex gap-3">
                <button type="submit" disabled={isGenerating} style={{ background: isGenerating ? "#c8d6e5" : "linear-gradient(135deg,#1e67a8,#2d9cdb)", border: "none", borderRadius: "10px", padding: "12px 32px", fontWeight: 700, fontSize: "14px", color: "#fff", cursor: isGenerating ? "not-allowed" : "pointer", boxShadow: isGenerating ? "none" : "0 4px 14px rgba(30,103,168,0.35)", display: "flex", alignItems: "center", gap: "8px" }}>
                  {isGenerating ? <><span className="spinner-border spinner-border-sm" />{t("Generating…", { ns: "reports" })}</> : <>📄 {t("Generate Permit Report", { ns: "reports" })}</>}
                </button>
              </div>
            </Form>
          </Card.Body>
...
const seedMarketPermitReportStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default SeedMarketPermitReport;

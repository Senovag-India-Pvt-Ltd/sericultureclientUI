...
if (!document.getElementById("gf1-styles")) {
  const s = document.createElement("style");
  s.id = "gf1-styles";
  s.innerHTML = `
    .gf1-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gf1-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gf1-swal .swal2-icon { margin:20px auto 4px !important; }
    .gf1-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gf1-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gf1-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gf1-wrap { animation: gf1-in .35s ease; }
    .gf1-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gf1-table th { letter-spacing:.02em; }
    .gf1-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gf1-scroll::-webkit-scrollbar { height:9px; }
    .gf1-scroll::-webkit-scrollbar-track { background:#f0fdfa; border-radius:6px; }
    .gf1-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#0f766e,#3730a3); border-radius:6px; }
  `;
...
// 23 sections — each gets an icon + hue. Sections that are always headers (no values
// of their own) carry isHeader=true so we render the description as a section band.
const SECTION_META = {
  "1":  { icon: "🎯", hue: "amber",   en: "Annual Target" },
  "2":  { icon: "📅", hue: "amber",   en: "Monthly Target" },
  "3":  { icon: "📦", hue: "teal",    en: "Stored Sub-lots / Lots" },
  "4":  { icon: "🪺", hue: "orange",  en: "Stored Multi-cross Cocoons" },
  "5":  { icon: "🪺", hue: "violet",  en: "Stored Bivoltine — Mixed" },
  "6":  { icon: "🪺", hue: "violet",  en: "Stored Bivoltine — Shinichi" },
  "7":  { icon: "🪺", hue: "violet",  en: "Stored Bivoltine — CSR-4" },
  "8":  { icon: "🥚", hue: "indigo",  en: "Bivoltine DFL Prep (P2/P1/Hybrid)" },
  ...
  "16": { icon: "🎚️", hue: "blue",    en: "Average Quality Performance" },
  "17": { icon: "📈", hue: "emerald", en: "Profit / Loss (₹)" },
  "18": { icon: "👷", hue: "lime",    en: "Labour Productivity" },
  "19": { icon: "📝", hue: "slate",   en: "Reasons (Recovery < 75%)" },
  "20": { icon: "🥚", hue: "blue",    en: "DFLs Received" },
  "21": { icon: "⚖️", hue: "orange",  en: "Bivoltine Grainage Cocoons (kg)" },
  "22": { icon: "🌐", hue: "indigo",  en: "Bivoltine DFL Prep — KA vs Foreign" },
  "23": { icon: "🦠", hue: "rose",    en: "Destroyed DFLs (Pebrine + Other)" },
};
...
// Sub-label classifier — used to choose pill colour and indent depth
const isTotalSub = (sub) => {
  const s = String(sub || "").trim();
  return s === "ಒಟ್ಟು" || s.startsWith("ಒಟ್ಟು") || /^[0-9]+ಒ$/.test(s) || s.endsWith("ಒ");
};
...
function GrainageForm1DetailedReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gf1-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/grainage-form1-detailed", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Grainage Form-1 Detailed report.", { ns: "reports" }));
        }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/grainage-form1-detailed/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/grainage-form1-detailed/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a"); a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `grainage_form1_detailed_${filter.grainageId}_${year}_${m}.xlsx`;
  ...
  const kpis = useMemo(() => {
    ...
    const allStored = find(9);
    const bvDfls = find(11, "ಒಟ್ಟು");
    const recvTot = find(20, "ಒಟ್ಟು");
    const pebrine = find(23, "ಇ");
    return {
      annualCY: numOrZero(annual.cy_month),
      annualPY: numOrZero(annual.py_month),
      monthlyCY: numOrZero(monthly.cy_month),
      monthlyPY: numOrZero(monthly.py_month),
      allStoredCY: numOrZero(allStored.cy_month),
      allStoredCYCum: numOrZero(allStored.cy_cum),
      bvDflsCY: numOrZero(bvDfls.cy_month),
      bvDflsCYCum: numOrZero(bvDfls.cy_cum),
      recvCY: numOrZero(recvTot.cy_month),
  ...
  const achPct = kpis.monthlyCY === 0 ? 0 : (kpis.bvDflsCY / kpis.monthlyCY) * 100;

  return (
    <Layout title={t("Form-1 · Detailed Monthly Progress (race-split)", { ns: "reports" })}>
      <Block.Head><Block.HeadBetween><Block.HeadContent>
        <Block.Title tag="h2">
          {t("ಪಾತ ಸಂಖ್ಯೆ-1 · ಬಿತ್ತನೆ ಕೋಠಿ ಮಾಹೆಯ ಪ್ರಗತಿಯ ವರದಿ (ಮಿಶ್ರತಳಿ + ಪಿ2 + ಪಿ1 + ಹೈ)")}
          <span style={{
          ...
          }}>P1 & P2 · Bivoltine · Form-1 · Detailed CY vs PY</span>
        </Block.Title>
      ...
      <Block className="mt-n4">
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2 }}>
                ಪಾತ ಸಂಖ್ಯೆ-1 · ಬಿತ್ತನೆ ಕೋಠಿ — ಮಾಹೆಯ ಪ್ರಗತಿಯ ವರದಿ (ವಿವರವಾದ)
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P1 &amp; P2 Grainage (Bivoltine) — Detailed Monthly Progress: 23 metric sections, race-split (CY vs PY)</div>
            </div>
            {hasReport && (
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                <span style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>{kpis.populated}/{kpis.total} populated</span>
              </div>
            )}
          </div>
        ...
        {hasReport && <ReportBody
          grouped={grouped} kpis={kpis} achPct={achPct}
          grainageName={grainageName} monthLabel={monthLabel} monthKn={monthKn} monthYear={monthYear}
          handlePdf={handlePdf} handleExcel={handleExcel}
          isDownloadingPdf={isDownloadingPdf} isDownloadingExcel={isDownloadingExcel}
        />}
      </Block>
    </Layout>
  );
...
function ReportBody({ grouped, kpis, achPct, grainageName, monthLabel, monthKn, monthYear,
                     handlePdf, handleExcel, isDownloadingPdf, isDownloadingExcel }) {
  const { t } = useTranslation();
  return (
    <div className="gf1-wrap mt-4">
      <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
        ...
        <div style={kpi("#fed7aa", "#fb923c", "#7c2d12")}>
          <span style={kpiLbl("#7c2d12")}>🎯 ವಾರ್ಷಿಕ ಗುರಿ Annual</span>
          <span className="gf1-num" style={kpiVal("#7c2d12", 16)}>{kpis.annualCY.toLocaleString()}</span>
          <span style={{ fontSize: "10.5px", color: "#9a3412", fontWeight: 700, marginTop: "1px" }}>PY: {kpis.annualPY.toLocaleString()}</span>
        </div>
        <div style={kpi("#fde68a", "#fcd34d", "#78350f")}>
          <span style={kpiLbl("#78350f")}>📅 ಮಾಸ ಗುರಿ Monthly</span>
          <span className="gf1-num" style={kpiVal("#78350f", 16)}>{kpis.monthlyCY.toLocaleString()}</span>
        </div>
        <div style={kpi("#a7f3d0", "#6ee7b7", "#065f46")}>
          <span style={kpiLbl("#065f46")}>Σ ಒಟ್ಟು ಶೇಖರಿಸಿದ Stored</span>
          <span className="gf1-num" style={kpiVal("#064e3b", 16)}>{kpis.allStoredCY.toLocaleString()}</span>
          <span style={{ fontSize: "10.5px", color: "#065f46", fontWeight: 700, marginTop: "1px" }}>FY Cum: {kpis.allStoredCYCum.toLocaleString()}</span>
        </div>
        <div style={kpi("#ddd6fe", "#a78bfa", "#5b21b6")}>
          <span style={kpiLbl("#5b21b6")}>🥚 ದ್ವಿತಳಿ ಮೊಟ್ಟೆ Bivoltine DFLs</span>
          <span className="gf1-num" style={kpiVal("#4c1d95", 16)}>{kpis.bvDflsCY.toLocaleString()}</span>
          <span style={{ fontSize: "10.5px", color: "#5b21b6", fontWeight: 700, marginTop: "1px" }}>FY Cum: {kpis.bvDflsCYCum.toLocaleString()}</span>
        </div>
        <div style={{
          ...kpi(
          ...
          <span style={kpiLbl(achPct >= 100 ? "#14532d" : achPct >= 75 ? "#065f46" : achPct >= 50 ? "#92400e" : "#7f1d1d")}>
            {achPct >= 100 ? "🎉" : "📊"} BV / Monthly Goal
          </span>
          <span className="gf1-num" style={kpiVal(achPct >= 100 ? "#14532d" : achPct >= 75 ? "#064e3b" : achPct >= 50 ? "#78350f" : "#7f1d1d", 16)}>{achPct.toFixed(2)}%</span>
        </div>
        <div style={kpi("#fecdd3", "#fda4af", "#9f1239")}>
          <span style={kpiLbl("#9f1239")}>🦠 Pebrine Destroyed</span>
          <span className="gf1-num" style={kpiVal("#881337", 16)}>{kpis.pebrineCY.toLocaleString()}</span>
        </div>
      </div>
      ...
      <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(15,118,110,.12)", overflow: "hidden" }}>
        <div style={{
          ...
          ಪಾತ ಸಂಖ್ಯೆ-1 · ಬಿತ್ತನೆ ಕೋಠಿ {grainageName} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
          <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
            Form-1 · Detailed Monthly Progress &nbsp;·&nbsp; CY {monthYear || ""}-{monthYear ? (monthYear + 1) % 100 : ""} vs PY {monthYear ? monthYear - 1 : ""}-{monthYear ? monthYear % 100 : ""} &nbsp;·&nbsp; {monthLabel}
          </div>
        </div>

        <div className="gf1-scroll" style={{ overflowX: "auto" }}>
          <table className="gf1-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "1200px" }}>
            <thead>
              <tr>
            ...
            <tbody>
              {grouped.length === 0 && (
                <tr><td colSpan={7} style={{ padding: "60px 20px", textAlign: "center", background: "linear-gradient(180deg,#f0fdfa,#fff)" }}>
                  ...
                  <div style={{ fontSize: "13px", color: "#475569", fontWeight: 600 }}>No Form-1 data found for this grainage in {monthLabel} {monthYear}.</div>
                </td></tr>
              )}
              {grouped.map((g) => {
                ...
                return g.rows.map((row, ri) => {
                  ...
                  return (
                    <tr key={`${g.sn}-${ri}`} className="gf1-tr"
                        style={{
                          background: totalSub
        ...
        <div style={{ background: "linear-gradient(135deg,#f0fdfa,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #a7f3d0" }}>
          <span style={{ fontSize: "12px", color: "#0f766e", fontWeight: 600 }}>
            Form-1 · {grainageName} — {monthLabel} {monthKn} {monthYear} &nbsp;·&nbsp; {kpis.populated}/{kpis.total} rows populated &nbsp;·&nbsp; BV achievement {achPct.toFixed(1)}%
          </span>
          <div className="d-flex gap-2 flex-wrap">
...
export default GrainageForm1DetailedReport;

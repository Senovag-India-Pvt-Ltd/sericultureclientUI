...
function GrainageFarmFutureChawkiPlanReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Future Chawki Plan report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("Farm Future Chawki Plan Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              ...
              }}>{t("Farm · Future Chawki Plan", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 14px rgba(13,78,72,.08)", overflow: "hidden" }}>
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಮುಂಬರುವ ಚಾಕಿ ಯೋಜನೆ / {t("Future Chawki Plan", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">

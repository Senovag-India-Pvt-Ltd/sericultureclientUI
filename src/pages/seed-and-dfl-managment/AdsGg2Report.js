...
function AdsGg2Report() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the ADS GG2 report.");
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("ADS · GG-2 — Weekly DFL Production Progress")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
          ...
          <Card.Body style={{ padding: "16px 20px 18px", background: "linear-gradient(180deg,#ffffff,#fff7ed)" }}>
            <Form onSubmit={handleView}>
              <Row className="g-2 align-items-end">
                <Col md={4}>
                  <label style={lbl}>
                    Grainages <span style={{ color: "#94a3b8", fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>(optional · max {GRAINAGE_CAP} P1)</span>
                  </label>
                  <ReactSelect
                    isMulti
                    ...
                    placeholder="— Select up to 6 P1 grainages —"
                    isSearchable
                    isClearable
                  ...
                  {overCap && (
                    <div style={{
                      ...
                      ⓘ Backend caps at {GRAINAGE_CAP} grainages — only the first {GRAINAGE_CAP} will be used.
                    </div>
                  )}
                </Col>
                <Col md={2}>
          ...
          <div className="adsgg2-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#a7f3d0,#ecfdf5)", border: "1.5px solid #6ee7b7", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#065f46", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>% Achieved</span>

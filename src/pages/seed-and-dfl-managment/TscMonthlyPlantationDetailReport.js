...
function TscMonthlyPlantationDetailReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC Monthly Plantation Detail (Sheet 4) report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("TSC Monthly Plantation Detail (Sheet 4)", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
            ...
            <Form onSubmit={handleView}>
              ...
              {hasReport && (
                <Row className="g-2 mt-2 align-items-end">
                  <Col md={6}>
                    <label style={lbl}>{t("Quick Search (TSC, Farmer, Village, Variety…)", { ns: "reports" })}</label>
                    <input

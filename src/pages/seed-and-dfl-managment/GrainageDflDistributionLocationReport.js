...
function GrainageDflDistributionLocationReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          showErr(t("No Data Found", { ns: "reports" }), t("No records found for the selected criteria.", { ns: "reports" }));
        } else {
          const data = err?.response?.data;
          const backendMsg = typeof data === "string"
            ? data
            : (data?.message || data?.error || data?.errorMessage || data?.error_description);
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the DFL Distribution by Location report.", { ns: "reports" }));
        }
  ...
  const grainageName = (i18n.language === "kn" ? (selectedGrainage?.grainageMasterNameInKannada || selectedGrainage?.grainageMasterName) : selectedGrainage?.grainageMasterName) || "—";
  const districtName = (i18n.language === "kn" ? (selectedDistrict?.districtNameInKannada || selectedDistrict?.districtName) : selectedDistrict?.districtName) || t("All Districts", { ns: "reports" });
              ...
              <Row className="g-2 align-items-end">
                ...
                <Col md={3}>
                  ...
                  <ReactSelect
                    ...
                    noOptionsMessage={() => t("No district found", { ns: "reports" })}
                  />
                </Col>
                <Col md={2}>
                ...
                <Col md={2}>
                  <div className="d-flex gap-2 flex-wrap">
                    <button type="submit" disabled={isLoading} style={btn("linear-gradient(135deg,#0f766e,#14b8a6)", "0 4px 12px rgba(15,118,110,.32)", isLoading)}>
                      {isLoading ? <><span className="spinner-border spinner-border-sm" /> …</> : <>📋 {t("View", { ns: "reports" })}</>}
                    </button>
                  ...
                  <tbody>
                    {dataRows.length === 0 && (
                      <tr><td colSpan={5} style={{ padding: "60px 20px", textAlign: "center", background: "linear-gradient(180deg,#f0fdfa,#fff)" }}>
                        ...
                        <div style={{ fontSize: "13px", color: "#475569", fontWeight: 600 }}>{t("No DFL distribution data found for this grainage in {{month}} {{year}}.", { ns: "reports", month: monthLabel, year: monthYear })}</div>
                      </td></tr>
                    )}

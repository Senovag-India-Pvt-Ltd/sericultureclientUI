...
if (!document.getElementById("tscsgrc-styles")) {
  const s = document.createElement("style");
  s.id = "tscsgrc-styles";
  s.innerHTML = `
    .tscsgrc-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .tscsgrc-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .tscsgrc-swal .swal2-icon { margin:20px auto 4px !important; }
    .tscsgrc-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .tscsgrc-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes tscsgrc-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .tscsgrc-wrap { animation: tscsgrc-in .35s ease; }
    .tscsgrc-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .tscsgrc-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .tscsgrc-scroll::-webkit-scrollbar { height:9px; }
    .tscsgrc-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .tscsgrc-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#0f766e,#5b57ac); border-radius:6px; }
    .tscsgrc-pbar-track { width:100%; height:5px; background:rgba(0,0,0,.06); border-radius:99px; overflow:hidden; margin-top:4px; }
    .tscsgrc-pbar-fill  { height:100%; border-radius:99px; transition:width .4s ease; }
  `;
...
// Stable hue assignment for source-grainages — palette of pleasant gradients
const GRAINAGE_PALETTE = [
  { bg: "linear-gradient(135deg,#cffafe,#a5f3fc)", color: "#155e75" },
  { bg: "linear-gradient(135deg,#bbf7d0,#86efac)", color: "#14532d" },
  { bg: "linear-gradient(135deg,#fed7aa,#fdba74)", color: "#7c2d12" },
  { bg: "linear-gradient(135deg,#ede9fe,#ddd6fe)", color: "#5b21b6" },
  { bg: "linear-gradient(135deg,#fbcfe8,#f9a8d4)", color: "#9d174d" },
  { bg: "linear-gradient(135deg,#fef3c7,#fde68a)", color: "#854d0e" },
  { bg: "linear-gradient(135deg,#dbeafe,#bfdbfe)", color: "#1e40af" },
  { bg: "linear-gradient(135deg,#ccfbf1,#99f6e4)", color: "#115e59" },
];
const grainageTone = (name) => {
  const s = String(name || "");
  let h = 0;
  for (let i = 0; i < s.length; i++) h = ((h << 5) - h + s.charCodeAt(i)) | 0;
  const idx = Math.abs(h) % GRAINAGE_PALETTE.length;
  return GRAINAGE_PALETTE[idx];
};

function TscMonthlySourceGrainageChawkiReport() {
  ...
  const handleView = async (e) => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/source-grainage-chawki", { params: params() });
      setDataRows(Array.isArray(res.data) ? res.data : []); setHasReport(true);
    } catch { showErr("Fetch Failed", "Failed to load the TSC Monthly Source-Grainage Chawki (Sheet 16) report."); }
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/source-grainage-chawki/excel", { params: params(), responseType: "blob" });
  ...
  const filteredRows = useMemo(() => {
    ...
    return dataRows.filter((r) => [r.race, r.source_grainage].some((v) => String(v ?? "").toLowerCase().includes(q)));
  }, [dataRows, search]);

  // Race-merge groups (consecutive same race) — race chip rendered once per group
  ...
  const kpis = useMemo(() => {
    ...
    const races = new Set(), grainages = new Set();
    const byRaceMe = {};
    const byGrMe   = {};
    dataRows.forEach((r) => {
      const mo = numOrZero(r.mo_dfls), me = numOrZero(r.me_dfls);
      totalMo += mo; totalMe += me;
      if (r.race) races.add(r.race);
      if (r.source_grainage) grainages.add(r.source_grainage);
      const rc = r.race || "—", gr = r.source_grainage || "—";
      byRaceMe[rc] = (byRaceMe[rc] || 0) + me;
      byGrMe[gr]   = (byGrMe[gr]   || 0) + me;
    });
    const topRace = Object.entries(byRaceMe).sort((a, b) => b[1] - a[1])[0] || ["—", 0];
    const topGr   = Object.entries(byGrMe).sort((a, b) => b[1] - a[1])[0] || ["—", 0];
    return { totalMo, totalMe, races: races.size, grainages: grainages.size, topRace, topGr, byGrMe };
  }, [dataRows]);

  // Top 5 source-grainages by ME for a horizontal bar mini-chart
  const top5Grainages = useMemo(() => {
    const arr = Object.entries(kpis.byGrMe).sort((a, b) => b[1] - a[1]).slice(0, 5);
    const max = arr.reduce((m, [, v]) => Math.max(m, v), 1);
    return arr.map(([name, v]) => ({ name, v, pct: Math.min(100, (v * 100) / max) }));
  }, [kpis.byGrMe]);
    ...
    <Layout title={t("TSC Monthly Source-Grainage Chawki — Sheet 16")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಬಿತ್ತನೆ ಕೋಠಿವಾರು ಚಾಕಿ ವಿವರ — ಮಾಸಿಕ")}
      ...
      <Block className="mt-n4">
        <Card className="mt-1" style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 20px rgba(13,78,72,.10)", overflow: "hidden" }}>
          <div style={{ background: "linear-gradient(135deg,#0e7490 0%,#06b6d4 50%,#7c3aed 100%)", padding: "12px 20px", display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2 }}>
                ಬಿತ್ತನೆ ಕೋಠಿವಾರು ಚಾಕಿ — Source-Grainage-wise DFL Brushing
                <span style={{ background: "rgba(255,255,255,.22)", padding: "2px 9px", borderRadius: "12px", marginLeft: "8px", fontSize: "10.5px", fontWeight: 800 }}>SHEET 16</span>
              </div>
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>
                Race × Source-Grainage — DFLs brushed · Month / ME — for the selected Taluk
              </div>
            </div>
            {hasReport && (
        ...
        {hasReport && (
          <div className="tscsgrc-wrap mt-4">
                ...
                <div style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: "14px", padding: "10px 16px", flex: 1, minWidth: "320px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
                    {top5Grainages.map((g, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 90px", gap: "10px", alignItems: "center" }}>
                          <div>
                            <div style={{ fontSize: "11px", fontWeight: 700, color: "#1e293b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={g.name}>{g.name}</div>
                            <div className="tscsgrc-pbar-track">
                              <div className="tscsgrc-pbar-fill" style={{ width: `${g.pct}%`, background: tone.bg, opacity: .9 }} />
                            </div>
                          </div>
                          <div className="tscsgrc-num" style={{ textAlign: "right", fontSize: "11.5px", fontWeight: 800, color: tone.color }}>{fmtInt(g.v)}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              <div style={{ background: "linear-gradient(135deg,#134e4a,#0f766e 50%,#5b57ac)", color: "#fff", padding: "16px 22px", fontWeight: 800, fontSize: "15px", textAlign: "center" }}>
                Sheet 16 · ಬಿತ್ತನೆ ಕೋಠಿವಾರು ಚಾಕಿ ವಿವರ — {talukName} — {monthKn} {filter.year}
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  Source-Grainage-wise Chawki Detail · Race × Source-Grainage · {monthLabel} {filter.year}
                </div>
              </div>

              <div className="tscsgrc-scroll" style={{ overflowX: "auto", maxHeight: "70vh", overflowY: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "1100px" }}>
                  <thead>
                    <tr>
                      {[
                        ...
                        { kn: "ಬಿತ್ತನೆಕೋಠಿ ಮೂಲ", en: "Source Grainage",     w: 320, bg: "linear-gradient(135deg,#0e7490,#06b6d4)", align: "left" },
                        { kn: "ಚಾಕಿ ಮಾಹೆ",       en: "Mo (DFLs)",            w: 160, bg: "linear-gradient(135deg,#0f766e,#14b8a6)", align: "right" },
                        { kn: "ಚಾಕಿ ಅಂತ್ಯ",      en: "ME (DFLs)",            w: 160, bg: "linear-gradient(135deg,#15803d,#22c55e)", align: "right" },
                      ].map((c, i) => (
                        <th key={i} style={{
                  ...
                  <tbody>
                    ...
                    {filteredRows.map((row, ri) => {
                      ...
                      const gTone = grainageTone(row.source_grainage);
                      ...
                      return (
                        <tr key={`${row.sl_no}-${row.race}-${row.source_grainage}-${ri}`} className="tscsgrc-tr" style={{ background: rowBg }}>
                          ...
                          <td style={{ ...cb, textAlign: "left", paddingLeft: "14px", borderRight: "2px solid #e2e8f0" }}>
                            {row.source_grainage ? (
                              <span style={{ display: "inline-block", padding: "4px 12px", borderRadius: "10px", background: gTone.bg, color: gTone.color, fontWeight: 700, fontSize: "12px" }}>
                                🏭 {row.source_grainage}
                              </span>
                            ) : <span style={{ color: "#cbd5e0" }}>—</span>}
                          </td>
                          <td className="tscsgrc-num" style={{
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  {districtName} · {talukName} — {monthLabel} {monthKn} {filter.year} &nbsp;·&nbsp; Sheet 16 · Race × Source-Grainage Chawki (DFLs)
                </span>
...
export default TscMonthlySourceGrainageChawkiReport;

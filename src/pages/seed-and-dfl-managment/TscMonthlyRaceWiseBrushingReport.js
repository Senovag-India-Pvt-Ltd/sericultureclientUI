...
if (!document.getElementById("tscrw-styles")) {
  const s = document.createElement("style");
  s.id = "tscrw-styles";
  s.innerHTML = `
    .tscrw-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .tscrw-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .tscrw-swal .swal2-icon { margin:20px auto 4px !important; }
    .tscrw-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .tscrw-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes tscrw-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .tscrw-wrap { animation: tscrw-in .35s ease; }
    .tscrw-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .tscrw-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .tscrw-scroll::-webkit-scrollbar { height:9px; }
    .tscrw-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .tscrw-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#0f766e,#5b57ac); border-radius:6px; }
  `;
...
// Race chip colour map — categorize race name into a tone
...
function TscMonthlyRaceWiseBrushingReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "tscrw-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/race-wise-brushing", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC Monthly Race-wise Brushing (Sheet 5c) report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/race-wise-brushing/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/race-wise-brushing/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      a.download = `tsc_monthly_race_wise_brushing_${filter.talukId}_${filter.year}_${filter.month}.xlsx`;
      a.click();
  ...
  // Filter rows by search (TSC, Race) — preserve order
  ...
  // Compute Sl-merge groups (consecutive identical sl_no -> single visible cell)
  ...
  // First-row index per merge group → quick lookup for "show TSC + sl"
  ...
  const kpis = useMemo(() => {
    ...
    dataRows.filter((r) => String(r.tsc_name || "").trim() !== "ಒಟ್ಟು" && String(r.race || "").trim() !== "ಒಟ್ಟು").forEach((r) => {
      ...
      const me = numOrZero(r.me_brushed);
      const raceKey = (r.race || "—").toString();
      byRaceMe[raceKey] = (byRaceMe[raceKey] || 0) + me;
      const tone = raceTone(r.race);
      if (tone.label === "Hybrid") byCategoryMe.Hybrid += me;
      else                          byCategoryMe.Pure   += me;
    });
    const top = Object.entries(byRaceMe).sort((a, b) => b[1] - a[1])[0] || ["—", 0];
    const purePct = totalMe > 0 ? (byCategoryMe.Pure   * 100) / totalMe : 0;
    const hybPct  = totalMe > 0 ? (byCategoryMe.Hybrid * 100) / totalMe : 0;
    return {
      totalMo, totalMe,
      tscCount: tscs.size,
      ...
      topRace: top[0],
      topRaceMe: top[1],
      pureMe: byCategoryMe.Pure,
      hybMe:  byCategoryMe.Hybrid,
      purePct, hybPct,
    };
  }, [dataRows]);

  return (
    <Layout title={t("TSC Monthly Race-wise Brushing — Sheet 5c", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ತಳಿವಾರು ಮೊಟ್ಟೆ ಚಾಕಿ ವಿವರ — ಮಾಸಿಕ")}
              <span style={{
              ...
              }}>ನಮೂನೆ-28 · Form 28 · Race-wise</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
      ...
      <Block className="mt-n4">
        ...
        <Card className="mt-1" style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 20px rgba(13,78,72,.10)", overflow: "hidden" }}>
          <div style={{
            background: "linear-gradient(135deg,#a16207 0%,#ca8a04 50%,#7c3aed 100%)",
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ತಳಿವಾರು ಮೊಟ್ಟೆ ಚಾಕಿ ವಿವರ — TSC × Race DFL Brushing Breakdown
                <span style={{ background: "rgba(255,255,255,.22)", padding: "2px 9px", borderRadius: "12px", marginLeft: "8px", fontSize: "10.5px", fontWeight: 800 }}>SHEET 5c</span>
              </div>
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>
                One row per TSC × Race · Month / ME (DFLs) · Sl merged across rows of the same TSC
              </div>
            </div>
            {hasReport && (
          ...
          <div className="tscrw-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#dbeafe,#eff6ff)", border: "1.5px solid #93c5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "190px" }}>
                <span style={{ fontSize: "11px", color: "#1d4ed8", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("ME · Total Brushed", { ns: "reports" })}</span>
                <span className="tscrw-num" style={{ fontSize: "16px", color: "#1e3a8a", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.totalMe)} <span style={{ fontSize: "11px", color: "#1e40af" }}>DFLs</span></span>
                <span className="tscrw-num" style={{ fontSize: "11px", color: "#1e40af", marginTop: "2px" }}>Mo: {fmtInt(kpis.totalMo)}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#bbf7d0,#dcfce7)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#166534", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Pure · ME", { ns: "reports" })}</span>
                <span className="tscrw-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.pureMe)} DFLs</span>
                <span className="tscrw-num" style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, marginTop: "2px" }}>{kpis.purePct.toFixed(2)}% of total</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Hybrid · ME", { ns: "reports" })}</span>
                <span className="tscrw-num" style={{ fontSize: "14px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.hybMe)} DFLs</span>
                <span className="tscrw-num" style={{ fontSize: "11px", color: "#6d28d9", fontWeight: 700, marginTop: "2px" }}>{kpis.hybPct.toFixed(2)}% of total</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#cffafe,#ecfeff)", border: "1.5px solid #67e8f9", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                ...
                <span className="tscrw-num" style={{ fontSize: "14px", color: "#155e75", fontWeight: 800, marginTop: "2px" }}>{kpis.tscCount} / {kpis.raceCount}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "210px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Top Race · ME", { ns: "reports" })}</span>
                <span style={{ fontSize: "13px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{kpis.topRace}</span>
                <span className="tscrw-num" style={{ fontSize: "11px", color: "#a16207", fontWeight: 700, marginTop: "2px" }}>{fmtInt(kpis.topRaceMe)} DFLs</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              <div style={{
                ...
                ನಮೂನೆ-28 · ತಳಿವಾರು ಮೊಟ್ಟೆ ಚಾಕಿ ವಿವರ — {talukName} — {monthKn} {filter.year}
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  Form 28 · TSC × Race DFL Brushing Breakdown · {monthLabel} {filter.year}
                </div>
              </div>

              <div className="tscrw-scroll" style={{ overflowX: "auto", maxHeight: "70vh", overflowY: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "900px" }}>
                  <thead>
                    <tr>
                      {[
                        { kn: "ಕ್ರ.ಸಂ.",       en: "Sl",         w: 60,  bg: "linear-gradient(135deg,#1e293b,#36506b)", align: "center" },
                        { kn: "ತಾಂತ್ರಿಕ ಸೇವಾ ಕೇಂದ್ರ", en: "TSC", w: 240, bg: "linear-gradient(135deg,#334155,#475569)", align: "left" },
                        { kn: "ತಳಿ",          en: "Race",        w: 240, bg: "linear-gradient(135deg,#a16207,#ca8a04)", align: "left" },
                        { kn: "ಚಾಕಿ ಮಾಹೆ",   en: "Month Brushed (DFLs)", w: 160, bg: "linear-gradient(135deg,#0f766e,#14b8a6)", align: "right" },
                        { kn: "ಚಾಕಿ ಅಂತ್ಯ",  en: "ME Brushed (DFLs)",    w: 160, bg: "linear-gradient(135deg,#15803d,#22c55e)", align: "right" },
                      ].map((c, i) => (
                        <th key={i} style={{
                  ...
                  <tbody>
                    ...
                    {filteredRows.map((row, ri) => {
                      ...
                      const moHas = numOrZero(row.mo_brushed) > 0;
                      const meHas = numOrZero(row.me_brushed) > 0;
                      // Alternate row banding by group index (find which group this row belongs to)
                      let groupIdx = 0;
                      for (let k = 0; k < slMerges.length; k++) {
                        if (ri >= slMerges[k].start && ri < slMerges[k].start + slMerges[k].count) {
                          groupIdx = k; break;
                        }
                      }
                      ...
                      return (
                        <tr key={`${row.sl_no}-${row.race}-${ri}`} className="tscrw-tr" style={{ background: rowBg }}>
                          {isFirstInGroup ? (
                          ...
                          <td style={{ ...cellBase, textAlign: "left" }}>
                            ...
                            <span style={{
                              marginLeft: "6px", fontSize: "9.5px", color: "#94a3b8", fontWeight: 700,
                            }}>{tone.label}</span>
                          </td>
                          <td className="tscrw-num" style={{
                            ...cellBase, textAlign: "right", paddingRight: "16px",
                            background: moHas ? "#f0fdfa" : "transparent",
                            color: moHas ? "#115e59" : "#cbd5e0",
                            fontWeight: 700,
                          }}>
                            {moHas ? fmtInt(row.mo_brushed) : "—"}
                          </td>
                          <td className="tscrw-num" style={{
                            ...cellBase, textAlign: "right", paddingRight: "16px",
                            background: meHas ? "linear-gradient(135deg,#dcfce7,#bbf7d0)" : "transparent",
                            color: meHas ? "#14532d" : "#cbd5e0",
                            fontWeight: 800,
                          }}>
                            {meHas ? fmtInt(row.me_brushed) : "—"}
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div style={{
                ...
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ನಮೂನೆ-28 / Form 28 — Race-wise Brushing (DFLs) · {filteredRows.length} / {dataRows.length} rows
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default TscMonthlyRaceWiseBrushingReport;

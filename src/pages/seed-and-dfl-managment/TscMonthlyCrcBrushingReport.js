...
function TscMonthlyCrcBrushingReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      confirmButtonText: "Got it", confirmButtonColor: "#d97706", background: "#fff", customClass: { popup: "tsccrc-swal" },
    });
  const showErr = (title, msg) =>
    Swal.fire({
      ...
      confirmButtonText: "Close", confirmButtonColor: "#e53e3e", background: "#fff", customClass: { popup: "tsccrc-swal" },
    });
    ...
    try {
      ...
      setDataRows(Array.isArray(res.data) ? res.data : []); setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the TSC Monthly CRC Brushing (Sheet 8) report.");
        }
      }
  ...
  return (
    <Layout title={t("TSC Monthly CRC Brushing — Sheet 8")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಸರ್ಕಾರಿ + ಖಾಸಗಿ ಚಾಕಿ ಸಾಕಾಣಿಕೆ ಕೇಂದ್ರಗಳ ವರದಿ — ಮಾಸಿಕ")}
            ...
            <Form onSubmit={handleView}>
              ...
              {hasReport && (
                <Row className="g-2 mt-2 align-items-end">
                  <Col md={6}>
                    <label style={lbl}>Quick Search (CRC, Race, Source)</label>

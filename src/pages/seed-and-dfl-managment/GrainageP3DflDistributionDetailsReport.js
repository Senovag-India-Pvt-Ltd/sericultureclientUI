...
if (!document.getElementById("gp3dd-styles")) {
  const s = document.createElement("style");
  s.id = "gp3dd-styles";
  s.innerHTML = `
    .gp3dd-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp3dd-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp3dd-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp3dd-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp3dd-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp3dd-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gp3dd-wrap { animation: gp3dd-in .35s ease; }
    .gp3dd-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gp3dd-table th { letter-spacing:.02em; }
    .gp3dd-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp3dd-scroll::-webkit-scrollbar { height:9px; }
    .gp3dd-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gp3dd-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
function GrainageP3DflDistributionDetailsReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp3dd-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-distribution-details", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the P3 DFL Distribution Details report.");
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-distribution-details/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-distribution-details/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p3_dfl_dist_details_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  // Grand total of DFLs (number_of_dfls is the only summable column)
  const totalDfls = useMemo(
    () => dataRows.reduce((acc, r) => acc + numOrZero(r.number_of_dfls), 0),
    [dataRows],
  );
  const distinctRecipients = useMemo(
    () => new Set(dataRows.map((r) => r.farm_name).filter(Boolean)).size,
    [dataRows],
  );

  return (
    <Layout title={t("P3 DFL Distribution Details Report")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ3 ಬಿತ್ತನೆಕೋಠಿಯಿಂದ ಪಿ4 ಕ್ಷೇತ್ರಗಳಿಗೆ ಹಂಚಿಕೆ ಮಾಡಿದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು")}
              <span style={{
              ...
              }}>P3 · DFL Distribution Details</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ3 ಬಿತ್ತನೆಕೋಠಿಯಿಂದ ಪಿ4 ಕ್ಷೇತ್ರಗಳಿಗೆ ಹಂಚಿಕೆ ಮಾಡಿದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P3 DFL Distribution Details — Per-lot details of DFLs distributed to P4 fields</div>
            </div>
            {hasReport && (
          ...
          <div className="gp3dd-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>Recipients</span>
                <span style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{distinctRecipients}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                ...
                <span className="gp3dd-num" style={{ fontSize: "16px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>
                  {totalDfls.toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಪಿ3 ಬಿತ್ತನೆಕೋಠಿಯಿಂದ ಪಿ4 ಕ್ಷೇತ್ರಗಳಿಗೆ ಹಂಚಿಕೆ ಮಾಡಿದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು &nbsp;·&nbsp; DFL Distribution Details
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  {grainageDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gp3dd-scroll" style={{ overflowX: "auto" }}>
                <table className="gp3dd-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "1200px" }}>
                  <thead>
                    <tr>
                      <th style={{
                      ...
                      <th style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಪಿ3 ಕ್ಷೇತ್ರಗಳ ವಿವರಗಳು</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Recipient Farm</div>
                      </th>
                      <th style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ತಂಡದ ಸಂಖ್ಯೆ</div>
                      ...
                      <th style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಲೈನ್</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Line</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(135deg,#0f766e,#14b8a6)",
                        color: "#fff", padding: "12px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಮೊಟ್ಟೆಗಳ ಸಂಖ್ಯೆ</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>DFLs</div>
                      </th>
                      <th style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಮೊಟ್ಟೆ ಇಟ್ಟ ದಿನಾಂಕ</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Laid</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(135deg,#4338ca,#6366f1)",
                        color: "#fff", padding: "12px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಶೀತೀಕರಿಸಿದ ದಿನಾಂಕ</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Cold Storage</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(135deg,#4338ca,#6366f1)",
                        color: "#fff", padding: "12px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಬಿಡುಗಡೆ ದಿನಾಂಕ</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Release</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(135deg,#4338ca,#6366f1)",
                        color: "#fff", padding: "12px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಚಾಕಿಯಾದ ದಿನಾಂಕ</div>
                        <div style={{ fontSize: "10px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Hatching</div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {dataRows.map((row, ri) => {
                      const alt = ri % 2 === 1;
                      return (
                        <tr key={`${row.sl_no}-${ri}`} className="gp3dd-tr" style={{ background: alt ? "#f8fafc" : "#ffffff" }}>
                          <td style={{
                          ...
                          <td className="gp3dd-num" style={{
                            padding: "11px 12px", textAlign: "center",
                            borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #eef2f6",
                            background: numOrZero(row.number_of_dfls) > 0 ? "#f0fdfa" : (alt ? "#f8fafc" : "#ffffff"),
                            color: numOrZero(row.number_of_dfls) > 0 ? "#134e4a" : "#cbd5e0",
                            fontWeight: 800, fontSize: "13px",
                          }}>{numOrZero(row.number_of_dfls) > 0 ? fmt(row.number_of_dfls) : "—"}</td>
                          <td style={{
                            padding: "11px 10px", textAlign: "center",
                            borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #e2e8f0",
                            color: "#334155", fontSize: "12.5px",
                          }}>
                            {row.laid_on_date || "—"}
                          </td>
                          <td style={{
                            ...
                            {row.cold_storage_date || "—"}
                          </td>
                          <td style={{
                            padding: "11px 10px", textAlign: "center",
                            borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #e2e8f0",
                            color: "#312e81", fontSize: "12.5px", fontWeight: 700,
                          }}>
                            {row.release_date || "—"}
                          </td>
                          <td style={{
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಪಿ3 / P3 DFL Distribution Details
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageP3DflDistributionDetailsReport;

...
if (!document.getElementById("tscdemo-styles")) {
  const s = document.createElement("style");
  s.id = "tscdemo-styles";
  s.innerHTML = `
    .tscdemo-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .tscdemo-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .tscdemo-swal .swal2-icon { margin:20px auto 4px !important; }
    .tscdemo-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .tscdemo-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes tscdemo-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .tscdemo-wrap { animation: tscdemo-in .35s ease; }
    .tscdemo-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .tscdemo-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .tscdemo-scroll::-webkit-scrollbar { height:9px; }
    .tscdemo-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .tscdemo-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#0f766e,#5b57ac); border-radius:6px; }
  `;
...
// Each snapshot has 8 sub-columns: SC, ST, Min, Dis, Oth, Total, Women, Men
const SNAPSHOT_SUBS = [
  { kn: "ಪ.ಜಾ.",  en: "SC",     suffix: "sc"    },
  { kn: "ಪ.ಪಂ.",  en: "ST",     suffix: "st"    },
  { kn: "ಅಲ್ಪಸಂ.", en: "Min",    suffix: "min"   },
  { kn: "ಚೇತನ",  en: "Disabled", suffix: "dis"  },
  { kn: "ಇತರೆ",  en: "Others",  suffix: "oth"   },
  { kn: "ಒಟ್ಟು",  en: "Total",   suffix: "total", total: true },
  { kn: "ಮಹಿಳೆ",  en: "Women",   suffix: "women" },
  { kn: "ಪುರುಷ",  en: "Men",     suffix: "men"   },
];

function TscMonthlyDemographicsReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "tscdemo-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/demographics", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC Monthly Demographics (Form 2A) report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/demographics/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-monthly/demographics/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      a.download = `tsc_monthly_demographics_${filter.talukId}_${filter.year}_${filter.month}.xlsx`;
      a.click();
  ...
  const kpis = useMemo(() => {
    ...
    dataRows.filter((r) => String(r.tsc_name || "").trim() !== "ಒಟ್ಟು").forEach((r) => {
      ...
      meSc             += numOrZero(r.me_sc);
      meSt             += numOrZero(r.me_st);
      meMin            += numOrZero(r.me_min);
      meDis            += numOrZero(r.me_dis);
      meWomen          += numOrZero(r.me_women);
      meMen            += numOrZero(r.me_men);
    });
    const growth = meTotal - fyTotal;
    const growthPct = fyTotal > 0 ? (growth * 100) / fyTotal : 0;
    return { totalVillages, mulberryVillages, fyTotal, meTotal, meSc, meSt, meMin, meDis, meWomen, meMen, growth, growthPct };
  }, [dataRows]);

  return (
    <Layout title={t("TSC Monthly Demographics (Form 2A)", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ರೇಷ್ಮೆ ಕೃಷಿಕರ ಮಾಹಿತಿ — ಮಾಸಿಕ ಸ್ನ್ಯಾಪ್‌ಶಾಟ್")}
              <span style={{
      ...
      <Block className="mt-n4">
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ರೇಷ್ಮೆ ಕೃಷಿಕರ ಮಾಹಿತಿ — TSC-wise Demographics Snapshot
                <span style={{ background: "rgba(255,255,255,.22)", padding: "2px 9px", borderRadius: "12px", marginLeft: "8px", fontSize: "10.5px", fontWeight: 800 }}>FORM 2A</span>
              </div>
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>
                Villages · FY-end Snapshot · Month-End Snapshot — Caste · Minority · Disability · Gender
              </div>
            </div>
            {hasReport && (
              ...
              <Row className="g-2 align-items-end">
                ...
                <Col md={3}>
                  ...
                  <ReactSelect
                    ...
                    value={
                      talukList
                        .map((tk) => ({ value: String(tk.talukId), label: i18n.language === "kn" ? (tk.talukNameInKannada || tk.talukName) : tk.talukName }))
        ...
        {/* ── Report Body ──────────────────────────────────────────────── */}
        {hasReport && (
          <div className="tscdemo-wrap mt-4">
            {/* KPI pills */}
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#cffafe,#ecfeff)", border: "1.5px solid #67e8f9", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0e7490", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಒಟ್ಟು / ರೇಷ್ಮೆ ಗ್ರಾಮಗಳು</span>
                <span className="tscdemo-num" style={{ fontSize: "16px", color: "#155e75", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.totalVillages)} / {fmtInt(kpis.mulberryVillages)}</span>
                <span style={{ fontSize: "10.5px", color: "#0e7490", marginTop: "2px" }}>Total / Mulberry villages</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#bbf7d0,#dcfce7)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#166534", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ME ಒಟ್ಟು ರೈತರು · Total Farmers</span>
                <span className="tscdemo-num" style={{ fontSize: "16px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.meTotal)}</span>
                <span className="tscdemo-num" style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, marginTop: "2px" }}>
                  Δ FY-end &nbsp;{kpis.growth >= 0 ? "▲" : "▼"} {fmtInt(Math.abs(kpis.growth))} ({kpis.growthPct >= 0 ? "+" : ""}{kpis.growthPct.toFixed(2)}%)
                </span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dbeafe,#eff6ff)", border: "1.5px solid #93c5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "170px" }}>
                <span style={{ fontSize: "11px", color: "#1d4ed8", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ME · SC / ST</span>
                <span className="tscdemo-num" style={{ fontSize: "14px", color: "#1e3a8a", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.meSc)} / {fmtInt(kpis.meSt)}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fbcfe8,#fce7f3)", border: "1.5px solid #f9a8d4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "190px" }}>
                <span style={{ fontSize: "11px", color: "#9d174d", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ME · Women / Men</span>
                <span className="tscdemo-num" style={{ fontSize: "14px", color: "#831843", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.meWomen)} / {fmtInt(kpis.meMen)}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "190px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ME · Min / Disabled</span>
                <span className="tscdemo-num" style={{ fontSize: "14px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>{fmtInt(kpis.meMin)} / {fmtInt(kpis.meDis)}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ನಮೂನೆ-2ಎ · ರೇಷ್ಮೆ ಕೃಷಿಕರ ಮಾಹಿತಿ — {talukName} — {monthKn} {filter.year}
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  Form 2A · TSC-wise Demographics Snapshot · {monthLabel} {filter.year}
                </div>
              </div>

              <div className="tscdemo-scroll" style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11.5px", minWidth: "1900px" }}>
                  <thead>
                    ...
                    <tr>
                      ...
                      <th colSpan={2} style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಗ್ರಾಮಗಳ ಮಾಹಿತಿ</div>
                        <div style={{ fontSize: "9.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Villages</div>
                      </th>
                      <th colSpan={8} style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>{fyEndLabel} ಇದ್ದಂತೆ</div>
                        <div style={{ fontSize: "9.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>FY-end Snapshot</div>
                      </th>
                      <th colSpan={8} style={{
                        ...
                        <div style={{ fontSize: "12.5px" }}>ಮಾಸಾಂತ್ಯ</div>
                        <div style={{ fontSize: "9.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Month-End Snapshot</div>
                      </th>
                    </tr>
                    {/* Row 2 — leaf headers */}
                    <tr>
                      ...
                      {[
                        { kn: "ಒಟ್ಟು",  en: "Total" },
                        { kn: "ರೇಷ್ಮೆ", en: "Mulberry" },
                      ].map((c, i) => (
                        <th key={`v-${i}`} style={{
                          background: "linear-gradient(180deg,#67e8f9,#22d3ee)", color: "#0c4a6e",
                          padding: "8px 4px", textAlign: "center",
                          border: "1px solid rgba(255,255,255,.18)",
                          fontWeight: 700, minWidth: "70px",
                        }}>
                          <div style={{ fontSize: "10.5px" }}>{c.kn}</div>
                          <div style={{ fontSize: "9px", opacity: .8, marginTop: "1px" }}>{c.en}</div>
                        </th>
                      ))}
                      {/* FY-end leaves */}
                      {SNAPSHOT_SUBS.map((s, i) => (
                        <th key={`fy-${i}`} style={{
                          background: "linear-gradient(180deg,#93c5fd,#60a5fa)",
                          color: "#1e3a8a",
                          padding: "8px 4px", textAlign: "center",
                          border: "1px solid rgba(255,255,255,.18)",
                          fontWeight: s.total ? 800 : 700, minWidth: "65px",
                        }}>
                          <div style={{ fontSize: "10.5px" }}>{s.kn}</div>
                          <div style={{ fontSize: "9px", opacity: .8, marginTop: "1px" }}>{s.en}</div>
                        </th>
                      ))}
                      {/* ME leaves */}
                      {SNAPSHOT_SUBS.map((s, i) => (
                  ...
                  <tbody>
                    {dataRows.length === 0 && (
                      <tr>
                        <td colSpan={20} style={{ padding: "40px 20px", textAlign: "center", color: "#a0aec0", fontSize: "14px" }}>
                    ...
                    {dataRows.map((row, ri) => {
                      ...
                      const numCell = (val, palette, isTotal, extraStyle = {}) => {
                        ...
                        return (
                          <td className="tscdemo-num" style={{
                            ...cellBase,
                            ...extraStyle,
                      ...
                      return (
                        <tr key={`${row.sl_no}-${ri}`} className="tscdemo-tr" style={{ background: rowBg }}>
                          <td style={{
                          ...
                          {/* Villages */}
                          {numCell(row.total_villages,    cyan, true)}
                          {numCell(row.mulberry_villages, cyan, false, { borderRight: "2px solid #e2e8f0" })}
                          {/* FY-end snapshot */}
                          {SNAPSHOT_SUBS.map((s, si) => {
                            const key = `fy_end_${s.suffix}`;
                            const isLast = si === SNAPSHOT_SUBS.length - 1;
                            return (
                              <React.Fragment key={`fy-c-${si}`}>
                                {numCell(row[key], blue, !!s.total, isLast ? { borderRight: "2px solid #e2e8f0" } : {})}
                              </React.Fragment>
                            );
                          })}
                          {/* ME snapshot */}
                          {SNAPSHOT_SUBS.map((s, si) => {
                            const key = `me_${s.suffix}`;
                            return (
                              <React.Fragment key={`me-c-${si}`}>
                                {numCell(row[key], green, !!s.total)}
                              </React.Fragment>
                            );
                          })}
                        </tr>
              ...
              <div style={{
                ...
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ನಮೂನೆ-2ಎ / Form 2A — Demographics Snapshot
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default TscMonthlyDemographicsReport;

...
if (!document.getElementById("gp3g-styles")) {
  const s = document.createElement("style");
  s.id = "gp3g-styles";
  s.innerHTML = `
    .gp3g-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp3g-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp3g-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp3g-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp3g-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp3g-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    @keyframes gp3g-pulse { 0%,100% { box-shadow:0 0 0 0 rgba(91,87,172,.35);} 50% { box-shadow:0 0 0 8px rgba(91,87,172,0);} }
    .gp3g-wrap { animation: gp3g-in .35s ease; }
    .gp3g-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gp3g-pill { animation: gp3g-pulse 2.4s infinite; }
    .gp3g-table th { letter-spacing:.02em; }
    .gp3g-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp3g-scroll::-webkit-scrollbar { height:9px; }
    .gp3g-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gp3g-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
    .gp3g-section-row td {
...
// Section-header row detector: serial_numbers 25 and 30 have an empty sub_label
// row that acts as a section heading with sub-rows (1, 2, ..., ∑) underneath.
const isSectionHeader = (row) => {
  const sn  = String(row.serial_number ?? "");
  const sub = String(row.sub_label ?? "");
  return (sn === "25" || sn === "30") && sub === "";
};

function GrainageP3GrainageMonthlyReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp3g-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "p3-grainage-monthly-report", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P3 Grainage Monthly Progress report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "p3-grainage-monthly-report/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "p3-grainage-monthly-report/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p3_grainage_monthly_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  // Visual grouping by serial_number — each unique sl_no produces one badge
  // (rendered with rowSpan), but every sub-row keeps its OWN description.
  ...
  // KPIs sourced from canonical rows in this 31-row form.
  const findRow = (sn, sub) => dataRows.find((r) =>
    String(r.serial_number) === String(sn) &&
    (sub === undefined || String(r.sub_label || "") === sub));
  const kpiCocoonQty   = numOrZero(findRow(3)?.cy_month);   // ಶೇಖರಿಸಿದ ಗೂಡಿನ ಪ್ರಮಾಣ
  const kpiDflsTotal   = numOrZero(findRow(8)?.cy_month);   // ತಯಾರಿಸಿದ ಒಟ್ಟು ಮೊಟ್ಟೆಗಳು
  const kpiDiseaseFree = numOrZero(findRow(11)?.cy_month);  // ಪಡೆದ ರೋಗ ರಹಿತ ಮೊಟ್ಟೆಗಳು
  const kpiSold        = numOrZero(findRow(20)?.cy_month);  // ಮಾರಾಟವಾದ ಮೊಟ್ಟೆಗಳು
  const kpiRevenue     = numOrZero(findRow(21)?.cy_month);  // ಮಾರಾಟದಿಂದ ಬಂದ ಹಣ
  const dfPct = kpiDflsTotal > 0 ? Math.round((kpiDiseaseFree / kpiDflsTotal) * 100) : null;

  return (
    <Layout title={t("P3 Grainage Monthly Progress Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ3 ಬಿತ್ತನೆಕೋಠಿ — ಮಾಸಿಕ ಪ್ರಗತಿ ವರದಿ")}
              <span style={{
              ...
              }}>{t("P3 · 31-row form", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ3 ಬಿತ್ತನೆಕೋಠಿ ಮಾಸಿಕ ಪ್ರಗತಿ ವರದಿ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P3 Grainage (Bittanekothi) Monthly Progress Report — Current Year vs Previous Year (31 rows)</div>
            </div>
            {hasReport && (
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                {dfPct !== null && (
                  <span className="gp3g-pill" style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>
                    {dfPct}% {t("Disease-free", { ns: "reports" })}
                  </span>
                )}
              </div>
            )}
        ...
        {hasReport && (
          <div className="gp3g-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಗೂಡಿನ ಪ್ರಮಾಣ</span>
                <span className="gp3g-num" style={{ fontSize: "14px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>{kpiCocoonQty.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                ...
                <span className="gp3g-num" style={{ fontSize: "14px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>{kpiDflsTotal.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dcfce7,#ecfdf5)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ರೋಗ ರಹಿತ</span>
                <span className="gp3g-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>
                  {kpiDiseaseFree.toLocaleString()}{dfPct !== null ? ` · ${dfPct}%` : ""}
                </span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಮಾರಾಟ / ಆದಾಯ</span>
                <span className="gp3g-num" style={{ fontSize: "13.5px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>
                  {kpiSold.toLocaleString()} &nbsp;·&nbsp; ₹ {kpiRevenue.toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಪಿ3 ಬಿತ್ತನೆಕೋಠಿ &nbsp;·&nbsp; P3 Grainage (Bittanekothi) Monthly Progress Report
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  ಸರ್ಕಾರಿ ರೇಷ್ಮೆ ಕೃಷಿ ಪಿ3 ಬಿತ್ತನೆಕೋಠಿ ವಿಭಾಗ, {grainageDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gp3g-scroll" style={{ overflowX: "auto" }}>
                <table className="gp3g-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "1100px" }}>
                  <thead>
                    ...
                    <tr>
                      ...
                      <th rowSpan={2} style={{
                        ...
                        border: "1px solid rgba(255,255,255,.18)", fontWeight: 800,
                        minWidth: "360px", verticalAlign: "middle",
                      }}>
                  ...
                  <tbody>
                    {groups.map((grp, gi) => {
                      ...
                      return grp.rows.map((row, ri) => {
                        ...
                        // Sub-label tinted pill
                        const subTint =
                          sub === "∑"            ? { bg: "linear-gradient(135deg,#fef3c7,#fcd34d)", color: "#78350f" }
                          : /^[1-9]$/.test(sub)   ? { bg: "linear-gradient(135deg,#e0f2fe,#bae6fd)", color: "#075985" }
                          : sub.includes("ಗುರಿ")   ? { bg: "linear-gradient(135deg,#bbf7d0,#86efac)", color: "#14532d" }
                        ...
                        if (sectionHeader) {
                          // Render the "ಖರ್ಚು" / "ಉತ್ಪಾದನಾ ವೆಚ್ಚ" headings as a full-width amber band.
                          return (
                            <tr key={`${grp.sn}-h-${ri}`} className="gp3g-section-row">
                              <td style={{ padding: "12px 8px", textAlign: "center", borderTop: "2px solid #f59e0b", borderBottom: "1px solid #f59e0b" }}>
                                <span style={{
                                  display: "inline-flex", alignItems: "center", justifyContent: "center",
                                  minWidth: "30px", height: "30px",
                                  borderRadius: "50%", background: "linear-gradient(135deg,#92400e,#78350f)",
                                  color: "#fff", fontWeight: 800, fontSize: "13px",
                                }}>{grp.sn}</span>
                              </td>
                              <td colSpan={6} style={{ padding: "12px 16px", textAlign: "left", borderTop: "2px solid #f59e0b", borderBottom: "1px solid #f59e0b", fontSize: "14px" }}>
                                {desc || "—"}
                              </td>
                            </tr>
                          );
                        ...
                        return (
                          <tr key={`${grp.sn}-${ri}`} className="gp3g-tr" style={{ background: altGroup ? "#f8fafc" : "#ffffff" }}>
                            {isFirst && (
                            ...
                            {/* Each sub-row keeps its OWN description (not shared via rowSpan) */}
                            <td style={{
                              padding: "11px 16px",
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಪಿ3 / {t("P3 Grainage Monthly Progress Report", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageP3GrainageMonthlyReport;

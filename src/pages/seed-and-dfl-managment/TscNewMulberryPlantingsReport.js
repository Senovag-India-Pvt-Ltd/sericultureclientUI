...
function TscNewMulberryPlantingsReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC New Mulberry Plantings report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("TSC New Mulberry Plantings Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">

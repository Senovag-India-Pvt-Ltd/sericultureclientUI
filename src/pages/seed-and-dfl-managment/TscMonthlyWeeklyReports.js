...
function TscMonthlyWeeklyReport({ config }) {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      confirmButtonText: t("Got it", { ns: "reports" }), confirmButtonColor: "#d97706", background: "#fff", customClass: { popup: "tscwk-swal" },
    });
  const showErr = (title, msg) =>
    Swal.fire({
      ...
      confirmButtonText: t("Close"), confirmButtonColor: "#e53e3e", background: "#fff", customClass: { popup: "tscwk-swal" },
    });
            ...
            <Form onSubmit={handleView}>
              ...
              {hasReport && (
                <Row className="g-2 mt-2 align-items-end">
                  <Col md={6}>
                    <label style={lbl}>{t("Quick Search (Race, Week)", { ns: "reports" })}</label>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "900px" }}>
                  ...
                  <tbody>
                    {filteredRows.length === 0 && (
                      <tr><td colSpan={5} style={{ padding: "40px 20px", textAlign: "center", color: "#a0aec0", fontSize: "14px" }}>{dataRows.length === 0 ? <>ಯಾವುದೇ ಮಾಹಿತಿ ಲಭ್ಯವಿಲ್ಲ / {t("No records found.", { ns: "reports" })}</> : t('No matches for "{{search}}".', { ns: "reports", search })}</td></tr>
                    )}

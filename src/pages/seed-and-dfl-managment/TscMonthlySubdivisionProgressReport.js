...
function TscMonthlySubdivisionProgressReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      confirmButtonText: "Got it", confirmButtonColor: "#d97706", background: "#fff", customClass: { popup: "tscsubdiv-swal" },
    });
  const showErr = (title, msg) =>
    Swal.fire({
      ...
      confirmButtonText: "Close", confirmButtonColor: "#e53e3e", background: "#fff", customClass: { popup: "tscsubdiv-swal" },
    });
    ...
    try {
      ...
      setDataRows(Array.isArray(res.data) ? res.data : []); setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the TSC Monthly Subdivision Progress report.");
        }
      }
  ...
  return (
    <Layout title={t("TSC Monthly Subdivision Progress")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
            ...
            <Form onSubmit={handleView}>
              ...
              {hasReport && (
                <Row className="g-2 mt-2 align-items-end">
                  <Col md={4}>
                    <label style={lbl}>Quick Search (TSC, Programme)</label>
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Type to filter…" style={{ ...sel, padding: "8px 12px" }} />
                  </Col>
                  <Col md={4}>
                    <label style={lbl}>Filter by Programme</label>
                    <ReactSelect
                      options={[{ value: "", label: "All Programmes" }, ...distinctProgrammes.map((p) => ({ value: p, label: `${programmeMeta(p).icon} ${p}` }))]}
                      value={programmeFilter ? { value: programmeFilter, label: `${programmeMeta(programmeFilter).icon} ${programmeFilter}` } : { value: "", label: "All Programmes" }}
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "1700px" }}>
                  ...
                  <tbody>
                    {filteredRows.length === 0 && (
                      <tr><td colSpan={11} style={{ padding: "40px 20px", textAlign: "center", color: "#a0aec0", fontSize: "14px" }}>{dataRows.length === 0 ? "ಯಾವುದೇ ಮಾಹಿತಿ ಲಭ್ಯವಿಲ್ಲ / No records found." : `No matches for the current filters.`}</td></tr>
                    )}
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; Subdivision Progress · 5 Programmes · {filteredRows.length} / {dataRows.length} rows
                </span>

...
if (!document.getElementById("gp3l-styles")) {
  const s = document.createElement("style");
  s.id = "gp3l-styles";
  s.innerHTML = `
    .gp3l-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp3l-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp3l-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp3l-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp3l-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp3l-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gp3l-wrap { animation: gp3l-in .35s ease; }
    .gp3l-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gp3l-table th { letter-spacing:.02em; }
    .gp3l-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp3l-scroll::-webkit-scrollbar { height:9px; }
    .gp3l-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gp3l-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
    .gp3l-section-row td {
...
// Section-header rows: serial_numbers 15 and 16 have an empty sub_label
// row (the section heading) and numbered sub-rows (1, 2) underneath.
const isSectionHeader = (row) => {
  const sn  = String(row.serial_number ?? "");
  const sub = String(row.sub_label ?? "");
  return (sn === "15" || sn === "16") && sub === "";
};
...
function GrainageP3LotwiseMonthlyReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp3l-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-lotwise-monthly", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P3 Lotwise Monthly DFL Production report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-lotwise-monthly/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-lotwise-monthly/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p3_lotwise_monthly_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  // KPIs sourced from canonical rows (cy_month = Lot 1, cy_cum = Lot 2)
  const findRow = (sn, sub) => dataRows.find((r) =>
    String(r.serial_number) === String(sn) &&
    (sub === undefined || String(r.sub_label || "") === sub));
  const lot1Number    = String(findRow(2)?.cy_month ?? "").trim();
  const lot2Number    = String(findRow(2)?.cy_cum   ?? "").trim();
  const lot1Dfls      = numOrZero(findRow(11)?.cy_month);
  const lot2Dfls      = numOrZero(findRow(11)?.cy_cum);
  const lot1Disease   = numOrZero(findRow(13)?.cy_month);
  const lot2Disease   = numOrZero(findRow(13)?.cy_cum);
  const lot1Selected  = numOrZero(findRow(8)?.cy_month);
  const lot2Selected  = numOrZero(findRow(8)?.cy_cum);

  return (
    <Layout title={t("P3 Lotwise Monthly DFL Production Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ3 ತಂಡಗಳವಾರು — ಮಾಸಿಕ ಮೊಟ್ಟೆ ಉತ್ಪಾದನೆ")}
              <span style={{
              ...
              }}>{t("P3 · Lotwise (Lot 1 vs Lot 2)", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ3 ತಂಡಗಳವಾರು ಮಾಸಿಕ ಮೊಟ್ಟೆ ಉತ್ಪಾದನೆ ವರದಿ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P3 Lot-wise Monthly DFL Production Report — Lot 1 vs Lot 2 (top-2 lots in month)</div>
            </div>
            {hasReport && (
        ...
        {hasReport && (
          <div className="gp3l-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Lot 1", { ns: "reports" })}</span>
                <span className="gp3l-num" style={{ fontSize: "13.5px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>
                  {lot1Number || "—"} &nbsp;·&nbsp; {lot1Dfls.toLocaleString()} DFLs
                </span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#4338ca", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Lot 2", { ns: "reports" })}</span>
                <span className="gp3l-num" style={{ fontSize: "13.5px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>
                  {lot2Number || "—"} &nbsp;·&nbsp; {lot2Dfls.toLocaleString()} DFLs
                </span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dcfce7,#ecfdf5)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ರೋಗರಹಿತ (Lot 1 / Lot 2)</span>
                <span className="gp3l-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>
                  {lot1Disease.toLocaleString()} &nbsp;·&nbsp; {lot2Disease.toLocaleString()}
                </span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಆಯ್ಕೆ ಗೂಡುಗಳು (Lot 1 / Lot 2)</span>
                <span className="gp3l-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>
                  {lot1Selected.toLocaleString()} &nbsp;·&nbsp; {lot2Selected.toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಪಿ3 ತಂಡಗಳವಾರು &nbsp;·&nbsp; P3 Lot-wise Monthly DFL Production Report
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  ಸರ್ಕಾರಿ ರೇಷ್ಮೆ ಕೃಷಿ ಪಿ3 ಬಿತ್ತನೆಕೋಠಿ ವಿಭಾಗ, {grainageDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gp3l-scroll" style={{ overflowX: "auto" }}>
                <table className="gp3l-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "900px" }}>
                  <thead>
                    <tr>
                      <th style={{
                      ...
                      <th style={{
                        ...
                        <div style={{ fontSize: "13px", fontWeight: 800 }}>ಬೆಳೆ-1</div>
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Lot 1</div>
                      </th>
                      <th style={{
                        ...
                        <div style={{ fontSize: "13px", fontWeight: 800 }}>ಬೆಳೆ-2</div>
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Lot 2</div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {groups.map((grp, gi) => {
                      ...
                      return grp.rows.map((row, ri) => {
                        ...
                        const lot1 = row.cy_month;
                        const lot2 = row.cy_cum;
                        ...
                        if (sectionHeader) {
                          return (
                            <tr key={`${grp.sn}-h-${ri}`} className="gp3l-section-row">
                              ...
                              <td colSpan={4} style={{ padding: "12px 16px", textAlign: "left", borderTop: "2px solid #f59e0b", borderBottom: "1px solid #f59e0b", fontSize: "14px" }}>
                                {desc || "—"}
                              </td>
                            </tr>
                          );
                        ...
                        return (
                          <tr key={`${grp.sn}-${ri}`} className="gp3l-tr" style={{ background: altGroup ? "#f8fafc" : "#ffffff" }}>
                            {isFirst && (
                            ...
                            <td className="gp3l-num" style={{
                              padding: "11px 12px", textAlign: "center",
                              borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #eef2f6",
                              background: has(lot1) ? "linear-gradient(135deg,#ccfbf1,#a7f3d0)" : (altGroup ? "#f8fafc" : "#ffffff"),
                              color: has(lot1) ? "#134e4a" : "#cbd5e0",
                              fontWeight: 800, fontSize: "13px",
                            }}>{has(lot1) ? fmt(lot1) : "—"}</td>
                            <td className="gp3l-num" style={{
                              padding: "11px 12px", textAlign: "center",
                              borderBottom: "1px solid #e2e8f0",
                              background: has(lot2) ? "linear-gradient(135deg,#e0e7ff,#c7d2fe)" : (altGroup ? "#f8fafc" : "#ffffff"),
                              color: has(lot2) ? "#312e81" : "#cbd5e0",
                              fontWeight: 800, fontSize: "13px",
                            }}>{has(lot2) ? fmt(lot2) : "—"}</td>
                          </tr>
                        );
                      });
                    })}
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಪಿ3 / {t("P3 Lotwise Monthly Report", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageP3LotwiseMonthlyReport;

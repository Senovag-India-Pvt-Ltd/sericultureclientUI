...
if (!document.getElementById("gp3p-styles")) {
  const s = document.createElement("style");
  s.id = "gp3p-styles";
  s.innerHTML = `
    .gp3p-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp3p-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp3p-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp3p-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp3p-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp3p-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gp3p-wrap { animation: gp3p-in .35s ease; }
    .gp3p-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gp3p-table th { letter-spacing:.02em; }
    .gp3p-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp3p-scroll::-webkit-scrollbar { height:9px; }
    .gp3p-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gp3p-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
function GrainageP3DflPreparationReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp3p-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-preparation", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P3 DFL Preparation Programme report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-preparation/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-dfl-preparation/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p3_dfl_preparation_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  const nextMonth = useMemo(() => {
    if (!monthNum) return { kn: "", label: "", year: null };
    let nm = monthNum + 1;
    let ny = monthYear;
    if (nm > 12) { nm = 1; ny = (monthYear || 0) + 1; }
    return { kn: MONTH_KN[nm] || "", label: MONTHS[nm - 1].label, year: ny };
  }, [monthNum, monthYear]);

  const row = dataRows[0] || {};
  const tgtTotal = numOrZero(row.tgt_total);
  const achTotal = numOrZero(row.ach_total);
  const nxtTotal = numOrZero(row.nxt_total);
  const achPct   = tgtTotal > 0 ? Math.round((achTotal / tgtTotal) * 100) : null;

  // Subtle red/green tint for the achievement vs target gap
  const achTone =
    achPct === null               ? { bg: "#f0fdfa", color: "#134e4a" }
  ...
  return (
    <Layout title={t("P3 DFL Preparation Programme Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ4 ಕ್ಷೇತ್ರಗಳಿಂದ ತಯಾರಿಸಿದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು")}
              <span style={{
              ...
              }}>{t("P3 · Target / Achievement / Next", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ3 ಮೊಟ್ಟೆ ತಯಾರಿಕೆ ಕಾರ್ಯಕ್ರಮ ಹಾಗೂ ಸಾಧನೆ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P3 DFL Preparation Programme — Target vs Achievement vs Next-month Target</div>
            </div>
            {hasReport && (
          ...
          <div className="gp3p-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                ...
                <span className="gp3p-num" style={{ fontSize: "14px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>{tgtTotal.toLocaleString()}</span>
              </div>
              <div style={{ background: achTone.bg, border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                ...
                <span className="gp3p-num" style={{ fontSize: "14px", color: achTone.color, fontWeight: 800, marginTop: "2px" }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Next Month Target", { ns: "reports" })} ({nextMonth.label || "—"})</span>
                <span className="gp3p-num" style={{ fontSize: "14px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>{nxtTotal.toLocaleString()}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಪಿ4 ಕ್ಷೇತ್ರಗಳಿಂದ ತಯಾರಿಸಿದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು &nbsp;·&nbsp; DFL Preparation Programme
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  {grainageDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gp3p-scroll" style={{ overflowX: "auto" }}>
                <table className="gp3p-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "1100px" }}>
                  <thead>
                    {/* Row 1 — group titles (Sl.No spans 2 rows; 3 groups across 5 cols each) */}
                    <tr>
                      <th rowSpan={2} style={{
                      ...
                      <th colSpan={5} style={{
                        ...
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>
                          Achievement — {monthLabel} {monthYear}
                        </div>
                      </th>
                      <th colSpan={5} style={{
                        ...
                        <div style={{ fontSize: "13px", fontWeight: 800 }}>ಮುಂದಿನ ಮಾಹೆಯ ಗುರಿ</div>
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>
                          Next Month Target — {nextMonth.label || "—"} {nextMonth.year || ""}
                        </div>
                      </th>
                    </tr>
                    {/* Row 2 — sub headers (I, II, III, IV, ಒಟ್ಟು) for each group */}
                    <tr>
                      {[0, 1, 2].map((gi) => {
                        const colors = [
                          { bg: "linear-gradient(180deg,#14b8a6,#0d9488)" },
                          { bg: "linear-gradient(180deg,#6366f1,#4f46e5)" },
                          { bg: "linear-gradient(180deg,#7d79c6,#5b57ac)" },
                        ];
                        return ["I", "II", "III", "IV", "ಒಟ್ಟು"].map((s, si) => (
                          <th key={`${gi}-${si}`} style={{
                            background: colors[gi].bg,
                            ...
                            <div style={{ fontSize: "9.5px", opacity: .85, marginTop: "1px" }}>{si === 4 ? "Total" : `Wk ${si + 1}`}</div>
                          </th>
                        ));
                      })}
                    </tr>
                  ...
                  <tbody>
                    ...
                    {dataRows.map((r, ri) => {
                      const cell = (key, tone) => {
                        ...
                        return (
                          <td key={key} className="gp3p-num" style={{
                            padding: "13px 10px", textAlign: "center",
                            borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #eef2f6",
                            background: has ? tone.bg : "#ffffff",
                            color: has ? tone.color : "#cbd5e0",
                            fontWeight: 700, fontSize: "13px",
                          }}>{has ? fmt(v) : "—"}</td>
                        );
                      };
                      const tgtTone = { bg: "#f0fdfa",                                  color: "#134e4a" };
                      const tgtTot  = { bg: "linear-gradient(135deg,#ccfbf1,#a7f3d0)", color: "#134e4a" };
                      const achTon2 = { bg: "#eef2ff",                                  color: "#312e81" };
                      const achTot  = { bg: "linear-gradient(135deg,#e0e7ff,#c7d2fe)", color: "#312e81" };
                      const nxtTone = { bg: "#f5f3ff",                                  color: "#4c1d95" };
                      const nxtTot  = { bg: "linear-gradient(135deg,#ede9fe,#ddd6fe)", color: "#4c1d95" };
                      return (
                        <tr key={ri} className="gp3p-tr">
                          <td style={{
                          ...
                          {cell("tgt_w3", tgtTone)}
                          {cell("tgt_w4", tgtTone)}
                          {cell("tgt_total", tgtTot)}
                          {cell("ach_w1", achTon2)}
                          {cell("ach_w2", achTon2)}
                          {cell("ach_w3", achTon2)}
                          {cell("ach_w4", achTon2)}
                          {cell("ach_total", achTot)}
                          {cell("nxt_w1", nxtTone)}
                          {cell("nxt_w2", nxtTone)}
                          {cell("nxt_w3", nxtTone)}
                          {cell("nxt_w4", nxtTone)}
                          {cell("nxt_total", nxtTot)}
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಪಿ3 / {t("P3 DFL Preparation Programme Report", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageP3DflPreparationReport;

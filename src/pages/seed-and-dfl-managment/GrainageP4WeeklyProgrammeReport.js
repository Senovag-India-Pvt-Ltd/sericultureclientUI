...
function GrainageP4WeeklyProgrammeReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P4 Weekly Programme report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("P4 Weekly Programme — Cocoon, Hatching & Upcoming Plan", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>

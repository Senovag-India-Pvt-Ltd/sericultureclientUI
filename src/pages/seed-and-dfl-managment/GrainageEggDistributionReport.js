    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          errAlert(t("No Data Found", { ns: "reports" }), t("No records found for the selected criteria.", { ns: "reports" }));
        } else {
          const data = err?.response?.data;
          const backendMsg = typeof data === "string"
            ? data
            : (data?.message || data?.error || data?.errorMessage || data?.error_description);
          errAlert(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the egg distribution report. Please try again.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
          ...
          <Card.Body style={{ padding: "14px 18px 16px" }}>
            <Form onSubmit={handleView}>
              <Row className="g-2 align-items-end">
                ...
                <Col md={3}>
                  <label style={lblSt}>{t("Financial Year")} <span style={{ color: "#e53e3e" }}>*</span></label>
                    ...
                    {dataRows.length === 0 && (
                      <tr>
                        <td colSpan={7} style={{ padding: "40px 20px", textAlign: "center", color: "#a0aec0", fontSize: "14px" }}>
                          {t("No egg distribution data found for the selected filters.", { ns: "reports" })}
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div style={{
                ...
                <div className="d-flex gap-2">
                  <button onClick={handlePdf} disabled={isDownloadingPdf} type="button"
                    ...
                    {isDownloadingPdf ? <><span className="spinner-border spinner-border-sm" style={{ width: "13px", height: "13px" }} /> …</> : <>📥 {t("PDF", { ns: "reports" })}</>}
                  </button>

...
if (!document.getElementById("gfmg-styles")) {
  const s = document.createElement("style");
  s.id = "gfmg-styles";
  s.innerHTML = `
    .gfmg-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gfmg-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gfmg-swal .swal2-icon { margin:20px auto 4px !important; }
    .gfmg-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gfmg-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gfmg-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gfmg-wrap { animation: gfmg-in .35s ease; }
    .gfmg-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gfmg-table th { letter-spacing:.02em; }
    .gfmg-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gfmg-scroll::-webkit-scrollbar { height:9px; }
    .gfmg-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gfmg-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
    .gfmg-section-row td {
...
// Section-header rows: 8 (organic-fertilizer) and 9 (chemical-fertilizer) carry
// only a description and act as headings for sub-rows below them.
const isSectionHeader = (row) => {
  const sn  = String(row.serial_number ?? "");
  const sub = String(row.sub_label ?? "");
  const desc = String(row.description_kannada ?? "");
  return (sn === "8" || sn === "9") && sub === "" && desc !== "";
};

function GrainageFarmMulberryGardenReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gfmg-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-mulberry-garden", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Farm Mulberry Garden report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-mulberry-garden/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-mulberry-garden/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `farm_mulberry_garden_${filter.farmId}_${year}_${m}.xlsx`;
      a.click();
  ...
  // Group rows by serial_number — each unique sl_no gets one badge with rowSpan
  const groups = useMemo(() => {
    const g = [];
    let cur = null;
    dataRows.forEach((row) => {
      const sn = String(row.serial_number ?? "").trim();
      // empty sn → continuation row of the previous group (variety sub-rows etc.)
      if (sn === "" && cur) {
        cur.rows.push(row);
        return;
      }
  ...
  // KPIs — pull totals from the SQL canonical rows
  const findRow = (sn, sub) => dataRows.find((r) =>
    String(r.serial_number) === String(sn) &&
    (sub === undefined || String(r.sub_label || "") === sub));
  const kpiArea       = numOrZero(findRow(1)?.cy_month);   // total area (hectares)
  const kpiIrrigated  = numOrZero(findRow(2)?.cy_month);   // irrigated (guntas)
  const kpiPlanted    = numOrZero(findRow(5)?.cy_month);   // newly planted
  // Variety totals — 'ಒಟ್ಟು' row sits under the variety block (sub_label='ಒಟ್ಟು', sn='')
  const kpiVarietyTot = numOrZero(dataRows.find(r => String(r.sub_label || "") === "ಒಟ್ಟು")?.cy_month);

  return (
    <Layout title={t("Farm Mulberry Garden Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಸರ್ಕಾರಿ ರೇಷ್ಮೆ ಕೃಷಿ ಕ್ಷೇತ್ರ — ಹಿಪ್ಪುನೇರಳೆ ತೋಟದ ವಿವರಗಳು")}
              <span style={{
              ...
              }}>{t("Farm · Mulberry Garden", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಹಿಪ್ಪುನೇರಳೆ ತೋಟದ ನಿರ್ವಹಣಾ ವರದಿ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>Mulberry Garden Maintenance — area, varieties, plantation, fertilizer (CY vs PY)</div>
            </div>
            {hasReport && (
        ...
        {hasReport && (
          <div className="gfmg-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ತೋಟ ವಿಸ್ತೀರ್ಣ (ಹೆ)</span>
                <span className="gfmg-num" style={{ fontSize: "14px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>{kpiArea > 0 ? fmt(kpiArea) : "—"}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#4338ca", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ನೀರಾವರಿ (ಗುಂಟೆ)</span>
                <span className="gfmg-num" style={{ fontSize: "14px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>{kpiIrrigated > 0 ? fmt(kpiIrrigated) : "—"}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಹೊಸ ನಾಟಿ</span>
                <span className="gfmg-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{kpiPlanted > 0 ? fmt(kpiPlanted) : "—"}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dcfce7,#ecfdf5)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ತಳಿವಾರು ಒಟ್ಟು</span>
                <span className="gfmg-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{kpiVarietyTot > 0 ? fmt(kpiVarietyTot) : "—"}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  Mulberry Garden Maintenance Report &nbsp;·&nbsp; CY vs PY (Month / Cumulative)
                </div>
              </div>

              <div className="gfmg-scroll" style={{ overflowX: "auto" }}>
                <table className="gfmg-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "1100px" }}>
                  <thead>
                    <tr>
                      <th rowSpan={2} style={{
                  ...
                  <tbody>
                    {groups.map((grp, gi) => {
                      ...
                      return grp.rows.map((row, ri) => {
                        ...
                        const subTint =
                          /ಒಟ್ಟು/.test(sub) ? { bg: "linear-gradient(135deg,#fef3c7,#fcd34d)", color: "#78350f" }
                          : /^[ಎಸ್|ಎಸ್‌|S]/.test(sub) ? { bg: "linear-gradient(135deg,#e0f2fe,#bae6fd)", color: "#075985" }
                          : /ವಿ1|V1/.test(sub) ? { bg: "linear-gradient(135deg,#dcfce7,#86efac)", color: "#14532d" }
                          : /ಇತರೆ/.test(sub) ? { bg: "linear-gradient(135deg,#f3f4f6,#e5e7eb)", color: "#374151" }
                          : /^[ಅಆಇಈ]\)/.test(sub) ? { bg: "linear-gradient(135deg,#fce7f3,#fbcfe8)", color: "#9d174d" }
                          : null;

                        if (sectionHeader) {
                          return (
                            <tr key={`${grp.sn || gi}-h-${ri}`} className="gfmg-section-row">
                        ...
                        return (
                          <tr key={`${grp.sn || gi}-${ri}`} className="gfmg-tr" style={{ background: altGroup ? "#f8fafc" : "#ffffff" }}>
                            {isFirst && (
                            ...
                            <td style={{
                              ...
                              {desc || (sub && !grp.sn ? <span style={{ color: "#64748b", fontWeight: 600 }}>↳</span> : "—")}
                            </td>
                            <td style={{
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಹಿಪ್ಪುನೇರಳೆ ತೋಟ / {t("Mulberry Garden", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageFarmMulberryGardenReport;

    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Diapause DFL Availability report.", { ns: "reports" }));
        }
          ...
          <Card.Body style={{ padding: "16px 20px 18px", background: "linear-gradient(180deg,#ffffff,#ecfeff)" }}>
            <Form onSubmit={handleView}>
              <Row className="g-2 align-items-end">
                ...
                <Col md={2}>
                  <label style={lbl}>
                    {t("Anchor Month", { ns: "reports" })} <span style={{ color: "#e53e3e" }}>*</span>
                    <span style={{ color: "#94a3b8", fontWeight: 600, textTransform: "none", letterSpacing: 0, marginLeft: "4px" }}>{t("(forecast = anchor + next 4)", { ns: "reports" })}</span>
                  </label>
                ...
                <Col md={4}>
                  <div className="d-flex gap-2 flex-wrap">
                    <button type="submit" disabled={isLoading} style={btn("linear-gradient(135deg,#155e75,#0e7490)", "0 4px 12px rgba(14,116,144,.32)", isLoading)}>
                      {isLoading ? <><span className="spinner-border spinner-border-sm" /> {t("Loading…", { ns: "reports" })}</> : <>📋 {t("View Month Forecast", { ns: "reports" })}</>}
                    </button>
                  ...
                  <tbody>
                    {dataRows.length === 0 && (
                      <tr><td colSpan={7} style={{ padding: "60px 20px", textAlign: "center", background: "linear-gradient(180deg,#ecfeff,#fff)" }}>
                        ...
                        <div style={{ fontSize: "13px", color: "#475569", fontWeight: 600 }}>{t("No diapause DFL availability data found for this grainage.", { ns: "reports" })}</div>
                      </td></tr>
                    )}

...
function GrainageMonthlyCostEdit() {
  ...
  const [data, setData] = useState({
    ...
    mysoreRaceCocoonValue: row?.mysoreRaceCocoonValue ?? "",
    chemicalEggSheetCost: row?.chemicalEggSheetCost ?? "",
    electricityCost: row?.electricityCost ?? "",
    fuelCost: row?.fuelCost ?? "",
    mysoreRaceEggsValue: row?.mysoreRaceEggsValue ?? "",
  });
  ...
  if (!row) {
    return (
      <Layout title={t("Edit Grainage Monthly Cost")}>
        <style>{grainageMonthlyCostEditStyles}</style>
        <Block className="mt-n4 sh-form-wrap">
          <Card className="sh-section-card">
            <Card.Body>
              <p>{t("Please open Edit from the list.")}</p>
              <Link to="/seriui/grainage-monthly-cost-list" className="btn btn-primary sh-cta-btn">
    ...
    <Layout title={t("Edit Grainage Monthly Cost")}>
      ...
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Grainage Monthly Cost")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Form noValidate validated={validated} onSubmit={postData}>
          <Card className="sh-section-card">
            <Card.Header className="sh-section-header">
              <Icon name="coins" />
              <span>{t("Edit Grainage Monthly Cost")}</span>
            </Card.Header>
            <Card.Body>
              <Row className="g-gs">
                <Col lg="6">
...
const grainageMonthlyCostEditStyles = `
  .sh-page-header {
    padding: 20px 24px;

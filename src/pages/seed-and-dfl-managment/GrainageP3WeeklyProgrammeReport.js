...
if (!document.getElementById("gp3w-styles")) {
  const s = document.createElement("style");
  s.id = "gp3w-styles";
  s.innerHTML = `
    .gp3w-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp3w-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp3w-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp3w-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp3w-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp3w-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    @keyframes gp3w-pulse { 0%,100% { box-shadow:0 0 0 0 rgba(91,87,172,.35);} 50% { box-shadow:0 0 0 8px rgba(91,87,172,0);} }
    .gp3w-wrap { animation: gp3w-in .35s ease; }
    .gp3w-pill { animation: gp3w-pulse 2.4s infinite; }
    .gp3w-table th { letter-spacing:.02em; }
    .gp3w-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp3w-card { transition: transform .15s ease, box-shadow .15s ease; }
    .gp3w-card:hover { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(67,56,202,.15); }
    .gp3w-badge {
      display:inline-flex; align-items:center; gap:4px;
      background: linear-gradient(135deg,#fef3c7,#fde68a);
      color:#92400e; padding:2px 10px; border-radius:20px;
      font-size:10.5px; font-weight:800; margin-left:8px;
...
const fmt = (v) => numOrZero(v).toLocaleString();
const shiftMonth = (year, month, delta) => {
  const d = new Date(year, month - 1 + delta, 1);
  return { year: d.getFullYear(), month: d.getMonth() + 1 };
};

function GrainageP3WeeklyProgrammeReport() {
  ...
  const [cocoonRow,    setCocoonRow]    = useState(null);
  const [hatchingRow,  setHatchingRow]  = useState(null);
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp3w-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    reset();
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-weekly-programme", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P3 Weekly Programme report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-weekly-programme/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/p3-weekly-programme/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p3_weekly_programme_${filter.farmId}_${year}_${m}.xlsx`;
      a.click();
    ...
    return (
      <div style={{ flex: 1, minWidth: "320px" }}>
        ...
        <div style={{ overflowX: "auto", border: "1.5px solid #e2e8f0", borderTop: "none", borderRadius: "0 0 10px 10px" }}>
          <table className="gp3w-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "440px" }}>
            <thead>
              <tr>
                {WEEK_COLS.map((w) => (
                  <th key={w.key} style={{
            ...
            <tbody>
              <tr>
                {WEEK_COLS.map((w) => {
                  ...
                  return (
                    <td key={w.key} className="gp3w-num" style={{
  ...
  return (
    <Layout title={t("P3 Weekly Programme — Cocoon, Hatching & Upcoming Plan", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ3 ಬಿಳಿದೇವಾಲಯ ವಾರವಾರು ಕಾರ್ಯಕ್ರಮ")}
              <span className="gp3w-badge">{t("P3 Division", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ3 ಬಿಳಿದೇವಾಲಯ ವಾರವಾರು ಕಾರ್ಯಕ್ರಮ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P3 Weekly Programme — Cocoon Production · Hatching Programme · Upcoming Plan</div>
            </div>
            {hasReport && (
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                {achPct !== null && (
                  <span className="gp3w-pill" style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>
                    {achPct}% {t("Achievement", { ns: "reports" })}
                  </span>
                )}
              </div>
            )}
        ...
        {hasReport && (
          <div className="gp3w-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Cocoon Prev / This", { ns: "reports" })}</span>
                <span className="gp3w-num" style={{ fontSize: "13.5px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                ...
                <span className="gp3w-num" style={{ fontSize: "13.5px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Upcoming Plan", { ns: "reports" })}</span>
                <span style={{ fontSize: "13.5px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>
                  {upcomingRows.length} {upcomingRows.length === 1 ? t("month", { ns: "reports" }) : t("months", { ns: "reports" })}
                </span>
              </div>
              <div style={{ marginLeft: "auto" }}>
                <span style={{ background: "linear-gradient(135deg,#fef3c7,#fde68a)", color: "#92400e", border: "1.5px solid #fcd34d", borderRadius: "20px", padding: "6px 16px", fontSize: "12px", fontWeight: 800 }}>
                  ಪಿ3 / {t("P3 Division", { ns: "reports" })}
                </span>
              </div>
            </div>

            {/* ── Section 1: Cocoon Production ─────────────────────── */}
            <Card className="gp3w-card" style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden", marginBottom: "20px" }}>
              <div style={{
                ...
                <div style={{ fontSize: "11.5px", fontWeight: 600, opacity: .9, marginTop: "3px" }}>
                  Cocoon Production (Previous Month) · Estimated Availability (This Month) — ಪಿ3 ವಿಭಾಗ
                </div>
              </div>
            ...
            <Card className="gp3w-card" style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(67,56,202,.10)", overflow: "hidden", marginBottom: "20px" }}>
              <div style={{
                ...
                <div style={{ fontSize: "11.5px", fontWeight: 600, opacity: .9, marginTop: "3px" }}>
                  Hatching Programme vs Achievement — ಪಿ3 ವಿಭಾಗ
                </div>
              </div>
            ...
            <Card className="gp3w-card" style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(124,58,237,.10)", overflow: "hidden", marginBottom: "20px" }}>
              <div style={{
                ...
                <div style={{ fontSize: "11.5px", fontWeight: 600, opacity: .9, marginTop: "3px" }}>
                  Upcoming 3-Month Hatching Programme — ಪಿ3 ವಿಭಾಗ
                </div>
              </div>
                  ...
                  return (
                    <div key={i} style={{ flex: 1, minWidth: "300px" }}>
                      <div style={{
                        ...
                        {row?.month_label || `${t("Month")} ${i + 1}`}
                      </div>
                      <div style={{ overflowX: "auto", border: "1.5px solid #e2e8f0", borderTop: "none", borderRadius: "0 0 10px 10px" }}>
                        <table className="gp3w-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "320px" }}>
                          <thead>
                            <tr>
                              {WEEK_COLS.map((w) => (
                                <th key={w.key} style={{
              ...
              <div style={{ background: "linear-gradient(135deg,#faf5ff,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #ddd6fe" }}>
                <span style={{ fontSize: "12px", color: "#5b21b6", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಪಿ3 / {t("P3 Weekly Programme", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageP3WeeklyProgrammeReport;

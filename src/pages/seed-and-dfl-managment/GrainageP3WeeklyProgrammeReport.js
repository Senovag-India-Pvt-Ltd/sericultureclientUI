...
function GrainageP3WeeklyProgrammeReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the P3 Weekly Programme report.");
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("P3 Weekly Programme — Cocoon, Hatching & Upcoming Plan")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ3 ಬಿಳಿದೇವಾಲಯ ವಾರವಾರು ಕಾರ್ಯಕ್ರಮ")}
              <span className="gp3w-badge">P3 Division</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>

...
function ColdStorageScheduleBVView() {
  ...
  return (
    <Layout title={t("View Cold Storage Schedule BV Details")}>
      <style>{coldStorageScheduleBVViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("View Cold Storage Schedule BV Details")} </Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-view-wrap">
        <Card className="sh-section-card">
          <Card.Header className="sh-section-header">
            <Icon name="calendar" />
            <span>{t("Cold Storage Schedule BV Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
              ...
              <Row className="g-gs">
                <Col lg="12">
                  <DetailGrid
                    items={[
                      { label: t("ID"), value: scheduleBV.id },
                      { label: t("Grainage Details"), value: scheduleBV.grainageDetails },
                      { label: t("Lot Number"), value: scheduleBV.lotNumber },
                      { label: t("Laid On Date"), value: scheduleBV.laidOnDate },
                      { label: t("Date Of Deposit"), value: scheduleBV.dateOfDeposit },
                      { label: t("Schedule Type"), value: scheduleBV.scheduleType },
                      { label: t("Date Of Release"), value: scheduleBV.dateOfRelease },
                    ]}
                  />
                </Col>
              </Row>
...
const coldStorageScheduleBVViewStyles = `
  .sh-page-header {
    padding: 20px 24px;

...
function GrainageP4FormSummaryReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P4 Form-27 B Summary report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("P4 Form-27 B Monthly Summary", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
              ...
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                {kpiAchPct !== null && (
                  <span className="gp4f-pill" style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>
                    {kpiAchPct}% {t("Achievement", { ns: "reports" })}
                  </span>
                )}
              </div>
            )}

...
function GrainageMonthlyFarmReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Monthly Farm Report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const farmDisplay = (i18n.language === "kn" ? (selectedFarm?.farmNameInKannada || selectedFarm?.farmName) : selectedFarm?.farmName) || "All Farms";
          ...
          <Card.Body style={{ padding: "16px 20px 18px", background: "linear-gradient(180deg,#ffffff,#f8fffe)" }}>
            <Form onSubmit={handleView}>
              <Row className="g-2 align-items-end">
                ...
                <Col md={3}>
                  <label style={lbl}>{t("Farm", { ns: "reports" })} <span style={{ color: "#94a3b8", fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>({t("optional", { ns: "reports" })})</span></label>
                  <Form.Select name="farmId" value={filter.farmId} onChange={handleChange} style={sel}>
                    <option value="">{t("— All Farms —", { ns: "reports" })}</option>
                    {farmList.map((f) => (

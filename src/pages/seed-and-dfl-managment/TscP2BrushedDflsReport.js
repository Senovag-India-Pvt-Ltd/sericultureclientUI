...
if (!document.getElementById("tscbd-styles")) {
  const s = document.createElement("style");
  s.id = "tscbd-styles";
  s.innerHTML = `
    .tscbd-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .tscbd-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .tscbd-swal .swal2-icon { margin:20px auto 4px !important; }
    .tscbd-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .tscbd-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes tscbd-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .tscbd-wrap { animation: tscbd-in .35s ease; }
    .tscbd-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .tscbd-table th { letter-spacing:.02em; }
    .tscbd-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .tscbd-scroll::-webkit-scrollbar { height:9px; }
    .tscbd-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .tscbd-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
// Stage tone — egg / nurturing / harvest
const STAGE_TONES = {
  "ಮೊಟ್ಟೆ":  { bg: "linear-gradient(135deg,#dbeafe,#bfdbfe)", color: "#1e3a8a", icon: "🥚", en: "Egg" },
  "ಪೋಷಣೆ": { bg: "linear-gradient(135deg,#fef3c7,#fde68a)", color: "#78350f", icon: "🐛", en: "Rearing" },
  "ಕಟಾವು": { bg: "linear-gradient(135deg,#bbf7d0,#86efac)", color: "#14532d", icon: "🌾", en: "Harvest" },
};

const COLUMNS = [
  { key: "sl_no",            kn: "ಕ್ರ.ಸಂ.",          en: "Sl.No",          width: "60px",  align: "center", group: "id"    },
  { key: "farmer_name",      kn: "ರೈತರ ಹೆಸರು",       en: "Farmer Name",    width: "220px", align: "left",   group: "id"    },
  { key: "village",          kn: "ಗ್ರಾಮ",            en: "Village",        width: "140px", align: "left",   group: "loc"   },
  { key: "lot_number",       kn: "ತಂಡದ ಸಂಖ್ಯೆ",      en: "Lot Number",     width: "120px", align: "center", group: "loc"   },
  { key: "dfl_count",        kn: "ಮೊಟ್ಟೆಗಳ ಸಂಖ್ಯೆ",  en: "DFL Count",      width: "110px", align: "center", group: "dfl",   numeric: true },
  { key: "laid_on_date",     kn: "ಮೊಟ್ಟೆ ಇಟ್ಟ ದಿನ",  en: "Laid On",        width: "110px", align: "center", group: "dfl"   },
  { key: "brushing_date",    kn: "ಚಾಕಿ ದಿನಾಂಕ",      en: "Brushing Date",  width: "110px", align: "center", group: "dfl"   },
  { key: "current_stage",    kn: "ಪ್ರಸ್ತುತ ಹಂತ",     en: "Current Stage",  width: "130px", align: "center", group: "stage" },
  { key: "est_ripening_date",kn: "ಅಂದಾಜು ಹಣ್ಣಾಗುವಿಕೆ", en: "Est. Ripening", width: "130px", align: "center", group: "stage" },
  { key: "est_cocoon_kg",    kn: "ಅಂದಾಜು ಗೂಡು (ಕಿ.ಗ್ರಾಂ)", en: "Est. Cocoon (kg)", width: "140px", align: "center", group: "stage" },
];

function TscP2BrushedDflsReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "tscbd-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-p2-brushed-dfls", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC P2 Brushed DFLs report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-p2-brushed-dfls/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-p2-brushed-dfls/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `tsc_p2_brushed_dfls_${filter.tscId}_${year}_${m}.xlsx`;
      a.click();
  ...
  const stats = useMemo(() => {
    let totalDfls = 0;
    const byStage = { "ಮೊಟ್ಟೆ": 0, "ಪೋಷಣೆ": 0, "ಕಟಾವು": 0 };
    dataRows.forEach((r) => {
      totalDfls += numOrZero(r.dfl_count);
      const s = String(r.current_stage || "").trim();
      if (byStage[s] !== undefined) byStage[s] += 1;
    });
    return { count: dataRows.length, totalDfls, byStage };
  }, [dataRows]);

  return (
    <Layout title={t("TSC P2 Brushed DFLs Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ತಾಂತ್ರಿಕ ಸೇವಾ ಕೇಂದ್ರ — ಚಾಕಿ ಮಾಡಿದ ಪಿ2 ಮೊಟ್ಟೆಗಳ ಬೆಳೆಗಳವಾರು ವರದಿ")}
              <span style={{
                display: "inline-flex", alignItems: "center", gap: "4px",
                background: "linear-gradient(135deg,#fce7f3,#fbcfe8)",
                color: "#9d174d", padding: "2px 10px", borderRadius: "20px",
                fontSize: "10.5px", fontWeight: 800, marginLeft: "8px",
                border: "1px solid #f9a8d4", verticalAlign: "middle",
              }}>ನಮೂನೆ-10ಎ · Form-10A · P2 Brushed DFLs</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ತಾಂತ್ರಿಕ ಸೇವಾ ಕೇಂದ್ರ — ಚಾಕಿ ಮಾಡಿದ ಪಿ2 ಮೊಟ್ಟೆಗಳ ಬೆಳೆಗಳವಾರು ವರದಿ
                <span style={{ background: "rgba(255,255,255,.22)", padding: "2px 9px", borderRadius: "12px", marginLeft: "8px", fontSize: "10.5px", fontWeight: 800 }}>ನಮೂನೆ-10ಎ</span>
              </div>
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>Form-10A · P2 Brushed DFLs — Crop-wise list with current rearing stage</div>
            </div>
            {hasReport && (
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                <span style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>
                  {stats.count} crop{stats.count === 1 ? "" : "s"}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="tscbd-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("TSC")}</span>
              ...
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#4338ca", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಒಟ್ಟು ಮೊಟ್ಟೆಗಳು</span>
                <span className="tscbd-num" style={{ fontSize: "14px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>{stats.totalDfls.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dbeafe,#eff6ff)", border: "1.5px solid #93c5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#1d4ed8", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>🥚 ಮೊಟ್ಟೆ</span>
                <span className="tscbd-num" style={{ fontSize: "14px", color: "#1e3a8a", fontWeight: 800, marginTop: "2px" }}>{stats.byStage["ಮೊಟ್ಟೆ"]}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>🐛 ಪೋಷಣೆ</span>
                <span className="tscbd-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{stats.byStage["ಪೋಷಣೆ"]}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#bbf7d0,#dcfce7)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#166534", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>🌾 ಕಟಾವು</span>
                <span className="tscbd-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{stats.byStage["ಕಟಾವು"]}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div className="tscbd-scroll" style={{ overflowX: "auto" }}>
                <table className="tscbd-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "12.5px", minWidth: "1400px" }}>
                  <thead>
                    <tr>
                      {COLUMNS.map((c) => {
                        const tone = c.group === "id"    ? "linear-gradient(135deg,#1e293b,#36506b)"
                                   : c.group === "loc"   ? "linear-gradient(135deg,#334155,#475569)"
                                   : c.group === "dfl"   ? "linear-gradient(135deg,#0f766e,#14b8a6)"
                  ...
                  <tbody>
                    ...
                    {dataRows.map((row, ri) => {
                      const alt = ri % 2 === 1;
                      return (
                        <tr key={`${row.sl_no}-${ri}`} className="tscbd-tr" style={{ background: alt ? "#f8fafc" : "#ffffff" }}>
                          {COLUMNS.map((c) => {
                            const v = row[c.key];
                            ...
                            if (c.key === "current_stage" && has) {
                              const tone = STAGE_TONES[String(v).trim()] || { bg: "#f1f5f9", color: "#475569", icon: "•", en: v };
                              return (
                                <td key={c.key} style={{
                                  padding: "10px 8px", textAlign: "center",
                              ...
                              return (
                                <td key={c.key} style={{
                                  ...
                                  <span style={{
                                    ...
                                    color: "#334155", fontWeight: 700, fontSize: "11.5px",
                                    fontFamily: "monospace",
                                  }}>{v}</span>
                                </td>
                              );
                            }
                            const display = c.numeric && has ? fmt(v) : (has ? v : "—");
                            return (
                              <td key={c.key} className={c.numeric ? "tscbd-num" : ""} style={{
                                padding: "10px 12px", textAlign: c.align,
                                borderBottom: "1px solid #e2e8f0", borderRight: "1px solid #eef2f6",
                                color: has
                                  ? (c.group === "dfl" ? "#134e4a" : c.group === "stage" ? "#312e81" : "#0f172a")
                                  : "#cbd5e0",
                    ...
                    {dataRows.length > 0 && (
                      <tr style={{ background: "linear-gradient(135deg,#fef3c7,#fde68a)" }}>
                        ...
                        <td className="tscbd-num" style={{
                          padding: "13px 12px", textAlign: "center",
                          color: "#134e4a", fontWeight: 800, fontSize: "13.5px",
                          borderTop: "2px solid #f59e0b",
                          background: "linear-gradient(135deg,#ccfbf1,#a7f3d0)",
                        }}>{stats.totalDfls.toLocaleString()}</td>
                        <td colSpan={5} style={{
                          padding: "13px 16px", textAlign: "left",
                          color: "#78350f", fontWeight: 600, fontSize: "12px",
                          borderTop: "2px solid #f59e0b", fontStyle: "italic",
                        }}>
                          {stats.count} crop{stats.count === 1 ? "" : "s"} &middot;
                          {" "}🥚 {stats.byStage["ಮೊಟ್ಟೆ"]} egg
                          &middot; 🐛 {stats.byStage["ಪೋಷಣೆ"]} rearing
                          &middot; 🌾 {stats.byStage["ಕಟಾವು"]} harvest
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ನಮೂನೆ-10ಎ / Form-10A P2 Brushed DFLs
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default TscP2BrushedDflsReport;

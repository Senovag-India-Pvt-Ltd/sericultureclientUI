...
const baseURLSeedDfl = process.env.REACT_APP_API_BASE_URL_SEED_DFL;
  ...
  function MaintenanceofLineRecordsforEachRaceForGrainage() {
    // Translation
    ...
    const postData = (event) => {
      ...
      if (form.checkValidity() === false) {
        ...
        api
          .post(baseURLSeedDfl + `LineRecordForGrainage/add-info`, payload)
          .then((response) => {
            // debugger;
  ...
  return (
    <Layout title={t("Maintenance of Line records for each race")}>
      <style>{lineRecordsForGrainageFormStyles}</style>
      <Block.Head>
            ...
            <Block className="mt-3">
              <Card className="sh-section-card">
                <Card.Header className="sh-section-header">
                  <Icon name="layers" />
                  <span>{t("Maintenance of Line records for each Female race")}</span>
                </Card.Header>
                <Card.Body>
                        <Row className="g-gs">
...
const lineRecordsForGrainageFormStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default MaintenanceofLineRecordsforEachRaceForGrainage;

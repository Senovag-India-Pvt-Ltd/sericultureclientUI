...
function GrainageP3CocoonSupplyReport() {
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr("Fetch Failed", backendMsg || err?.message || "Failed to load the P3 Cocoon Supply report.");
        }
      } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Layout title={t("P3 Cocoon Supply Report")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">

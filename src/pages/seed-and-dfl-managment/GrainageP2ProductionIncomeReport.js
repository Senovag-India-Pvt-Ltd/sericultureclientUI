...
if (!document.getElementById("gp2pi-styles")) {
  const s = document.createElement("style");
  s.id = "gp2pi-styles";
  s.innerHTML = `
    .gp2pi-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gp2pi-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gp2pi-swal .swal2-icon { margin:20px auto 4px !important; }
    .gp2pi-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gp2pi-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gp2pi-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gp2pi-wrap { animation: gp2pi-in .35s ease; }
    .gp2pi-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gp2pi-table th { letter-spacing:.02em; }
    .gp2pi-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gp2pi-scroll::-webkit-scrollbar { height:9px; }
    .gp2pi-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gp2pi-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
function GrainageP2ProductionIncomeReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gp2pi-swal" },
    });
    ...
    try {
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the P2 Production & Income report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handleExcel = async () => {
    ...
    try {
      ...
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `p2_production_income_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  // KPI snapshot from key metrics
  // sl 7  → DFLs produced (cy_month)
  // sl 13 → Disease-free DFLs (cy_month)
  // sl 18 → DFLs distributed (cy_month)
  // sl 19 → Revenue (cy_month)
  const kpis = useMemo(() => {
    const find = (sn) => dataRows.find((r) => String(r.serial_number) === String(sn) && (!r.sub_label || r.sub_label === "")) || {};
    return {
      dfls:        numOrZero(find(7).cy_month),
      diseaseFree: numOrZero(find(13).cy_month),
      distributed: numOrZero(find(18).cy_month),
      revenue:     numOrZero(find(19).cy_month),
    };
  }, [dataRows]);

  // Determine row "kind" for rendering — section-header (has sub-rows below sharing same SN), sub-row, total-row, or plain
  ...
  return (
    <Layout title={t("P2 Production & Income Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಪಿ2 ಮೊಟ್ಟೆ ಉತ್ಪಾದನೆ ಮತ್ತು ಆದಾಯ ವರದಿ")}
              <span style={{
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "15px", lineHeight: 1.2, letterSpacing: ".01em" }}>
                ಪಿ2 ಬಿತ್ತನೆಕೋಠಿ — ಮೊಟ್ಟೆ ಉತ್ಪಾದನೆ ಮತ್ತು ಆದಾಯ ವಿವರ
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>P2 Grainage Production & Income — Current Year vs Previous Year, Month vs Cumulative</div>
            </div>
            {hasReport && (
        ...
        {hasReport && (
          <div className="gp2pi-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#ccfbf1,#ecfdf5)", border: "1.5px solid #5eead4", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಉತ್ಪಾದಿಸಿದ ಮೊಟ್ಟೆಗಳು</span>
                <span className="gp2pi-num" style={{ fontSize: "14px", color: "#134e4a", fontWeight: 800, marginTop: "2px" }}>{kpis.dfls.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ರೋಗರಹಿತ ಮೊಟ್ಟೆಗಳು</span>
                <span className="gp2pi-num" style={{ fontSize: "14px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>{kpis.diseaseFree.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#4338ca", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ವಿತರಣೆಯಾದ ಮೊಟ್ಟೆಗಳು</span>
                <span className="gp2pi-num" style={{ fontSize: "14px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>{kpis.distributed.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಆದಾಯ ರೂ.</span>
                <span className="gp2pi-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>₹ {kpis.revenue.toLocaleString()}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಪಿ2 — ಮೊಟ್ಟೆ ಉತ್ಪಾದನೆ ಮತ್ತು ಆದಾಯ &nbsp;·&nbsp; P2 Production &amp; Income
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  {grainageDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gp2pi-scroll" style={{ overflowX: "auto" }}>
                <table className="gp2pi-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "12.5px", minWidth: "1200px" }}>
                  <thead>
                    {/* Row 1 — SL/Sub/Description span 2 rows; CY & PY span 2 cols each */}
                    <tr>
                      <th rowSpan={2} style={{
                      ...
                      <th rowSpan={2} style={{
                        ...
                        border: "1px solid rgba(255,255,255,.18)", fontWeight: 800,
                        minWidth: "350px", verticalAlign: "middle",
                      }}>
                    ...
                    {/* Row 2 — Month / Cumulative under each year group */}
                    <tr>
                      <th style={{
                  ...
                  <tbody>
                    ...
                    {enrichedRows.map((row, ri) => {
                      ...
                      return (
                        <tr key={`${row.serial_number}-${row.sub_label || ""}-${ri}`} className="gp2pi-tr" style={{ background: rowBg }}>
                          <td style={{
                          ...
                          <td className="gp2pi-num" style={{
                            ...
                            fontWeight: valWeight + (isTotal ? 0 : 100) > 800 ? 800 : valWeight,
                            fontSize: "12.5px",
                          }}>{has(cyC) ? fmt(cyC) : "—"}</td>
                          <td className="gp2pi-num" style={{
                            padding: "10px 12px", textAlign: "center",
...
export default GrainageP2ProductionIncomeReport;

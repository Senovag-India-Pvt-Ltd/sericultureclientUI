...
function RemittanceForFarmList() {
  ...
  const getList = () => {
    ...
    const response = api
      .get(baseURLSeedDfl + `RemittanceOfEggForFarm/get-info`)
      .then((response) => {
  ...
  const handleView = (_id) => {
    navigate(`/seriui/remittance-for-farm-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/remittance-for-farm-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        console.log("hello");
        const response = api
          .delete(baseURLSeedDfl + `RemittanceOfEggForFarm/delete-info/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("List Of Remittance(Eggs/PC/Others)")}>
      <style>{remittanceForFarmListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("List Of Remittance(Eggs/PC/Others)")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const remittanceForFarmListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default RemittanceForFarmList;

...
function P3FarmSheet9WeeklyHatchingReport() {
  ...
  const showWarn = (msg) => Swal.fire({ icon: "warning", title: t("Required Fields", { ns: "reports" }), html: `<div style="padding:8px 2px 12px"><div style="background:linear-gradient(135deg,#fffbeb,#fef9ec);border:1.5px solid #fcd34d;border-radius:14px;padding:16px 20px;color:#78350f">${msg}</div></div>`, confirmButtonText: t("Got it", { ns: "reports" }), confirmButtonColor: "#d97706", customClass: { popup: "p3s9-swal" } });
  const showErr = (title, msg) => Swal.fire({ icon: "error", title, html: `<div style="padding:8px 2px 12px"><div style="background:linear-gradient(135deg,#fff5f5,#fff);border:1.5px solid #feb2b2;border-radius:14px;padding:16px 20px;color:#9b2c2c">${msg}</div></div>`, confirmButtonText: t("Close"), confirmButtonColor: "#e53e3e", customClass: { popup: "p3s9-swal" } });
    ...
    try {
      ...
      setDataRows(Array.isArray(res.data) ? res.data : []); setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Sheet-9 Weekly Hatching report.", { ns: "reports" }));
        }
      }
  ...
  const farmName     = i18n.language === "kn"
    ? (selectedFarm?.farmNameInKannada || selectedFarm?.farmName || "—")
  ...
  return (
    <Layout title={t("Sheet-9 · P3 Farm Weekly Cocoon Hatching Programme", { ns: "reports" })}>

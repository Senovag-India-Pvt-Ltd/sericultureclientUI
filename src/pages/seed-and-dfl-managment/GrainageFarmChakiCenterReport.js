...
if (!document.getElementById("gfck-styles")) {
  const s = document.createElement("style");
  s.id = "gfck-styles";
  s.innerHTML = `
    .gfck-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .gfck-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .gfck-swal .swal2-icon { margin:20px auto 4px !important; }
    .gfck-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .gfck-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes gfck-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .gfck-wrap { animation: gfck-in .35s ease; }
    .gfck-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .gfck-table th { letter-spacing:.02em; }
    .gfck-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .gfck-scroll::-webkit-scrollbar { height:9px; }
    .gfck-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .gfck-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
const fmt = (v) => {
  ...
  if (!s) return "";
  if (s === "-") return "—";
  // Only number-format strict numeric strings; leave dates ("16-04-2026"),
  // date ranges ("16-04-2026 / 25-04-2026"), and any other text untouched.
...
function GrainageFarmChakiCenterReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "gfck-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-chaki-center", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the Farm Chaki Center report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-chaki-center/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/farm-chaki-center/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `farm_chaki_center_${filter.farmId}_${year}_${m}.xlsx`;
      a.click();
  ...
  const findRow = (sn) => dataRows.find((r) => String(r.sl_no) === String(sn));
  const totalDfls = useMemo(() => {
    const r6 = findRow(6);
    if (!r6) return 0;
    return numOrZero(r6.lot1) + numOrZero(r6.lot2) + numOrZero(r6.lot3) + numOrZero(r6.lot4);
  }, [dataRows]);
  const totalCocoons = useMemo(() => {
    const r14 = findRow(14);
    if (!r14) return 0;
    return numOrZero(r14.lot1) + numOrZero(r14.lot2) + numOrZero(r14.lot3) + numOrZero(r14.lot4);
  }, [dataRows]);
  const harvestedLots = useMemo(() => {
    const r1 = findRow(1);
    if (!r1) return 0;
    return [r1.lot1, r1.lot2].filter((v) => has(v)).length;
  }, [dataRows]);
  const inProgressLots = useMemo(() => {
    const r1 = findRow(1);
    if (!r1) return 0;
    return [r1.lot3, r1.lot4].filter((v) => has(v)).length;
  }, [dataRows]);

  return (
    <Layout title={t("Farm Chaki Center Monthly Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ಚಾಕಿ ಕೇಂದ್ರ ಮಾಸಿಕ ವಿವರಗಳು")}
              <span style={{
              ...
              }}>{t("Farm · Chaki Center", { ns: "reports" })}</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
        ...
        <Card className="mt-1" style={{ borderRadius: "14px", border: "none", boxShadow: "0 4px 20px rgba(13,78,72,.10)", overflow: "hidden" }}>
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: "11px", marginTop: "2px" }}>Chaki Center Monthly — Top 2 harvested lots + Top 2 in-progress lots, side by side</div>
            </div>
            {hasReport && (
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                ...
                <span style={{ background: "rgba(255,255,255,.22)", borderRadius: "20px", padding: "4px 12px", color: "#fff", fontSize: "11px", fontWeight: 700, backdropFilter: "blur(6px)" }}>
                  {harvestedLots} {t("harvested", { ns: "reports" })} · {inProgressLots} {t("in-progress", { ns: "reports" })}
                </span>
              </div>
            )}
          </div>
        ...
        {hasReport && (
          <div className="gfck-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#dcfce7,#ecfdf5)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#15803d", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಒಟ್ಟು ಮೊಟ್ಟೆಗಳು (Total DFLs)</span>
                <span className="gfck-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{totalDfls.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#e0e7ff,#eef2ff)", border: "1.5px solid #a5b4fc", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#4338ca", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಗೂಡಿನ ಪ್ರಮಾಣ (Cocoons)</span>
                <span className="gfck-num" style={{ fontSize: "14px", color: "#312e81", fontWeight: 800, marginTop: "2px" }}>{totalCocoons.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "150px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>{t("Harvested / In-progress", { ns: "reports" })}</span>
                <span className="gfck-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{harvestedLots} / {inProgressLots}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div style={{
                ...
                ಚಾಕಿ ಕೇಂದ್ರ ಮಾಸಿಕ ವಿವರಗಳು &nbsp;·&nbsp; Chaki Center Monthly Details
                <div style={{ fontSize: "12px", fontWeight: 600, opacity: .9, marginTop: "4px" }}>
                  {farmDisplay} &nbsp;·&nbsp; {monthKn} {monthYear || ""}
                </div>
              </div>

              <div className="gfck-scroll" style={{ overflowX: "auto" }}>
                <table className="gfck-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "1100px" }}>
                  <thead>
                    {/* Row 1: Sl.No / Description span 2 rows; group headers cover Lot 1+2 (Harvested) and Lot 3+4 (In-progress) */}
                    <tr>
                      <th rowSpan={2} style={{
                      ...
                      <th colSpan={2} style={{
                        ...
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>Harvested Lots</div>
                      </th>
                      <th colSpan={2} style={{
                        ...
                        <div style={{ fontSize: "13px", fontWeight: 800 }}>ಚಾಕಿಯಲ್ಲಿರುವ ಬೆಳೆಗಳು</div>
                        <div style={{ fontSize: "10.5px", fontWeight: 600, opacity: .85, marginTop: "2px" }}>In-progress Lots</div>
                      </th>
                    </tr>
                    <tr>
                      <th style={{
                        ...
                        <div style={{ fontSize: "12px" }}>ಬೆಳೆ-1</div>
                        <div style={{ fontSize: "9.5px", opacity: .85, marginTop: "1px" }}>Lot 1</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(180deg,#14b8a6,#0d9488)",
                        color: "#fff", padding: "8px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12px" }}>ಬೆಳೆ-2</div>
                        <div style={{ fontSize: "9.5px", opacity: .85, marginTop: "1px" }}>Lot 2</div>
                      </th>
                      <th style={{
                        ...
                        <div style={{ fontSize: "12px" }}>ಬೆಳೆ-3</div>
                        <div style={{ fontSize: "9.5px", opacity: .85, marginTop: "1px" }}>Lot 3</div>
                      </th>
                      <th style={{
                        background: "linear-gradient(180deg,#6366f1,#4f46e5)",
                        color: "#fff", padding: "8px 10px", textAlign: "center",
                        ...
                        <div style={{ fontSize: "12px" }}>ಬೆಳೆ-4</div>
                        <div style={{ fontSize: "9.5px", opacity: .85, marginTop: "1px" }}>Lot 4</div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {dataRows.map((row, ri) => {
                      const alt = ri % 2 === 1;
                      return (
                        <tr key={`${row.sl_no}-${ri}`} className="gfck-tr" style={{ background: alt ? "#f8fafc" : "#ffffff" }}>
                          <td style={{
                          ...
                          {[
                            { key: "lot1", tone: { bg: "linear-gradient(135deg,#ccfbf1,#a7f3d0)", color: "#134e4a" }, idle: "#f0fdfa" },
                            { key: "lot2", tone: { bg: "linear-gradient(135deg,#ccfbf1,#a7f3d0)", color: "#134e4a" }, idle: "#f0fdfa" },
                            { key: "lot3", tone: { bg: "linear-gradient(135deg,#e0e7ff,#c7d2fe)", color: "#312e81" }, idle: "#eef2ff" },
                            { key: "lot4", tone: { bg: "linear-gradient(135deg,#e0e7ff,#c7d2fe)", color: "#312e81" }, idle: "#eef2ff" },
                          ].map((c, ci) => {
                            const v = row[c.key];
                            const present = has(v);
                            // numeric values get the gradient highlight; text values (race name, lot number, dates) get the lighter idle bg
                            const isNumeric = present && /^-?\d+(\.\d+)?$/.test(String(v).trim());
                            return (
                              <td key={c.key} className="gfck-num" style={{
                                padding: "11px 12px", textAlign: "center",
                                borderBottom: "1px solid #e2e8f0",
                                borderRight: ci < 3 ? "1px solid #eef2f6" : undefined,
                                background: present
                                  ? (isNumeric ? c.tone.bg : c.idle)
                                  : (alt ? "#f8fafc" : "#ffffff"),
                                color: present ? c.tone.color : "#cbd5e0",
                                fontWeight: isNumeric ? 800 : 700, fontSize: "12.5px",
                              }}>{present ? fmt(v) : "—"}</td>
                            );
                          })}
                        </tr>
              ...
              <div style={{ background: "linear-gradient(135deg,#ecfdf5,#eef2ff)", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px", borderTop: "1.5px solid #c7d2fe" }}>
                <span style={{ fontSize: "12px", color: "#4338ca", fontWeight: 600 }}>
                  ...
                  &nbsp;·&nbsp; ಚಾಕಿ ಕೇಂದ್ರ / {t("Chaki Center", { ns: "reports" })}
                </span>
                <div className="d-flex gap-2 flex-wrap">
...
export default GrainageFarmChakiCenterReport;

...
if (!document.getElementById("tscch-styles")) {
  const s = document.createElement("style");
  s.id = "tscch-styles";
  s.innerHTML = `
    .tscch-swal { border-radius:22px !important; padding:8px !important; box-shadow:0 30px 90px rgba(0,0,0,.22) !important; }
    .tscch-swal .swal2-title { font-size:21px !important; font-weight:800 !important; color:#1a202c !important; }
    .tscch-swal .swal2-icon { margin:20px auto 4px !important; }
    .tscch-swal .swal2-html-container { margin:0 !important; padding:0 !important; }
    .tscch-swal .swal2-confirm { border-radius:11px !important; padding:12px 30px !important; font-weight:700 !important; }
    @keyframes tscch-in { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:none; } }
    .tscch-wrap { animation: tscch-in .35s ease; }
    .tscch-tr:hover td { filter:brightness(.97); transition:filter .12s; }
    .tscch-table th { letter-spacing:.02em; }
    .tscch-num { font-feature-settings:"tnum"; font-variant-numeric: tabular-nums; }
    .tscch-scroll::-webkit-scrollbar { height:9px; }
    .tscch-scroll::-webkit-scrollbar-track { background:#f1f5f9; border-radius:6px; }
    .tscch-scroll::-webkit-scrollbar-thumb { background:linear-gradient(135deg,#14b8a6,#5b57ac); border-radius:6px; }
  `;
...
// 12 numeric data keys ordered to match the visual column order
// (CY-Month-{dfl, crop, pct}) (CY-Cum-{dfl, crop, pct}) (PY-Month-...) (PY-Cum-...)
const VAL_KEYS = [
  "cy_m_dfl", "cy_m_crop", "cy_m_pct",
  "cy_c_dfl", "cy_c_crop", "cy_c_pct",
  "py_m_dfl", "py_m_crop", "py_m_pct",
  "py_c_dfl", "py_c_crop", "py_c_pct",
];

function TscChawkiHarvestReport() {
  ...
  const showWarn = (msg) =>
    Swal.fire({
      ...
      background: "#fff", customClass: { popup: "tscch-swal" },
    });
  ...
  const handleView = async (e) => {
    ...
    setDataRows([]);
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-chawki-harvest", { params: params() });
      ...
      setHasReport(true);
        ...
        if (status === 404 || status === 204) {
          ...
          showErr(t("Fetch Failed", { ns: "reports" }), backendMsg || err?.message || t("Failed to load the TSC Chawki & Harvest report.", { ns: "reports" }));
        }
      } finally {
      setIsLoading(false);
    }
  ...
  const handlePdf = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-chawki-harvest/pdf", { params: params(), responseType: "blob" });
  ...
  const handleExcel = async () => {
    ...
    try {
      const res = await api.get(baseURLSeedDFL + "grainage-progress-report/tsc-chawki-harvest/excel", { params: params(), responseType: "blob" });
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
      const a = document.createElement("a");
      a.href = url;
      const m = Number(filter.month);
      const year = m >= 4 ? fyStartYear : fyStartYear + 1;
      a.download = `tsc_chawki_harvest_${filter.tscId}_${filter.grainageId}_${year}_${m}.xlsx`;
      a.click();
  ...
  const kpis = useMemo(() => {
    ...
    return {
      achCYm: numOrZero(ach.cy_m_dfl),
      achCYc: numOrZero(ach.cy_c_dfl),
      achPYm: numOrZero(ach.py_m_dfl),
      p1Cur:  numOrZero(achP1.cy_m_dfl),
      p2Cur:  numOrZero(achP2.cy_m_dfl),
    };
  }, [dataRows]);

  return (
    <Layout title={t("TSC Chawki & Harvest Report", { ns: "reports" })}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">
              {t("ತಾಂತ್ರಿಕ ಸೇವಾ ಕೇಂದ್ರ — ಚಾಕಿ ಮಾಡಿದ ಮೊಟ್ಟೆಗಳ ಮತ್ತು ಕಟಾವಾದ ಮೊಟ್ಟೆಗಳ ವಿವರಗಳು")}
              <span style={{
              ...
              }}>ಭಾಗ-3 · Part-3 · Chawki & Harvest</span>
            </Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </Block.Head>
          ...
          <div className="tscch-wrap mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 mb-3 align-items-center">
              ...
              <div style={{ background: "linear-gradient(135deg,#bbf7d0,#dcfce7)", border: "1.5px solid #86efac", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "200px" }}>
                <span style={{ fontSize: "11px", color: "#166534", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಸಾಧನೆ (ಮಾಸ / ಒಟ್ಟು)</span>
                <span className="tscch-num" style={{ fontSize: "14px", color: "#14532d", fontWeight: 800, marginTop: "2px" }}>{kpis.achCYm.toLocaleString()} / {kpis.achCYc.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#dbeafe,#eff6ff)", border: "1.5px solid #93c5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#1d4ed8", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಪಿ1 (ಮಾಸ)</span>
                <span className="tscch-num" style={{ fontSize: "14px", color: "#1e3a8a", fontWeight: 800, marginTop: "2px" }}>{kpis.p1Cur.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#ede9fe,#f5f3ff)", border: "1.5px solid #c4b5fd", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಪಿ2 (ಮಾಸ)</span>
                <span className="tscch-num" style={{ fontSize: "14px", color: "#4c1d95", fontWeight: 800, marginTop: "2px" }}>{kpis.p2Cur.toLocaleString()}</span>
              </div>
              <div style={{ background: "linear-gradient(135deg,#fef3c7,#fef9ec)", border: "1.5px solid #fcd34d", borderRadius: "12px", padding: "10px 18px", display: "flex", flexDirection: "column", minWidth: "180px" }}>
                <span style={{ fontSize: "11px", color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em" }}>ಹಿಂದಿನ ವರ್ಷ (ಮಾಸ)</span>
                <span className="tscch-num" style={{ fontSize: "14px", color: "#78350f", fontWeight: 800, marginTop: "2px" }}>{kpis.achPYm.toLocaleString()}</span>
              </div>
            </div>
            ...
            <Card style={{ borderRadius: "14px", border: "none", boxShadow: "0 6px 28px rgba(13,78,72,.12)", overflow: "hidden" }}>
              ...
              <div className="tscch-scroll" style={{ overflowX: "auto" }}>
                <table className="tscch-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", minWidth: "1500px" }}>
                  <thead>
                      ...
                      {[0, 1, 2, 3].map((g) => {
                        ...
                        return [
                          <th key={`${g}-d`} style={{
                            ...
                            <div style={{ fontSize: "10.5px" }}>ಮೊಟ್ಟೆ</div>
                            <div style={{ fontSize: "8.5px", opacity: .8, marginTop: "1px" }}>DFLs</div>
                          </th>,
                          <th key={`${g}-c`} style={{
                            background: tone, color: text, padding: "8px 4px", textAlign: "center",
                            border: "1px solid rgba(255,255,255,.18)", fontWeight: 800,
                            ...
                            <div style={{ fontSize: "10.5px" }}>ಬೆಳೆ</div>
                            <div style={{ fontSize: "8.5px", opacity: .8, marginTop: "1px" }}>Crop</div>
                          </th>,
                          <th key={`${g}-p`} style={{
                            background: tone, color: text, padding: "8px 4px", textAlign: "center",
                            border: "1px solid rgba(255,255,255,.18)", fontWeight: 800,
                  ...
                  <tbody>
                    ...
                    {enrichedRows.map((row, ri) => {
                      ...
                      return (
                        <tr key={`${row.serial_number}-${row.sub_label || ""}-${ri}`} className="tscch-tr" style={{ background: rowBg }}>
                          <td style={{
                          ...
                          <td style={{
                            ...
                            paddingLeft: (k === "sub" || isTotal) ? "32px" : "14px",
                          }}>
                            {(k === "sub") && <span style={{ color: "#94a3b8", marginRight: "6px" }}>↳</span>}
                          ...
                          {VAL_KEYS.map((vk, vi) => {
                            ...
                            return (
                              <td key={vk} className="tscch-num" style={{
                                padding: "10px 6px", textAlign: "center",
...
export default TscChawkiHarvestReport;

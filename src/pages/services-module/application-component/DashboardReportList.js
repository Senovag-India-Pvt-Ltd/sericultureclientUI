...
function DashboardReportList() {
  ...
  const handleCloseModal = () => {
    ...
    setFirstRelease("");
    setFinalRelease("");
    setCstriFirstLetterNo("");
    setCstriFirstLetterDate("");
    setCstriFinalLetterNo("");
    setCstriFinalLetterDate("");
...
const handleDrawingOfficerChangeForSanction = (index, selectedUserId) => {
  ...
  const [cstriFirstLetterNo, setCstriFirstLetterNo] = useState("");
  const [cstriFirstLetterDate, setCstriFirstLetterDate] = useState("");
  const [cstriFinalLetterNo, setCstriFinalLetterNo] = useState("");
  const [cstriFinalLetterDate, setCstriFinalLetterDate] = useState("");
  ...
  const handleInvoiceRowChange = (idx, field, value) => {
    setProformaComponents((prev) => {
      ...
      updated[idx].eligibleAmount = (parseFloat(updated[idx].invoiceTotal) * (subsidyPct / 100)).toFixed(2);
      return updated;
    });
  };
  ...
  const loadArmProformaComponents = async (componentName, categoryId) => {
    ...
    try {
      ...
      const rows = (res.data.content?.armCalculation || []).map((c) => {
        const guidelineTotal = parseFloat(c.unitCost || 0);
        const subsidyPct     = parseFloat(c.centralPercentage || 0) + parseFloat(c.statePercentage || 0);
        return {
  ...
  const getActionFarmerList = async (fid, schemeId, componentType) => {
    ...
    api
      ...
      .then((response) => {
        ...
        if (recordData.directlyToFruits && !recordData.armFlow) {
        ...
        setDirectlyToFruits(recordData.directlyToFruits === true);
          ...
          =========================== */
        if (recordData.directlyToFruits === true && !recordData.armFlow) {
          // Submit must start as DISABLED always (non-ARM only)
          setDisplaySubmit(true);
        } else {
 ...
 useEffect(() => {
  ...
  if (missingOfficer) {
    ...
    if (actionFarmerData[0].pushToDbt) {
      ...
      sendPost = sendData;
    } else if (actionFarmerData[0].directlyToFruits && !actionFarmerData[0].armFlow) {
      const paymentType = actionData.schemeQuotaPaymentType;
      const ddoCodeToSend =
        paymentType === "B"
          ? reportingOfficerDdoCode
    ...
    // ARM stages with arm_flow=true bypass the directlyToFruits block and use inspectionUpdate
    if (
      !actionFarmerData[0].pushToDbt &&
      !actionFarmerData[0].sanctionOrder &&
      !actionFarmerData[0].workOrder &&
      (!actionFarmerData[0].directlyToFruits || actionFarmerData[0].armFlow)
    ) {
      apiCall = api.post(baseURLDBT + `service/inspectionUpdate`, sendPost);
    } else {
    ...
    if (actionFarmerData[0]?.sanctionOrder) {
      ...
      // Case 4: directly to fruits (skip for ARM — sanction order is Case 3)
      if (actionFarmerData[0].directlyToFruits &&
          actionFarmerData[0].workOrderForScheme !== "Automatic Reeling Machine Unit") {
        apiCall = api
          .post(
        ...
        .then((response) => {
          ...
          if (actionFarmerData[0]?.workOrder) {
            ...
            if (
              ...
              } else if (actionFarmerData[0].armStageConfig === "ARM User Master") {
                if (actionData.userId && actionData.stepId) {
                  const params = new URLSearchParams({
                    applicationFormId,
                    userMasterId: actionData.userId,
                    approvalStageId: actionData.stepId,
                  });
                  api.post(baseURLDBT + `service/saveArmUserMasterForApplication?${params.toString()}`)
                    .catch((err) => console.error("ARM User Master save failed:", err));
                }
              } else {
            ...
            // Auto-generate Advance Payment Letter on submit if flag is set
            if (
              actionFarmerData[0]?.advancePaymentLetter === true &&
              actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit"
            ) {
              api.post(
                baseURLReport + "arm-advance-payment",
                {
                  applicationFormId,
                  schemeId: workOrderSchemeId,
                  subSchemeId: actionFarmerData[0]?.subSchemeId,
                  categoryId: actionFarmerData[0]?.categoryId,
                },
                { responseType: "blob" }
              )
                ...
                .catch((err) => console.error("ARM Advance Payment letter generation failed:", err));
            }

            // Save CSTRI details + auto-generate First Release Letter on submit if flag is set
            if (
              actionFarmerData[0]?.firstReleaseLetter === true &&
              actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit"
            ) {
              const firstParams = new URLSearchParams({ applicationFormId, releaseType: "FIRST" });
              if (cstriFirstLetterNo)   firstParams.append("cstriLetterNo",   cstriFirstLetterNo);
              if (cstriFirstLetterDate) firstParams.append("cstriLetterDate", cstriFirstLetterDate);
              api.post(baseURLDBT + `service/updateCstriDetails?${firstParams.toString()}`)
                .then(() => {
                  api.post(
                    baseURLReport + "arm-first-release",
                    {
                      applicationFormId,
                      schemeId: workOrderSchemeId,
                      subSchemeId: actionFarmerData[0]?.subSchemeId,
                    ...
                    const file = new Blob([res.data], { type: "application/pdf" });
                    window.open(URL.createObjectURL(file));
                  }).catch((err) => console.error("ARM First Release letter generation failed:", err));
                })
                .catch((err) => console.error("CSTRI First Release save failed:", err));
            }

            // Save CSTRI details + auto-generate Final Release Letter on submit if flag is set
            if (
              actionFarmerData[0]?.finalReleaseLetter === true &&
              actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit"
            ) {
              const finalParams = new URLSearchParams({ applicationFormId, releaseType: "FINAL" });
              if (cstriFinalLetterNo)   finalParams.append("cstriLetterNo",   cstriFinalLetterNo);
              if (cstriFinalLetterDate) finalParams.append("cstriLetterDate", cstriFinalLetterDate);
              api.post(baseURLDBT + `service/updateCstriDetails?${finalParams.toString()}`)
                .then(() => {
                  api.post(
                    baseURLReport + "arm-final-release",
                    {
                      applicationFormId,
                      schemeId: workOrderSchemeId,
                      subSchemeId: actionFarmerData[0]?.subSchemeId,
                    ...
                    const file = new Blob([res.data], { type: "application/pdf" });
                    window.open(URL.createObjectURL(file));
                  }).catch((err) => console.error("ARM Final Release letter generation failed:", err));
                })
                .catch((err) => console.error("CSTRI Final Release save failed:", err));
            }
                        ...
                        >
                          ...
                          <Card.Body style={{ background: "#f8faff", padding: "20px", overflow: "visible" }}>
                            <Row>
                              ...
                              ) && !(actionFarmerData[0]?.armFlow && actionFarmerData[0]?.armStageConfig && actionFarmerData[0]?.armStageConfig !== "ARM User Master") && (
                              <Col lg="4">
                                <Form.Group className="form-group">
                              ...
                              {actionFarmerData[0]?.financialDelegation ? (
                                isSanctionOrderAllowed ? (
                                  ...
                                  <>
                                    ...
                                    {showDefaultUserField && actionFarmerData[0]?.armStageConfig !== "ARM User Master" && !actionFarmerData[0]?.finalReleaseLetter && (
                                    <Col lg="6">
                                      <Form.Group className="form-group">
                                        <Form.Label>
                                          User
                                    ...
                                    <Col lg="6">
                                      <Form.Group className="form-group">
                                        ...
                                        <Form.Select
                                          ...
                                          onChange={handleActionInputs}
                                          required
                                          disabled={fieldsDisabled || actionFarmerData[0]?.finalReleaseLetter === true}
                                        >
                                          <option value="">
                                            Select Approval Stage
                                          </option>
                              ...
                              {actionFarmerData[0]?.armStageConfig === "ARM User Master" && (
                                <Col lg="6">
                                  <Form.Group className="form-group">
                                    ...
                                    <ReactSelect
                                      ...
                                      value={
                                        userFromDistrictData
                                          ...
                                          .find((opt) => opt.value === actionData.userId) || null
                                      }
                                      onChange={(selectedOption) => {
                                        setActionData((prev) => ({
                                          ...prev,
  ...
  (
    ...
    (actionFarmerData[0].financialDelegation && isSanctionOrderAllowed)
                ...
                {/* ── ARM: Advance Payment Letter indicator (auto-generated on Submit) ── */}
                {actionFarmerData[0]?.advancePaymentLetter === true &&
                  actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit" && (
                  <Block className="mt-3">
                    <Card className="mb-3" style={{ border: "1px solid #b8d4f0", borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 8px rgba(15,108,190,0.10)" }}>
                      <Card.Header style={{ background: "linear-gradient(135deg, #1e67a8, #0d4f8a)", color: "#fff", fontWeight: "600", fontSize: "13px", padding: "10px 16px", letterSpacing: "0.3px" }}>
                        ARM Advance Payment Letter
                      </Card.Header>
                      <Card.Body style={{ padding: "12px 16px" }}>
                        <span style={{ fontSize: "13px", color: "#374151" }}>ARM Advance Payment Letter will be automatically generated on Submit.</span>
                      </Card.Body>
                    </Card>
                  </Block>
                )}

                {/* ── ARM: First Release Letter — CSTRI fields + auto-generated on Submit ── */}
                {actionFarmerData[0]?.firstReleaseLetter === true &&
                  actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit" && (
                  <Block className="mt-3">
                    <Card className="mb-3" style={{ border: "1px solid #b8d4f0", borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 8px rgba(15,108,190,0.10)" }}>
                      <Card.Header style={{ background: "linear-gradient(135deg, #1e67a8, #0d4f8a)", color: "#fff", fontWeight: "600", fontSize: "13px", padding: "10px 16px", letterSpacing: "0.3px" }}>
                        ARM First Release Letter
                      </Card.Header>
                      <Card.Body style={{ padding: "16px" }}>
                        <Row className="g-3">
                          <Col lg="6">
                            <Form.Group className="form-group">
                              <Form.Label style={{ fontWeight: "600", fontSize: "12px" }}>CSTRI Letter No.</Form.Label>
                              <Form.Control
                                type="text"
                                placeholder="Enter CSTRI Letter No."
                                value={cstriFirstLetterNo}
                                onChange={(e) => setCstriFirstLetterNo(e.target.value)}
                              />
                            </Form.Group>
                          </Col>
                          <Col lg="6">
                            <Form.Group className="form-group">
                              <Form.Label style={{ fontWeight: "600", fontSize: "12px" }}>CSTRI Letter Date</Form.Label>
                              <Form.Control
                                type="date"
                                value={cstriFirstLetterDate}
                                onChange={(e) => setCstriFirstLetterDate(e.target.value)}
                              />
                            </Form.Group>
                          </Col>
                        </Row>
                        <div style={{ marginTop: "10px", fontSize: "12px", color: "#6b7280" }}>
                          ARM First Release Letter will be automatically generated on Submit.
                        </div>
                      </Card.Body>
                    </Card>
                  </Block>
                ...
                {/* ── ARM: Final Release Letter — CSTRI fields + auto-generated on Submit ── */}
                {actionFarmerData[0]?.finalReleaseLetter === true &&
                  actionFarmerData[0]?.workOrderForScheme === "Automatic Reeling Machine Unit" && (
                  <Block className="mt-3">
                    <Card className="mb-3" style={{ border: "1px solid #b8d4f0", borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 8px rgba(15,108,190,0.10)" }}>
                      <Card.Header style={{ background: "linear-gradient(135deg, #1e67a8, #0d4f8a)", color: "#fff", fontWeight: "600", fontSize: "13px", padding: "10px 16px", letterSpacing: "0.3px" }}>
                        ARM Final Release Letter
                      </Card.Header>
                      <Card.Body style={{ padding: "16px" }}>
                        <Row className="g-3">
                          <Col lg="6">
                            <Form.Group className="form-group">
                              ...
                              <Form.Control
                                type="text"
                                placeholder="Enter CSTRI Letter No."
                                value={cstriFinalLetterNo}
                                onChange={(e) => setCstriFinalLetterNo(e.target.value)}
                              />
                            </Form.Group>
                          </Col>
                          <Col lg="6">
                            <Form.Group className="form-group">
                              ...
                              <Form.Control
                                type="date"
                                value={cstriFinalLetterDate}
                                onChange={(e) => setCstriFinalLetterDate(e.target.value)}
                              />
                            </Form.Group>
                          </Col>
                        </Row>
                        <div style={{ marginTop: "10px", fontSize: "12px", color: "#6b7280" }}>
                          ARM Final Release Letter will be automatically generated on Submit.
                        </div>
                      </Card.Body>
                    </Card>
                  </Block>
                ...
                <Col lg="12">
                  <div style={{
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => viewModal()}
                      hidden={!directlyToFruits || !!actionFarmerData[0]?.armFlow}
                      disabled={actionData.rejectType === "Permanent"}

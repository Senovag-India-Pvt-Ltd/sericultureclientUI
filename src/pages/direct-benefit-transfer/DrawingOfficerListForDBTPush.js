...
const [allowDbtPush, setAllowDbtPush] = useState(false);
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLDBT + `service/getDBTListForDBTPushReport`,
        {},
        {
          params: {
            userMasterId: localStorage.getItem("userMasterId"),
      ...
      .then((response) => {
        ...
        link.download = `dbt_list_for_dbt_push_report.csv`;
...
// }
  ...
  const drawingOfficerListForDBTPushStyles = `
    .sh-page-header {
      padding: 20px 24px;
  ...
  return (
    <Layout title="DBT Application Push">
      <style>{drawingOfficerListForDBTPushStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2" className="sh-page-title">{t("DBT Application Push")}</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>

...
const [sanctionOrderForScheme, setSanctionOrderForScheme] = useState(null);
  ...
  const getList = () => {
    ...
    api
      .post(
        baseURLDBT + `service/getK2RejectionList`,
        {},
        {
          params: {
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLDBT + `service/getK2RejectionListReport`,
        {},
        {
          params: {
          userMasterId: localStorage.getItem("userMasterId"),
      ...
      .then((response) => {
        ...
        link.download = `k2_rejection_list_report.csv`;
...
const handlePush = (id) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      // Step 2: Disable button immediately
      setDisabledIds((prevState) => [...prevState, id]);
      setCurrentPushId(id);
      // Step 3: Show modal instead of saving directly
      getCheckFileDetails([id]); // This opens setShowModal6(true)
    }
  });
};
 ...
 const handleCloseModal6 = () => {
  ...
  // re-enable button when modal closed
  setDisabledIds([]);
};
...
// to get sc-scheme-details
  ...
  const rejectionListForK2Styles = `
    .sh-page-header {
      padding: 20px 24px;
    ...
    .sh-modal-content .accordion-button { background: #eef4fc; font-weight: 700; color: #1e67a8; }
    .sh-modal-content .accordion-button:not(.collapsed) { background: linear-gradient(90deg, #1e67a8 0%, #2b7ac0 60%, #3b8dd6 100%); color: #fff; }
         ...
         <Form.Label column sm={1}>
            ...
            <Col sm={3}>
              <Form.Select
                ...
                onChange={handleInputsaddress}
              >
                <option value="">{t("Select Rejected Reason")}</option>
                {rejectedReasonListData && rejectedReasonListData.length
                  ? rejectedReasonListData.map((list) => (
                <option
                        key={list.rejectionListId}
                        value={list.rejectionListName}
                        >
...
export default RejectionListForK2;

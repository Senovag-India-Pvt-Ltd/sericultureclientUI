...
function DbtK2Application() {
  ...
  const search = (e) => {
    api
      .post(
        baseURLDBT + `service/getDbtK2ByList`,
        {},
        {
          params: {
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLDBT + `service/dbt-k2-list-report`,
        {},
        {
          params: {
            districtId: addressDetails.districtId,
      ...
      .then((response) => {
        ...
        link.download = `dbt_k2_report.xlsx`;
  ...
  const ApplicationDataColumns = [
    ...
    { name: colHeader(t("Application Status")), selector: (row) => row.applicationStatus,   cell: (row) => isPaymentSuccessInDbt(row.applicationStatus) ? <CreditedBankDetailsButton applicationFormId={row.scApplicationFormId} /> : <span style={{ color: "green", fontWeight: "bold" }}>{row.applicationStatus}</span>, sortable: true, hide: "md" },
    { name: colHeader(t("Quota Type")),         selector: (row) => row.quotaType,           cell: (row) => <span>{row.quotaType}</span>,           sortable: true, hide: "md" },
  ];

return (
  <Layout title={t("DBT K2 Application Report")}>
    <style>{dbtK2ApplicationStyles}</style>
    <Block.Head>
      <div className="sh-page-header">
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2" className="sh-page-title">{t("DBT K2 Application Report")}</Block.Title>
          </Block.HeadContent>
        </Block.HeadBetween>
      </div>
    </Block.Head>
    ...
    <Block className="mt-n4 sh-form-wrap">
      <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
        <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
          ...
          <div>
            <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("DBT K2 Application Report")}</div>
            <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to search and export K2 application data")}</div>
          </div>
        </div>
            ...
            <Col xs={6} sm={4} lg>
              <Form.Group className="form-group">
                ...
                {Number(searchData.type) === 5 ? (
                  ...
                  <Form.Control id="fruitsId" name="text" value={searchData.text} onChange={handleInputsSearch} type="text" placeholder={t("Search")} style={sel} />
                )}
              </Form.Group>
            </Col>
            <Col xs={6} sm={4} lg>
...
export default DbtK2Application;

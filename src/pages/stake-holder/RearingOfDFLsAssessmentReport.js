...
function RearingOfDFLsAssessmentReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `rearing_of_dfls_assessment_report.xlsx`;
  ...
  return (
    <Layout title={t("Rearing Of DFLs Assessment Details Report")}>
      <style>{rearingOfDFLsAssessmentReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Rearing Of DFLs Assessment Details Report")}</Block.Title>
            </Block.HeadContent>
...
const rearingOfDFLsAssessmentReportStyles = `
  .sh-page-header {
    padding: 20px 24px;

...
function MaintenanceOfLineReportForGrainage() {
  ...
  const FarmerDataColumns = [
    ...
    { name: colHeader(t("Lot Number")),                  selector: (row) => row.lotNumberMale,          cell: (row) => <span>{row.lotNumberMale}</span>,          sortable: true },
    ...
    { name: colHeader(t("Date Of Selection Cocoons")),   selector: (row) => row.dateOfSelectionCocoon,  cell: (row) => <span>{row.dateOfSelectionCocoon}</span>,  sortable: true },
    { name: colHeader(t("Number of DFLs(Female)")),      selector: (row) => row.numberOfDfls,           cell: (row) => <span>{row.numberOfDfls}</span>,           sortable: true },
    { name: colHeader(t("Number of DFLs(Male)")),        selector: (row) => row.numberOfDflsMale,       cell: (row) => <span>{row.numberOfDflsMale}</span>,       sortable: true },
    { name: colHeader(t("Number of Cocoons(Female)")),   selector: (row) => row.noOfCocoonsSelected,    cell: (row) => <span>{row.noOfCocoonsSelected}</span>,    sortable: true },
    { name: colHeader(t("Number of Cocoons(Male)")),     selector: (row) => row.noOfCocoonsSelectedMale, cell: (row) => <span>{row.noOfCocoonsSelectedMale}</span>, sortable: true },
    { name: colHeader(t("Average weight(Male)")),        selector: (row) => row.averageWeightMale,      cell: (row) => <span>{row.averageWeightMale}</span>,      sortable: true },
    { name: colHeader(t("Average weight(Female)")),      selector: (row) => row.averageWeight,          cell: (row) => <span>{row.averageWeight}</span>,          sortable: true },
  ];
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
          <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("Maintenance Of Line Report For Grainage")}</div>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to view and export maintenance of line records data")}</div>
            </div>
          </div>

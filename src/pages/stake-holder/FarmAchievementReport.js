...
function FarmAchievementReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `farm_achievement_report.xlsx`;
  ...
  const FarmerDataColumns = [
    ...
    { name: colHeader(t("Farm Name")),   selector: (row) => row.farmName,     cell: (row) => <span>{row.farmName}</span>,     sortable: true, hide: "md" },
  ...
  return (
    <Layout title={t("Farm Achievement Report")}>
      <style>{farmAchievementReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Farm Achievement Report")}</Block.Title>
            </Block.HeadContent>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
          <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("Farm Achievement Report")}</div>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to view and export farm achievement data")}</div>
            </div>
          </div>
...
const farmAchievementReportStyles = `
  .sh-page-header {
    padding: 20px 24px;

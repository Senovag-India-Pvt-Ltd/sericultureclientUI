...
function CropInspectionReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `crop_inspection_report.xlsx`;
  ...
  return (
    <Layout title={t("Crop Inspection Report")}>
      <style>{cropInspectionReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Crop Inspection Report")}</Block.Title>
            </Block.HeadContent>
...
const cropInspectionReportStyles = `
  .sh-page-header {
    padding: 20px 24px;

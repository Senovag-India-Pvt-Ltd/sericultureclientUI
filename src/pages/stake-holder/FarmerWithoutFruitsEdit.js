...
function FarmerWithoutFruitsEdit() {
  const { id } = useParams();
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(baseURL2 + `farmer/edit-karnataka-farmer-without-fruits-id`, sendData)
  ...
  return (
    <Layout title="Edit Famer without Fruits">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">{t("Edit Famer without Fruits")}</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
...
export default FarmerWithoutFruitsEdit;

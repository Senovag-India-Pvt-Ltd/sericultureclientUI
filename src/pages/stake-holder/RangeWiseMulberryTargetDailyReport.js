...
function RangeWiseMulberryTargetDailyReport() {
  ...
  const search = (e) => {
    api
      .post(
        baseURLFarmer + `mulberryTargets/getRangeWiseMulberryTargetsDetailsForDay`,
        {},
        {
          params: {
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLFarmer + `mulberryTargets/getRangeWiseMulberryTargetsReportForDay`,
        {},
        {
          params: {
            financialYearId: data.financialYearId || 0,
      ...
      .then((response) => {
        ...
        link.download = `range_wise_mulberry_targets_report_for_day.xlsx`;
  ...
  const FarmerDataColumns = [
    ...
    { name: colHeader(t("Target Date")),    selector: (row) => row.targetDate,             cell: (row) => <span>{row.targetDate}</span>,             sortable: true, hide: "md" },
  ];

  return (
    <Layout title={t("Range Wise Area Under Mulberry Daily Target Details Report")}>
      <style>{rangeWiseMulberryTargetDailyReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Range Wise Area Under Mulberry Daily Target Details Report")}</Block.Title>
            </Block.HeadContent>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
          <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("Range Wise Area Under Mulberry Daily Target Details Report")}</div>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to view and export range wise mulberry daily target data")}</div>
            </div>
          </div>
...
const rangeWiseMulberryTargetDailyReportStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default RangeWiseMulberryTargetDailyReport;

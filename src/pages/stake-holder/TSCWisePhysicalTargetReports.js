...
function TSCWisePhysicalTargetReports() {
  ...
  const search = (e) => {
    api
      .post(
        baseURLFarmer + `productionTargets/getTSCWisePhysicalTargetsDetails`,
        {},
        {
          params: {
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLFarmer + `productionTargets/getTSCWisePhysicalTargetsReport`,
        {},
        {
          params: {
            financialYearId: data.financialYearId || 0,
      ...
      .then((response) => {
        ...
        link.download = `tsc_wise_physical_targets_report.xlsx`;
  ...
  return (
    <Layout title={t("TSC Wise Physical Target Details Report")}>
      <style>{tscWisePhysicalTargetReportsStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("TSC Wise Physical Target Details Report")}</Block.Title>
            </Block.HeadContent>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
          <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("TSC Wise Physical Target Details Report")}</div>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to view and export TSC wise physical target data")}</div>
            </div>
          </div>
...
const tscWisePhysicalTargetReportsStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default TSCWisePhysicalTargetReports;

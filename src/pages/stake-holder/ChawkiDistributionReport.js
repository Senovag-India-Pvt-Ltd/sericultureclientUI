...
function ChawkiDistributionReport() {
  ...
  const search = (e) => {
    api
      .post(
        baseURLFarmer + `farmer/primaryChowkiDistributionDetails`,
        {},
        {
          params: {
  ...
  const exportCsv = (e) => {
    api
      .post(
        baseURLFarmer + `farmer/chowki-distribution-report`,
        {},
        {
          params: {
            districtId: data.districtId || 0,
      ...
      .then((response) => {
        ...
        link.download = `chowki_distribution_report.xlsx`;
  ...
  const FarmerDataColumns = [
    ...
    { name: colHeader(t("Source of DFLs")),   selector: (row) => row.dflsSource,      cell: (row) => <span>{row.dflsSource}</span>,      sortable: true, hide: "md" },
    { name: colHeader(t("Race")),             selector: (row) => row.raceName,        cell: (row) => <span>{row.raceName}</span>,        sortable: true, hide: "md" },
    { name: colHeader(t("Number of DFLs")),   selector: (row) => row.numbersOfDfls,   cell: (row) => <span>{row.numbersOfDfls}</span>,   sortable: true, hide: "md" },
    { name: colHeader(t("Lot Number")),       selector: (row) => row.lotNumberRsp,    cell: (row) => <span>{row.lotNumberRsp}</span>,    sortable: true, hide: "md" },
  ...
  return (
    <Layout title={t("Chawki Distribution Report")}>
      <style>{chawkiDistributionReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Chawki Distribution Report")}</Block.Title>
            </Block.HeadContent>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card style={{ borderRadius: "12px", border: "none", boxShadow: "0 2px 16px rgba(30,103,168,0.10)", backgroundColor: "#fff" }}>
          <div style={{ background: ACCENT_HEADER, padding: "11px 18px", display: "flex", alignItems: "center", gap: "10px", borderRadius: "12px 12px 0 0" }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px" }}>{t("Chawki Distribution Report")}</div>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: "11px" }}>{t("Select filters to view and export chawki distribution data")}</div>
            </div>
          </div>
...
const chawkiDistributionReportStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default ChawkiDistributionReport;

...
function UpdatedDFLsReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `verified_dfl_report.csv`;
  ...
  const FarmerDataColumns = [
    ...
    },
    {
      ...
      selector: (row) => row.expectedHatchingDate,
      cell: (row) => <span>{row.expectedHatchingDate}</span>,
      sortable: true,
      hide: "md",
    },
    {
  ...
  return (
    <Layout title={t("Verified DFL Details Report")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">{t("Verified DFL Details Report")}</Block.Title>
          </Block.HeadContent>

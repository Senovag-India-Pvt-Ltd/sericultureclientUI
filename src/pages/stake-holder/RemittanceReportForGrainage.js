...
function RemittanceReportForGrainage() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `remittance_of_eggs_report.xlsx`;
  ...
  return (
    <Layout title={t("Remittance Report")}>
      <style>{remittanceReportForGrainageStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Remittance Report")}</Block.Title>
            </Block.HeadContent>
...
const remittanceReportForGrainageStyles = `
  .sh-page-header {
    padding: 20px 24px;

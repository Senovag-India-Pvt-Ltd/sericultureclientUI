...
function MaintenanceOfPiercedCocoonsReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `maintenance_of_pierced_cocoons_report.csv`;
  ...
  const FarmerDataColumns = [
    ...
    {
      name: "Quantity in NUmber and Kgs",
    ...
    {
      name: "Moth",
      selector: (row) => row.moth,
      cell: (row) => <span>{row.moth}</span>,
      sortable: true,
      hide: "md",
    },
    {
  ...
  return (
    <Layout title={t("Maintenance Of Pierced Cocoons Report")}>
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">{t("Maintenance Of Pierced Cocoons Report")}</Block.Title>
          </Block.HeadContent>

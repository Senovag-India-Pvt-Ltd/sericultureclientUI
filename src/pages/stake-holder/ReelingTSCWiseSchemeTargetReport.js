...
function ReelingTSCWiseSchemeTargetReport() {
  ...
  const exportCsv = (e) => {
    api
      ...
      .then((response) => {
        ...
        link.download = `reeling_tsc_scheme_targets_report.xlsx`;
  ...
  return (
    <Layout title={t("Reeling TSC Wise Scheme Target Details Report")}>
      <style>{reelingTSCWiseSchemeTargetReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Reeling TSC Wise Scheme Target Details Report")}</Block.Title>
            </Block.HeadContent>
...
const reelingTSCWiseSchemeTargetReportStyles = `
  .sh-page-header {
    padding: 20px 24px;

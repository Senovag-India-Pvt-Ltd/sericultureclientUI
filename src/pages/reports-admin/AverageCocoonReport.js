...
function AverageCocoonReport() {
  ...
  const exportCsv = (e) => {
    ...
    api
      .post(
        baseURLReport + `excel-report/average-cocoon-report`,
        {
      ...
      .then((response) => {
        ...
        link.download = `average-cocoon-report.csv`;
  ...
  return (
    <Layout title="Average Report">
      <style>{averageCocoonReportStyles}</style>
      <Block.Head>
...
const averageCocoonReportStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default AverageCocoonReport;

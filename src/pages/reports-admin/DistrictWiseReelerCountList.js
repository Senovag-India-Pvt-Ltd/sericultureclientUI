...
function DistrictWiseReelerCountList() {
  ...
  const getDistrictList = () => {
    const response = api
      .post(baseURLFarmer + `reeler/districtWiseReelerCount`)
      .then((response) => {
  ...
  const getTalukList = (districtId) => {
    setLoading(true);
    api
      .post(baseURLFarmer + `reeler/talukWiseReelerCount`, { districtId })
      .then((response) => {
        if (response.data.content) {
  ...
  const getMarketList = (marketId) => {
    setLoading(true);
    api
      .post(baseURLFarmer + `reeler/marketWiseReelerCount`, { marketId })
      .then((response) => {
        if (response.data.content) {
        setMarketListData(response.data.content);
          // setTotalRows(response.data.totalCount); // Assuming total count is returned
        } else {
          setMarketListData([]);
          setTotalRows(0);

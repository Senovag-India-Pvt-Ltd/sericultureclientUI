...
function BlankDtrReport() {
          ...
          .then((response) => {
            if (!response.data.content.error) {
              if (response.data.content.reelerId) {
                ...
                api
                  .post(baseURLMarket + `auction/report/getBlankDTROnlineReport`, {
                    marketId: marketId,
                    // godownId: godownId,
  ...
  return (
    <Layout title={t("Real Time DTR Report")}>
      <style>{blankDtrReportStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Real Time DTR Report")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              {/* <ul className="d-flex">
              <li>

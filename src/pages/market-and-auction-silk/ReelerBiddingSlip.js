...
function ReelerBiddingSlip() {
  ...
  const [reelerLicenseNumber, setReelerLicenseNumber] = useState("");
  ...
  const display = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      api
        .post(
          baseURLFarmer +
            `reeler/get-reeler-details-by-fruits-id-or-reeler-number-or-mobile-number-or-reeler-license-number`,
          sendData
        )
        .then((response) => {
          if (response.data.content) {
            const reelerResponse = response.data.content.body.content[0];
            setReelerLicenseNumber(reelerResponse.reelerLicenseNumber);
            const res = response.data.content.farmerBankAccount;
            setReelerDetails(reelerResponse);
            // setFarmerAddress(response.data.content.farmerAddressList);
            setData((prev) => {
              return {
                ...prev,
                reelerId: reelerResponse.reelerId,
              };
            });
            setLoading(false);
            setIsActive(true);
...
//   };
  ...
  return (
    <Layout title={t("Silk Market Bidding Slip")} show="true">
      <style>{reelerBiddingSlipStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2" className="sh-page-title">{t("Silk Market Bidding Slip")}</Block.Title>
            {/* <nav>
          ...
          <Row className="g-3 ">
            <div className={isActive ? "" : "d-none"}>
              <Row className="g-gs">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td style={styles.bottom}>
                          {reelerLicenseNumber ? reelerLicenseNumber : "---"}
                        </td>
                      </tr>
                      <tr>
                ...
                <Col lg="5">
                  <Card>
                    <Card.Header className="sh-section-header"><Icon name="user" /><span>{t("Reeler Personal Info")}</span></Card.Header>
                    <Card.Body>
                      <Row className="g-gs">
                        <Col lg="12">
                          <table className="table small table-bordered">
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td>
                                  ...
                                  {reelerDetails && reelerDetails.arnNumber}
                                </td>
                              </tr>
                              <tr>
                  ...
                  <Card className="card-gutter-md mt-4">
                    <Card.Header className="sh-section-header"><Icon name="map-pin" /><span>{t("Reeler Address")}</span></Card.Header>
                    <Card.Body>
                      <Row className="g-gs">
                        <Col lg="12">
...
export default ReelerBiddingSlip;

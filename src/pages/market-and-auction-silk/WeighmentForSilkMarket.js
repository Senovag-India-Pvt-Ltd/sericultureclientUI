...
function WeighmentForSilkMarket() {
  ...
  const getLotDetails = (allottedLotId) => {
    ...
    api
      .post(baseURL1 + `auction/silk-market-weigment/getUpdateSilkWeighmentByLotId`, sendData)
      .then((response) => {
        // debugger;
  ...
  const getCrateDetails = (noOfCrates, allottedLotId) => {
    ...
    api
      .post(baseURL1 + `auction/silk-market-weigment/canContinuetoSilkWeighment`, sendData)
      .then((response) => {
        // debugger;
      ...
      if (result.value) {
        if (result.isConfirmed) {
          ...
          if (isTriplet) {
            ...
            console.log(t("In Market Master Change setting"));
          }
        }
      } else {
        setData({
  ...
  const onSubmitting = (e) => {
    ...
    api
      .post(baseURL1 + `auction/silk-market-weigment/completeSilkWeighmentForLot`, submitData)
      .then((response) => {
        // debugger;
        console.log(response.data.content);
        if (response.data.content) {
                      ...
                      <Col lg="8">
                        <table className="table smallwhiteback table-bordered">
                          <tbody>
                            ...
                            <tr>
                              <td style={styles.smallwhiteback}>
                                {t("Trader Details")}: {weigh.traderName}{" "}
                                {weigh.traderLicenseNumber}
                                {/* 353535-MARUTI */}
                              </td>
                            </tr>
                            <tr>
...
export default WeighmentForSilkMarket;

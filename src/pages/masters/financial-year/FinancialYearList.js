...
function FinancialYearList() {
  ...
  const getList = () => {
    ...
    const response = api
      .get(baseURL + `financialYearMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.financialYearMaster);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/financial-year-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/financial-year-edit/${_id}`);
  };
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `financialYearMaster/delete/${_id}`)
          .then((response) => {
            getList();
            Swal.fire(
              "Deleted",
  ...
  return (
    <Layout title="Financial Year List">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Financial Year List</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>

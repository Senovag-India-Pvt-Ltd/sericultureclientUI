...
function DistrictWiseMegaCluster() {
  // Translation
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      setSubmitting(true);
      api
        .post(baseURL + `districtWiseMegaCluster/add`, data)
        .then((response) => {
  ...
  return (
    <Layout title="District Wise Mega Cluster">
      <style>{districtWiseMegaClusterStyles}</style>
      <Block.Head>
...
const districtWiseMegaClusterStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default DistrictWiseMegaCluster;

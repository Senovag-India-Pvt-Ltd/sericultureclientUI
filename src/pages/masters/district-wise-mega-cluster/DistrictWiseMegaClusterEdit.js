...
function DistrictWiseMegaClusterEdit() {
  // Translation
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(baseURL + `districtWiseMegaCluster/edit`, {
          ...data,
          districtWiseMegaClusterId: id,
        })
        .then((response) => {
  ...
  const getMegaClusterList = () => {
    api
      .get(baseURL + `megaCluster/get-all`)
      .then((response) => {
  ...
  return (
    <Layout title="District Wise Mega Cluster">
      <style>{districtWiseMegaClusterEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">
                {t("District Wise Mega Cluster")}
              </Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
...
const districtWiseMegaClusterEditStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default DistrictWiseMegaClusterEdit;

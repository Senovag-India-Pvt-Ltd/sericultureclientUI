...
function DivisionMasterList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `divisionMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.DivisionMaster);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/division-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/division-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `divisionMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Division List")}>
      <style>{divisionMasterListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Division List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const divisionMasterListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default DivisionMasterList;

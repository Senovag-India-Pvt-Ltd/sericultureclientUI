...
function DivisionMasterEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(baseURL + `divisionMaster/edit`, datas)
        .then((response) => {
  ...
  return (
    <Layout title={t("Edit Division")}>
      <style>{divisionMasterEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Division")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const divisionMasterEditStyles = `
  .sh-page-header {
    padding: 20px 24px;

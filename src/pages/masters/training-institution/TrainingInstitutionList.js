...
function TrainingInstitutionList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trInstitutionMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.trInstitutionMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trInstitutionMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const TrInstitutionMasterDataColumns = [
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.trInstitutionMasterId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.trInstitutionMasterId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.trInstitutionMasterId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
    ...
    {
      name: t("Training Institution Name in Kannada"),
  ...
  return (
    <Layout title="Training Institution List">
      <style>{trainingInstitutionListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Institution List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={TrInstitutionMasterDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const trainingInstitutionListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default TrainingInstitutionList;

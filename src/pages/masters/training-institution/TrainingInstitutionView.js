...
function TrainingInstitutionView() {
  ...
  //   setTrInstitutionMaster(findUser);
  // }, [id, data]);
  const getIdList = () => {
    setLoading(true);
    const response = api
      ...
      .then((response) => {
        setTrainingInstitution(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <Layout title="Training Institution View">
      <style>{trainingInstitutionViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Institution View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Training Institution Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
const trainingInstitutionViewStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default TrainingInstitutionView;

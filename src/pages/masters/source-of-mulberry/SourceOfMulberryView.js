...
function SourceOfMulberryView() {
  ...
  const [SourceOfMulberry, setSourceOfMulberry] = useState({});
  const [loading, setLoading] = useState(false);

  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `mulberry-source/get/${id}`)
      .then((response) => {
        setSourceOfMulberry(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setSourceOfMulberry({});
  ...
  return (
    <Layout title="Source Of Mulberry View">
      <style>{sourceOfMulberryViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Source Of Mulberry View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Source Of Mulberry Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
const sourceOfMulberryViewStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default SourceOfMulberryView;

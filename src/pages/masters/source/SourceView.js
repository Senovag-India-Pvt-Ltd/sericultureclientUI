...
function SourceView() {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `sourceMaster/get/${id}`)
      .then((response) => {
        setSourceMaster(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setSourceMaster({});
  ...
  return (
    <Layout title={t("Source View")}>
      <style>{sourceViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Source View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

...
function HobliList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hobli/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.hobli);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hobli/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const HobliDataColumns = [
    {
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          >
            <Icon name="eye" /><span>{t("View")}</span>
          </Button>
          <Button
            variant="primary"
            size="sm"
  ...
  return (
    <Layout title="Hobli List">
      <style>{hobliListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Hobli List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

...
function SilkWormVarietyList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `silk-worm-variety/list`, _params)
      .then((response) => {
        setListData(response.data.content.silkWormVariety);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `silk-worm-variety/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Silk Worm Variety List">
      <style>{silkWormVarietyListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("Silk Worm Variety List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={SilkWormVarietyDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const silkWormVarietyListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default SilkWormVarietyList;

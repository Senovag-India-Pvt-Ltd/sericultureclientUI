...
function LandCategoryList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `landCategory/list`, _params)
      .then((response) => {
        setListData(response.data.content.landCategory);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `landCategory/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Land Holding Category List">
      <style>{landCategoryListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Land Holding Category List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={LandCategoryDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
export default LandCategoryList;

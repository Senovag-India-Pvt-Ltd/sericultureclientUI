...
function RaceMappingList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `raceMarketMaster/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.raceMarketMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `raceMarketMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Race Mapping List")}>
      <style>{raceMappingListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Race Mapping List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
export default RaceMappingList;

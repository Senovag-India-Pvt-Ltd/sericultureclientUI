...
function IrrigationSourceList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `irrigationSource/list`, _params)
      .then((response) => {
        setListData(response.data.content.irrigationSource);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `irrigationSource/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Irrigation Source List">
      <style>{irrigationSourceListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("Irrigation Source List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const irrigationSourceListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default IrrigationSourceList;

...
function ScVendorList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `scVendor/list`, _params)
      .then((response) => {
        setListData(response.data.content.ScVendor);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/sc-vendor-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/sc-vendor-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `scVendor/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Sc Vendor List">
      <style>{scVendorListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Sc Vendor List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

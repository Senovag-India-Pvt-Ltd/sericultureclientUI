...
function HelpDeskFaqList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdQuestionMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.hdQuestionMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdQuestionMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const HdQuestionDataColumns = [
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            ...
            onClick={() => handleEdit(row.hdQuestionId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.hdQuestionId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
             <Icon name="trash" />
             {t("delete")}
    ...
    {
      name: t("Questions"),
      selector: (row) => row.hdQuestionName,
      cell: (row) => <span>{row.hdQuestionName}</span>,
      sortable: true,
      hide: "md",
    },
    {
  ...
  return (
    <Layout title="Help Desk Question List ">
      <style>{helpDeskFaqListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Help Desk Question List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
export default HelpDeskFaqList;

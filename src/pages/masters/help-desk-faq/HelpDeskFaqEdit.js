...
function HelpDeskFaqEdit() {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdQuestionMaster/get/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="Edit FAQ">
      <style>{helpDeskFaqEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit FAQ")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
export default HelpDeskFaqEdit;

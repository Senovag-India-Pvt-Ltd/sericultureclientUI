...
function HelpDeskModuleList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdModuleMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.hdModuleMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdModuleMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Modules">
      <style>{helpDeskModuleListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Modules")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const helpDeskModuleListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskModuleList;

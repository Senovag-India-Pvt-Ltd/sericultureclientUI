...
function ScHeadAccountCategoryEdit() {
  const { id } = useParams();
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `scHeadAccountCategory/edit`, data)
        .then((response) => {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `scHeadAccountCategory/get/${id}`)
      .then((response) => {
  ...
  const [headAccountListData, setHeadAccountListData] = useState([]);

  const getHeadAccountList = () => {
    const response = api
      .get(baseURL + `scHeadAccount/get-all`)
      .then((response) => {
        setHeadAccountListData(response.data.content.scHeadAccount);
      })
      .catch((err) => {
        setHeadAccountListData([]);
      });
  };
  ...
  return (
    <Layout title="Edit  Head Account Category">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Edit  Head Account Category</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
...
export default ScHeadAccountCategoryEdit;

...
function ConfigureImcbList() {
  ...
  const getList = () => {
    setLoading(true);
    api
      .get(`${baseURL}configureImcb/list-with-join`, _params)
      .then((response) => {
        const content = response.data.content || {};
        setListData(content.configureImcb || []);
  ...
  // 👇 Navigation actions
  const handleView = (id) => {
    navigate(`/seriui/configure-imcb-view/${id}`);
  };

  const handleEdit = (id) => {
    navigate(`/seriui/configure-imcb-edit/${id}`);
  };
  ...
  const deleteConfirm = (id) => {
    Swal.fire({
      ...
      if (result.value) {
        api
          .delete(`${baseURL}configureImcb/delete/${id}`)
          .then(() => {
            getList();
  ...
  // ✅ Columns based on ConfigureImcbResponse (joined data)
  const ConfigureImcbColumns = [
    {
      name: t("Action"),
      cell: (row) => (
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            ...
            size="sm"
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
            onClick={() => deleteConfirm(row.imcbId)}
          >
            <Icon name="trash" />
            {t("Delete")}
          </Button>
  ...
  return (
    <Layout title={t("Configure Imcb List")}>
      <style>{configureImcbListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Configure Imcb List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
export default ConfigureImcbList;

...
function ConfigureImcbEdit() {
  ...
  const getIdList = () => {
    setLoading(true);
    api
      .get(`${baseURL}configureImcb/get/${id}`)
      .then((res) => {
      ...
      .catch((err) => {
        ...
        Swal.fire({
          icon: "error",
          title: message,
        }).then(() => navigate("/seriui/configure-imcb-list"));
        setLoading(false);
      });
  };
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(`${baseURL}configureImcb/edit`, { ...data, configureImcbId: id })
        .then((res) => {
  ...
  return (
    <Layout title={t("Edit Configure IMCB/MERM")}>
      <style>{configureImcbEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Configure IMCB/MERM")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
          ...
          <Row className="g-3">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="edit" />
                <span>{t("Edit Configure IMCB/MERM")}</span>
              </Card.Header>
              <Card.Body>
                {loading ? (
...
export default ConfigureImcbEdit;

...
function GovtAccount() {
  ...
  const postData = (event) => {
    ...
    event.preventDefault();
    api
      .post(baseURL + `govtAccount/add`, data)
      .then((response) => {
        if (response.data?.content?.error) {
          Swal.fire({ icon: "error", title: t("Error"), text: response.data.content.error_description });
        } else {
          Swal.fire({ icon: "success", title: t("Saved successfully") }).then(() =>
      ...
      .catch((err) => {
        const msg =
          ...
          err.message ||
          t("Save attempt was not successful");
  ...
  return (
    <Layout title={t("Govt Account")}>
      <style>{govtAccountStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Govt Account")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
                  <Link to="/seriui/govt-account-list" className="btn btn-primary btn-md d-md-none sh-cta-btn">
                    <Icon name="arrow-long-left" />
                    <span>{t("Go To List")}</span>
                  </Link>
                </li>
                <li>
                  <Link to="/seriui/govt-account-list" className="btn btn-primary d-none d-md-inline-flex sh-cta-btn">
                    <Icon name="arrow-long-left" />
                    <span>{t("Go To List")}</span>
                  </Link>
                </li>
          ...
          <Row className="g-3">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="setting" />
                <span>{t("Govt Account Details")}</span>
              </Card.Header>
              <Card.Body>
                <Row className="g-gs">

...
function RejectReasonWorkFlowList() {
  ...
  const RejectReasonWorkFlowDataColumns = [
    ...
    },
    {
      ...
      cell: (row) => <span>{row.reason ? t(row.reason) : row.reason}</span>,
      sortable: true,
      hide: "md",
    },
  ...
  return (
    <Layout title="List of Reject Reason Work Flow">
      <style>{rejectReasonWorkFlowListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("List of Reject Reason Work Flow")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const rejectReasonWorkFlowListStyles = `
  .sh-page-header {
    padding: 20px 24px;

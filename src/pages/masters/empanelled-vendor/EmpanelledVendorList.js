...
function EmpanelledVendorList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `vendorMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.vendorMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `vendorMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Empaneled Vendor List")}>
      <style>{empanelledVendorListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Empaneled Vendor List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={EmpanelledVendorDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const empanelledVendorListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default EmpanelledVendorList;

...
function TrainingCourseEdit() {
  ...
  const postData = (event) => {
    const datas = {
      trCourseMasterId: id,
      trCourseMasterName: data.trCourseMasterName,
      trCourseNameInKannada: data.trCourseNameInKannada,
    };
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trCourseMaster/get/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="Edit Training Course">
      <style>{trainingCourseEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Training Course")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
          ...
          <Row className="g-3 ">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="edit" />
                <span>{t("Training Course Details")}</span>
              </Card.Header>
              <Card.Body>
                {loading ? (
...
const trainingCourseEditStyles = `
  .sh-page-header {
    padding: 20px 24px;

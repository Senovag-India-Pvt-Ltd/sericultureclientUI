...
function TrainingCourseList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trCourseMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.trCourseMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trCourseMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Training Courses List">
      <style>{trainingCourseListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Courses List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={TrainingCoursesDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const trainingCourseListStyles = `
  .sh-page-header {
    padding: 20px 24px;

...
function TrainingDeputedInstituteView() {
  ...
  const [DeputedInstituteMaster, setDeputedInstituteMaster] = useState({});
  const [loading, setLoading] = useState(false);

  // grabs the id form the url and loads the corresponding data
  // useEffect(() => {
  //   let findUser = data.find((item) => item.id === id);
  //   setDeputedInstituteMaster(findUser);
  // }, [id, data]);
  const getIdList = () => {
    setLoading(true);
    const response = api
      ...
      .then((response) => {
        setDeputedInstituteMaster(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setDeputedInstituteMaster({});
        setLoading(false);
      });
  };
  ...
  return (
    <Layout title="Deputed institute View">
      <style>{trainingDeputedInstituteViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Deputed Institute View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Deputed Institute Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
const trainingDeputedInstituteViewStyles = `
  .sh-page-header {
    padding: 20px 24px;

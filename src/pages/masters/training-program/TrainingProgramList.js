...
function TrainingProgramList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trProgramMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.trProgramMaster);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/trainingPrograms-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/trainingPrograms-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trProgramMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const TrainingProgramsDataColumns = [
    {
      name: t("Action"),
      cell: (row) => (
        //   Button style
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.trProgramMasterId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.trProgramMasterId)}
          >
             <Icon name="edit" />
             {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.trProgramMasterId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
    ...
    {
      name:t("Training Program Name in Kannada"),
      selector: (row) => row.trProgramNameInKannada,
      cell: (row) => <span>{row.trProgramNameInKannada}</span>,
      sortable: true,
      hide: "md",
    },
  ];
  ...
  return (
    <Layout title="Training Programs List">
      <style>{trainingProgramListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Program List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={TrainingProgramsDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const trainingProgramListStyles = `
  .sh-page-header {
    padding: 20px 24px;

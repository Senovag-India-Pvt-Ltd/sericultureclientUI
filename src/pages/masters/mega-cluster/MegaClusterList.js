...
function MegaClusterList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `megaCluster/list`, _params)
      .then((response) => {
        setListData(response.data.content.megaCluster);
  ...
  const search = (e) => {
    let joinColumn;
    if (data.searchBy === "megaCluster") {
      joinColumn = "megaCluster.name";
    }
    api
      .post(baseURL + `megaCluster/search`, {
  ...
  const handleView = (_id) => {
    navigate(`/seriui/mega-cluster-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/mega-cluster-edit/${_id}`);
  };
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `megaCluster/delete/${_id}`)
          .then((response) => {
            getList();
            Swal.fire(
              "Deleted",
  ...
  const MegaClusterDataColumns = [
      ...
      cell: (row) => (
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.megaClusterId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.megaClusterId)}
          >
            <Icon name="edit" />
            <span>{t("Edit")}</span>
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.megaClusterId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            <span>{t("delete")}</span>
    ...
    {
      name: t("Mega Cluster Name in Kannada"),
  ...
  return (
    <Layout title="Mega Cluster List">
      <style>{megaClusterListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Mega Cluster List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          ...
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={MegaClusterDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
export default MegaClusterList;

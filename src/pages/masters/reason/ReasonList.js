...
function ReasonList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `reason/list`, _params)
      .then((response) => {
        setListData(response.data.content.reason);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/reason-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/reason-edit/${_id}`);
      ...
      cell: (row) => (
        ...
        <div className="text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.reasonId)}
          >
            {t("View")}
          </Button>
          <Button
          ...
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.reasonId)}
            className="ms-2"
          >
            {t("delete")}
          </Button>
  ...
  return (
    <Layout title={t("Reason List")}>
      <style>{reasonListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Reason List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

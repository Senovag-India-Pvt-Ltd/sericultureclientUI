...
function ReasonEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `reason/edit`,{...data,reasonId:id})
        .then((response) => {
  ...
  return (
    <Layout title={t("Edit Reason")}>
      <style>{reasonEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Reason")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
                  <Link
                    to="/seriui/reason-list"

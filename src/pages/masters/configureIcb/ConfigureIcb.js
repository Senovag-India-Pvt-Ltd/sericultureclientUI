...
//     }, []);
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(`${baseURL}configureIcb/add`, data)
        .then((response) => {
  ...
  return (
    <Layout title={t("Configure ICB")}>
      <style>{configureIcbStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Configure ICB")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
          ...
          <Row className="g-3">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="setting" />
                <span>{t("Configure ICB Details")}</span>
              </Card.Header>
              <Card.Body>
                <Row className="g-gs">
                  {/* ICB Basin Ends */}

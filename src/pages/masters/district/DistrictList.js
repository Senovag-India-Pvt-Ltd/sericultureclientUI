...
function DistrictList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `district/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.district);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `district/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="District List">
      <style>{districtListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("District List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

...
function MulberryVarietyList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `mulberry-variety/list`, _params)
      .then((response) => {
        setListData(response.data.content.mulberryVariety);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `mulberry-variety/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const MulberryVarietyDataColumns = [
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.mulberryVarietyId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.mulberryVarietyId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.mulberryVarietyId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
    ...
    {
      name: t("Mulberry Variety Name in Kannada"),
      selector: (row) => row.mulberryVarietyNameInKannada,
      cell: (row) => <span>{row.mulberryVarietyNameInKannada}</span>,
      sortable: true,
      hide: "md",
    },
  ];
  ...
  return (
    <Layout title="Mulberry Variety List">
      <style>{mulberryVarietyListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Mulberry Variety List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={MulberryVarietyDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const mulberryVarietyListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default MulberryVarietyList;

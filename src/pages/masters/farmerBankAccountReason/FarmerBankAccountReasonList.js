...
function FarmerBankAccountReasonList() {
  // Translation
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `farmerBankAccountReason/list`, _params)
      .then((response) => {
        setListData(response.data.content.farmerBankAccountReason);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/farmerBankAccountReason-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/farmerBankAccountReason-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `reason/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const ReasonDataColumns = [
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.farmerBankAccountReasonId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.farmerBankAccountReasonId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.farmerBankAccountReasonId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
    ...
    {
      name: t("Farmer Bank Account Reason"),
      selector: (row) => row.farmerBankAccountReason,
      cell: (row) => <span>{row.farmerBankAccountReason}</span>,
      sortable: true,
      hide: "md",
    },
  ...
  return (
    <Layout title="Farmer Bank Reason  List">
      <style>{farmerBankAccountReasonListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Farmer Bank Reason List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const farmerBankAccountReasonListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default FarmerBankAccountReasonList;

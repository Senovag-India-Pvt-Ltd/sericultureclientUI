...
function TrainingModeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trModeMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.trModeMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trModeMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.trModeMasterId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.trModeMasterId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.trModeMasterId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
  ...
  return (
    <Layout title="Training Mode List">
      <style>{trainingModeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Mode List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={TrModeMasterDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
export default TrainingModeList;

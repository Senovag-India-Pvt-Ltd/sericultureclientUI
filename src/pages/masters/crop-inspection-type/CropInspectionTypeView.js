...
function CropInspectionTypeView() {
  ...
  const [CropInspectionType, setCropInspectionType] = useState({});
  ...
  const getIdList = () => {
    ...
    const response = api
      ...
      .then((response) => {
        setCropInspectionType(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setCropInspectionType({});
        setLoading(false);
      });
  };
  ...
  <Block.Head>
    ...
    <Block.HeadBetween>
      <Block.HeadContent>
        <Block.Title tag="h2" className="sh-page-title">{t("Crop Inspection Type View")}</Block.Title>
      </Block.HeadContent>
      <Block.HeadContent>
        <ul className="d-flex">
          <li>
  ...
  <Block className="mt-4 sh-form-wrap">
    <Card>
      <Card.Header className="sh-section-header">
        <Icon name="eye" />
        <span>{t("Crop Inspection Type Details")}</span>
      </Card.Header>
      <Card.Body>
        {loading ? (
...
const cropInspectionTypeViewStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default CropInspectionTypeView;

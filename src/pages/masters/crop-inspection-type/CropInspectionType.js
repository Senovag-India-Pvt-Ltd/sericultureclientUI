...
function CropInspectionType() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `cropInspectionType/add`, data)
        .then((response) => {
...
<Layout title={t("Crop Inspection Type")}>
  <style>{cropInspectionTypeStyles}</style>
  <Block.Head>
    <div className="sh-page-header">
    <Block.HeadBetween>
      <Block.HeadContent>
        <Block.Title tag="h2" className="sh-page-title">{t("Crop Inspection Type")}</Block.Title>
      </Block.HeadContent>
      <Block.HeadContent>
        <ul className="d-flex">
          <li>
...
const cropInspectionTypeStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default CropInspectionType;

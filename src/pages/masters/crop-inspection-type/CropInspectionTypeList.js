...
function CropInspectionTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `cropInspectionType/list`, _params)
      .then((response) => {
        setListData(response.data.content.cropInspectionType);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/crop-inspection-type-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/crop-inspection-type-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `cropInspectionType/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  <style>{cropInspectionTypeListStyles}</style>
  <Block.Head>
    <div className="sh-page-header">
    <Block.HeadBetween>
      <Block.HeadContent>
        <Block.Title tag="h2" className="sh-page-title">{t('Crop Inspection Type List')}</Block.Title>
      </Block.HeadContent>
      <Block.HeadContent>
        <ul className="d-flex">
          <li>
...
const cropInspectionTypeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default CropInspectionTypeList;

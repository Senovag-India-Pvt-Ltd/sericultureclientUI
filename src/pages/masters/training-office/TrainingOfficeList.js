...
function TrainingOfficeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trOffice/list`, _params)
      .then((response) => {
        setListData(response.data.content.trOffice);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trOffice/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Training Office List">
      <style>{trainingOfficeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Office List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const trainingOfficeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default TrainingOfficeList;

...
function ExternalUnitTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `externalUnitType/list`, _params)
      .then((response) => {
        setListData(response.data.content.externalUnitType);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `externalUnitType/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("External Unit Type List")}>
      <style>{externalUnitTypeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("External Unit Type List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={ExternalUnitTypeDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const externalUnitTypeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default ExternalUnitTypeList;

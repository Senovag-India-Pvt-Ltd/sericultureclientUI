...
function RelationshipList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `relationship/list`, _params)
      .then((response) => {
        setListData(response.data.content.relationship);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `relationship/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Relationship List">
      <style>{relationshipListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Relationship List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <DataTable
            ...
            columns={RelationshipDataColumns}
            data={listData}
            highlightOnHover
            pagination
            paginationServer

...
function ScProgramList() {
  ...
  const getList = () => {
    setLoading(true);
    api
      .get(baseURL + `scProgram/list`, _params)
      .then((response) => {
        setListData(response.data.content.scProgram);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        api
          .delete(baseURL + `scProgram/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Program List">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Program List</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>

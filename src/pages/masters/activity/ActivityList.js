...
function ActivityList() {
  ...
  const handleView = (id) => {
    navigate(`/seriui/activity-view/${id}`);
  };

  const handleEdit = (id) => {
    navigate(`/seriui/activity-edit/${id}`);
  };
  ...
  return (
    <Layout title="Activity List">
      <style>{activityListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Activity List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

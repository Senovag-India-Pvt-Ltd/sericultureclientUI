...
function HelpDeskFeatureList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdFeatureMaster/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.hdFeatureMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdFeatureMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Feature List">
      <style>{helpDeskFeatureListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Feature List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const helpDeskFeatureListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskFeatureList;

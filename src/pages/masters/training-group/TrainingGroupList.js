...
function TrainingGroupList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `trGroupMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.trGroupMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `trGroupMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Training Group List">
      <style>{trainingGroupListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Training Group List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <DataTable
            ...
            columns={TrGroupMasterDataColumns}
            data={listData}
            highlightOnHover
            pagination
            paginationServer

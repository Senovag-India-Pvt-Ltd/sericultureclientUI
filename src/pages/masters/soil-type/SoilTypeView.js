...
function SoilTypeView() {
  ...
  const getIdList = () => {
    ...
    const response = api
      ...
      .then((response) => {
        setSoilType(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setSoilType({});
  ...
  return (
    <Layout title="Soil Type View">
      <style>{soilTypeViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Soil Type View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

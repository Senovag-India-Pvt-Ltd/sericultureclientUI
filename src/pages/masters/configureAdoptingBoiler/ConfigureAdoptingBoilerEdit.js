...
function ConfigureAdoptingBoilerEdit() {
  ...
  const getIdList = () => {
    setLoading(true);
    api
      .get(`${baseURL}configureAdoptingBoiler/get/${id}`)
      .then((res) => {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(`${baseURL}configureAdoptingBoiler/edit`, { ...data, adoptingBoilerId: id })
        .then((res) => {
  ...
  return (
    <Layout title={t("Edit Configure Adapting Boiler")}>
      <style>{configureAdoptingBoilerEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Configure Adapting Boiler")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
          ...
          <Row className="g-3">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="edit" />
                <span>{t("Edit Configure Adapting Boiler")}</span>
              </Card.Header>
              <Card.Body>
                {loading ? (
...
const configureAdoptingBoilerEditStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default ConfigureAdoptingBoilerEdit;

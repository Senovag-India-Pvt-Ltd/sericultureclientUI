...
function ConfigureAdoptingBoilerList() {
  ...
  // ✅ Fetch Configure imcb list with JOIN
  const getList = () => {
    setLoading(true);
    api
      .get(`${baseURL}configureAdoptingBoiler/list-with-join`, _params)
      .then((response) => {
        const content = response.data.content || {};
        setListData(content.configureAdoptingBoiler || []);
  ...
  // 👇 Navigation actions
  const handleView = (id) => {
    navigate(`/seriui/configure-adopting-boiler-view/${id}`);
  };

  const handleEdit = (id) => {
    navigate(`/seriui/configure-adopting-boiler-edit/${id}`);
  };
  ...
  const deleteConfirm = (id) => {
    Swal.fire({
      ...
      if (result.value) {
        api
          .delete(`${baseURL}configureAdoptingBoiler/delete/${id}`)
          .then(() => {
            getList();
  ...
  // ✅ Columns based on ConfigureAdoptingBoilerResponse (joined data)
  const ConfigureAdoptingBoilerColumns = [
    {
      name: t("Action"),
      cell: (row) => (
        <div className="text-start w-100">
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.adoptingBoilerId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.adoptingBoilerId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            ...
            size="sm"
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm"
            onClick={() => deleteConfirm(row.adoptingBoilerId)}
          >
            <Icon name="trash" />
            {t("Delete")}
          </Button>
  ...
  return (
    <Layout title={t("Configure Adapting Boiler List")}>
      <style>{configureAdoptingBoilerListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Configure Adapting Boiler List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <DataTable
            ...
            columns={ConfigureAdoptingBoilerColumns}
            data={listData}
            highlightOnHover
            pagination
            paginationServer
...
const configureAdoptingBoilerListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default ConfigureAdoptingBoilerList;

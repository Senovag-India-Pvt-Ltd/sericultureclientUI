...
function HelpDeskStatusList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdStatusMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.hdStatusMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdStatusMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.hdStatusId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.hdStatusId)}
          >
            <Icon name="edit" />
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.hdStatusId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
  ...
  return (
    <Layout title="Help Desk Status ">
      <style>{helpDeskStatusListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Help Desk Status")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const helpDeskStatusListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskStatusList;

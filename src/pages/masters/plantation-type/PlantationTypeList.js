...
function PlantationTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `plantationType/list`, _params)
      .then((response) => {
        setListData(response.data.content.plantationType);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `plantationType/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Plantation Type List">
      <style>{plantationTypeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("Plantation Type List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={PlantationTypeDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const plantationTypeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default PlantationTypeList;

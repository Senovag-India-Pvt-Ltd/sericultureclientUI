...
function GenerationNumberList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `generationNumberMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.generationNumberMaster);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/generation-number-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/generation-number-edit/${_id}`);
    // navigate("/seriui/generationNumber");
  };
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `generationNumberMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const GenerationNumberDataColumns = [
    {
      name: t("Action"),
      cell: (row) => (
        //   Button style
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.generationNumberId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.generationNumberId)}
          >
            {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.generationNumberId)}
            className="ms-2 text-nowrap"
          >
            {t("delete")}
          </Button>
  ...
  return (
    <Layout title={t("Generation Number List")}>
      <style>{generationNumberListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Generation Number List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={GenerationNumberDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const generationNumberListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default GenerationNumberList;

...
function GenerationNumberView() {
  ...
  //   setGenerationNumber(findUser);
  // }, [id, data]);
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `generationNumberMaster/get/${id}`)
      .then((response) => {
        setGenerationNumber(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setGenerationNumber({});
  ...
  return (
    <Layout title={t("Generation Number View")}>
      <style>{generationNumberViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Generation Number View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Generation Number Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
const generationNumberViewStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default GenerationNumberView;

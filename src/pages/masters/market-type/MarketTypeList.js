...
function MarketTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `marketTypeMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.marketTypeMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `marketTypeMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          {/* <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.marketTypeMasterId)}
            className="ms-2"
          >
            Delete
          </Button> */}
  ...
  return (
    <Layout title={t("Market Type List")}>
      <style>{marketTypeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Market Type List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

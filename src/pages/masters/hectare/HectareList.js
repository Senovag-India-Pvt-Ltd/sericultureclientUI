...
function HectareList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hectareMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.hectareMaster);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/hectare-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/hectare-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hectareMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          {/* <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.hectareId)}
          >
            View
          </Button> */}
          <Button
  ...
  return (
    <Layout title="List Of Hectare">
      <style>{hectareListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("List Of Hectare")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

...
function CrateEdit() {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `crateMaster/get/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title={t("Edit Crate")}>
      <style>{crateEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Crate")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

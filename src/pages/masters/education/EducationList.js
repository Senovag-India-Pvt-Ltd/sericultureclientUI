...
function EducationList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `education/list`, _params)
      .then((response) => {
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `education/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Education List">
      <style>{educationListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Education List")}</Block.Title>
              {/* <nav>

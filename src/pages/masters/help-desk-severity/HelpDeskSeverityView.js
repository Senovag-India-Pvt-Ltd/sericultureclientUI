...
function HelpDeskSeverityView() {
  ...
  const { id } = useParams();
  // const [data] = useState(LandSeverityDatas);
  const [HdSeverity, setHdSeverity] = useState({});
  const [loading, setLoading] = useState(false);

  // grabs the id form the url and loads the corresponding data
  // useEffect(() => {
  //   let findUser = data.find((item) => item.id === id);
  //   setHdSeverity(findUser);
  // }, [id, data]);
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdSeverityMaster/get/${id}`)
      .then((response) => {
        setHdSeverity(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setHdSeverity({});
  ...
  return (
    <Layout title="Help Desk Severity View">
      <style>{helpDeskSeverityViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Help Desk Severity View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const helpDeskSeverityViewStyles = `
  .sh-page-header {
    padding: 20px 24px;

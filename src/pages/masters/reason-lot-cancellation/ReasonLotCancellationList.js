...
function ReasonLotCancellationList() {
  ...
  const getList = () => {
    setLoading(true);
    api
      .get(baseURL + `reason-lot-reject-master/list`, _params)
      .then((response) => {
        setListData(response.data.content.reasonLotRejectMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        api
          .delete(baseURL + `reason-lot-reject-master/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Reason for Lot Cancellation List")}>
      <style>{reasonLotCancellationListStyles}</style>
      <Block.Head>
...
const reasonLotCancellationListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default ReasonLotCancellationList;

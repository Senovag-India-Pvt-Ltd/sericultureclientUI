...
function HelpDeskCategoryList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdCategoryMaster/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.hdCategoryMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdCategoryMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.hdCategoryId)}
          ...
          <Button
            ...
            onClick={() => handleEdit(row.hdCategoryId)}
          >
             <Icon name="edit" />
             {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.hdCategoryId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            {t("delete")}
  ...
  return (
    <Layout title="Category">
      <style>{helpDeskCategoryListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Category")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const helpDeskCategoryListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskCategoryList;

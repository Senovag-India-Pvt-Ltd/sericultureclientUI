...
function ScVendorBankList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `scVendorBank/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.ScVendorBank);
  ...
  const search = (e) => {
    ...
    if (data.searchBy === "scVendorBank") {
      joinColumn = "scVendorBank.bankName";
    }
    console.log(joinColumn);
    api
      .post(baseURL + `scVendorBank/search`, {
  ...
  const handleView = (_id) => {
    navigate(`/seriui/sc-vendor-bank-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/sc-vendor-bank-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `scVendorBank/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Vendor Bank List">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Vendor Bank List</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
                ...
                <Col sm={3}>
                  <div className="form-control-wrap">
                    ...
                    >
                      ...
                      <option value="scVendorBank">VendorBank</option>

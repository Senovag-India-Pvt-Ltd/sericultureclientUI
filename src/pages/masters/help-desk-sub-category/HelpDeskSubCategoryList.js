...
function HelpDeskSubCategoryList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdSubCategoryMaster/list-with-join`, _params)
      .then((response) => {
        setListData(response.data.content.hdSubCategoryMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdSubCategoryMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Sub Category List">
      <style>{helpDeskSubCategoryListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Sub Category List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={HdSubCategoryDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const helpDeskSubCategoryListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskSubCategoryList;

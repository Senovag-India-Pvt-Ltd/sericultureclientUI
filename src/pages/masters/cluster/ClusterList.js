...
function ClusterList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `cluster/list`, _params)
      .then((response) => {
        setListData(response.data.content.cluster);
  ...
  const search = (e) => {
    let joinColumn;
    if (data.searchBy === "cluster") {
      joinColumn = "cluster.name";
    }
    api
      .post(baseURL + `cluster/search`, {
  ...
  const handleView = (_id) => {
    navigate(`/seriui/cluster-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/cluster-edit/${_id}`);
  };
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `cluster/delete/${_id}`)
          .then((response) => {
            getList();
            Swal.fire(
              "Deleted",
  ...
  const ClusterDataColumns = [
      ...
      cell: (row) => (
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          <Button
            ...
            onClick={() => handleView(row.clusterId)}
          >
            <Icon name="eye" />
            <span>{t("View")}</span>
          </Button>
          <Button
            ...
            onClick={() => handleEdit(row.clusterId)}
          >
            <Icon name="edit" />
            <span>{t("Edit")}</span>
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.clusterId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            <span>{t("delete")}</span>
    ...
    {
      name: t("Cluster Name in Kannada"),
  ...
  return (
    <Layout title="Cluster List">
      <style>{clusterListStyles}</style>
      <Block.Head>

...
function WorkingInstitutionList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `workingInstitution/list`, _params)
      .then((response) => {
        setListData(response.data.content.workingInstitution);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `workingInstitution/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="WorkingInstitutions List">
      <style>{workingInstitutionListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("WorkingInstitution List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              // title="WorkingInstitution List"
              tableClassName="data-table-head-light table-responsive"
              columns={WorkingInstitutionsDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const workingInstitutionListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default WorkingInstitutionList;

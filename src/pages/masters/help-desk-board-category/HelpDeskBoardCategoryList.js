...
function HelpDeskBoardCategoryList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `hdBoardCategoryMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.hdBoardCategoryMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `hdBoardCategoryMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Broad Category">
      <style>{helpDeskBoardCategoryListStyles}</style>
      <Block.Head>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={HdBoardCategoryDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer
...
const helpDeskBoardCategoryListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default HelpDeskBoardCategoryList;

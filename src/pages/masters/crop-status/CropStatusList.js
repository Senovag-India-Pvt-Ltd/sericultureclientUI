...
function CropStatusList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `cropStatus/list`, _params)
      .then((response) => {
        setListData(response.data.content.cropStatus);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/crop-status-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/crop-status-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `cropStatus/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
...
<Layout title={t("Crop Status List")}>
  <style>{cropStatusListStyles}</style>
  <Block.Head>
    <div className="sh-page-header">
      <Block.HeadBetween>
        <Block.HeadContent>
          <Block.Title tag="h2" className="sh-page-title">{t("Crop Status List")}</Block.Title>
        </Block.HeadContent>
        <Block.HeadContent>
          <ul className="d-flex">
            <li>

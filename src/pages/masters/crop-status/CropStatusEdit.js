...
function CropStatusEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `cropStatus/edit`,{...data,cropStatusId:id})
        .then((response) => {
 ...
 <Layout title={t("Edit Crop Status")}>
  <style>{cropStatusEditStyles}</style>
  <Block.Head>
    <div className="sh-page-header">
      <Block.HeadBetween>
        <Block.HeadContent>
          <Block.Title tag="h2" className="sh-page-title">{t("Edit Crop Status")}</Block.Title>
        </Block.HeadContent>
        <Block.HeadContent>
          <ul className="d-flex">
            <li>

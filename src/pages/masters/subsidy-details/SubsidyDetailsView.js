...
function SubsidyDetailsView() {
  ...
  const { id } = useParams();
  // const [data] = useState(SubsidyDatas);
  ...
  const getIdList = () => {
    ...
    const response = api
      ...
      .then((response) => {
        setSubsidy(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setSubsidy({});
  ...
  return (
    <Layout title="Rearing Equipment Details View">
      <style>{subsidyDetailsViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Rearing Equipment Details View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Rearing Equipment Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
const subsidyDetailsViewStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default SubsidyDetailsView;

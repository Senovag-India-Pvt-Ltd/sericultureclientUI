...
function ReelerTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `reelerTypeMaster/list`, _params)
      .then((response) => {
        setListData(response.data.content.reelerTypeMaster);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `reelerTypeMaster/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Reeler Type List")}>
      <style>{reelerTypeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Reeler Type List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <DataTable
            ...
            columns={ReleerTypeMasterDataColumns}
            data={listData}
            highlightOnHover
            pagination
            paginationServer

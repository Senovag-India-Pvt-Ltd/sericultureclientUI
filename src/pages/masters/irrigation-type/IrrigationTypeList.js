...
function IrrigationTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `irrigationType/list`, _params)
      .then((response) => {
        setListData(response.data.content.irrigationType);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `irrigationType/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Irrigation Type List">
      <style>{irrigationTypeListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("Irrigation Type List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
...
const irrigationTypeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default IrrigationTypeList;

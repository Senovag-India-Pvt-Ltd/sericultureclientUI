...
function DiseaseStatusList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `diseaseStatus/list`, _params)
      .then((response) => {
        setListData(response.data.content.diseaseStatus);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/disease-status-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/disease-status-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `diseaseStatus/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title={t("Disease Status List")}>
      <style>{diseaseStatusListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Disease Status List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={DiseaseStatusDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer

...
function SpacingEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `spacingMaster/edit`, data)
        .then((response) => {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `spacingMaster/get/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="Edit Spacing">
      <style>{spacingEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Spacing")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

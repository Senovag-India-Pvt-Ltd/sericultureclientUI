...
function TscEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      event.preventDefault();
      api
        .post(baseURL + `tscMaster/edit`, datas)
        .then((response) => {
  ...
  return (
    <Layout title={t("Edit Tsc")}>
      <style>{tscEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Edit Tsc")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

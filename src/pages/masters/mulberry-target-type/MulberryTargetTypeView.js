...
function MulberryTargetTypeView() {
  ...
  const [mulberryTargetType, setMulberryTargetType] = useState({});
  ...
  const getIdList = () => {
    ...
    const response = api
      ...
      .then((response) => {
        setMulberryTargetType(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setMulberryTargetType({});
        setLoading(false);
      });
  };
  ...
  return (
    <Layout title="Mulberry Target Type View">
      <Block.Head>
        <Block.HeadBetween>
          <Block.HeadContent>
            <Block.Title tag="h2">Mulberry Target Type View</Block.Title>
          </Block.HeadContent>
          <Block.HeadContent>
            <ul className="d-flex">
              <li>
      ...
      <Block className="mt-4">
        <Card>
          <Card.Header>Mulberry Target Type Details</Card.Header>
          <Card.Body>
            {loading ? (
...
export default MulberryTargetTypeView;

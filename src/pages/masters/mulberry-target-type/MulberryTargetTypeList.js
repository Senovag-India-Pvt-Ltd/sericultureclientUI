...
function MulberryTargetTypeList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `mulberryTargetType/list`, _params)
      .then((response) => {
        setListData(response.data.content.mulberryTargetType);
  ...
  const handleView = (_id) => {
    navigate(`/seriui/mulberry-target-type-view/${_id}`);
  };

  const handleEdit = (_id) => {
    navigate(`/seriui/mulberry-target-type-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `mulberryTargetType/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
      ...
      cell: (row) => (
        ...
        <div className="d-flex flex-nowrap align-items-center text-start w-100">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => handleView(row.mulberryTargetTypeId)}
          ...
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.mulberryTargetTypeId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm text-nowrap"
          >
            <Icon name="trash" />
            Delete
  ...
  return (
    <Layout title="Mulberry Target Type List">
      <style>{mulberryTargetTypeListStyles}</style>
      <Block.Head>
...
const mulberryTargetTypeListStyles = `
  .sh-page-header {
    padding: 20px 24px;
...
export default MulberryTargetTypeList;

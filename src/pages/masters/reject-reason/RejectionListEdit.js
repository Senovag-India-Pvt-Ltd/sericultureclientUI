...
function RejectionListEdit() {
  ...
  const postData = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // event.stopPropagation();
      api
        .post(baseURL + `rejection-list/edit`, data)
        .then((response) => {
  ...
  const getIdList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `rejection-list/get/${id}`)
      .then((response) => {
  ...
  return (
    <Layout title="Reject Reason">
      <style>{rejectionListEditStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Reject")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
          ...
          <Row className="g-3 ">
            <Card>
              <Card.Header className="sh-section-header">
                <Icon name="edit" />
                <span>{t("Reject Reason Details")}</span>
              </Card.Header>
              <Card.Body>
...
export default RejectionListEdit;

...
function RejectReasonList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `rejection-list/list`, _params)
      .then((response) => {
        setListData(response.data.content.rejectionList);
...
//   };
  ...
  const handleEdit = (_id) => {
    navigate(`/seriui/reject-reason-edit/${_id}`);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `rejection-list/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  const RejectReasonWorkFlowDataColumns = [
      ...
      cell: (row) => (
        ...
        <div className="text-start w-100">
          ...
          <Button
            ...
            onClick={() => handleEdit(row.rejectionListId)}
          >
             <Icon name="edit" />
             {t("Edit")}
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteConfirm(row.rejectionListId)}
            className="ms-2 d-inline-flex align-items-center gap-1 shadow-sm"
          >
             <Icon name="trash" />
             {t("delete")}
    ...
    {
      name:  t("Reason"),
      selector: (row) => row.rejectionListName,
      cell: (row) => <span>{row.rejectionListName}</span>,
      sortable: true,
      hide: "md",
    },
  ...
  return (
    <Layout title="List of Reject Reason">
      <style>{rejectReasonListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("List of Reject Reason")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>

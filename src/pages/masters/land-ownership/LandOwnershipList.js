...
function LandOwnershipList() {
  ...
  const getList = () => {
    setLoading(true);
    const response = api
      .get(baseURL + `landOwnership/list`, _params)
      .then((response) => {
        setListData(response.data.content.landOwnership);
  ...
  const deleteConfirm = (_id) => {
    Swal.fire({
      ...
      if (result.value) {
        const response = api
          .delete(baseURL + `landOwnership/delete/${_id}`)
          .then((response) => {
            // deleteConfirm(_id);
            getList();
            Swal.fire(
  ...
  return (
    <Layout title="Land Ownership List">
      <style>{landOwnershipListStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title"> {t("Land Ownership List")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
        ...
        <Card>
          <div style={{ overflowX: "auto" }}>
            <DataTable
              ...
              columns={LandOwnershipDataColumns}
              data={listData}
              highlightOnHover
              pagination
              paginationServer

...
function LandOwnershipView() {
  ...
  const getIdList = () => {
    ...
    const response = api
      ...
      .then((response) => {
        setLandOwnership(response.data.content);
        setLoading(false);
      })
      .catch((err) => {
        setLandOwnership({});
  ...
  return (
    <Layout title="Land Ownership View">
      <style>{landOwnershipViewStyles}</style>
      <Block.Head>
        <div className="sh-page-header">
          <Block.HeadBetween>
            <Block.HeadContent>
              <Block.Title tag="h2" className="sh-page-title">{t("Land Ownership View")}</Block.Title>
            </Block.HeadContent>
            <Block.HeadContent>
              <ul className="d-flex">
                <li>
      ...
      <Block className="mt-n4 sh-form-wrap">
        <Card>
          <Card.Header className="sh-section-header">
            <Icon name="eye" />
            <span>{t("Land Ownership Details")}</span>
          </Card.Header>
          <Card.Body>
            {loading ? (
...
export default LandOwnershipView;
